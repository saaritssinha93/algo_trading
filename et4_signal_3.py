...
def detect_signals_in_memory(ticker, df_today, existing_signal_ids):
    ...
    if adx_val <= 25:  # Maintained ADX threshold
        return signals_detected
    ...
    # Define thresholds for large MACD values and potential reversal boundaries
    macd_bullish_threshold = 50         # Lower bound for significant bullish momentum
    macd_bearish_threshold = -50        # Upper bound for significant bearish momentum
    macd_extreme_upper_limit = 50       # Beyond this, bullish signals might reverse
    macd_extreme_lower_limit = -50      # Beyond this, bearish signals might reverse
    ...
    # ------------------------------------------------------------------------
    # Relaxed momentum analysis functions (Approach #1: simple difference check)
    # ------------------------------------------------------------------------

    '''
    def is_macd_histogram_increasing(df, window=2, tolerance=0.00):
        """
    Return True if the MACD histogram has a significant overall rise
    in the last `window` points. A positive `tolerance` ensures that
    only substantial increases are considered, ignoring minor fluctuations.
    
    Parameters:
    - df: pandas.DataFrame containing MACD histogram data.
    - window: int, number of recent points to consider.
    - tolerance: float, minimum required increase over the window.
    
    Returns:
    - bool: True if increasing beyond tolerance, else False.
        """
        if "MACD_histogram" in df.columns and len(df["MACD_histogram"]) >= window:
            recent_vals = df["MACD_histogram"].iloc[-window:].values
            total_change = recent_vals[-1] - recent_vals[0]
            return total_change > tolerance
        return False

    def is_macd_histogram_decreasing(df, window=2, tolerance=0.00):
        """
    Return True if the MACD histogram has a significant overall drop
    in the last `window` points. A positive `tolerance` ensures that
    only substantial decreases are considered, ignoring minor fluctuations.
    
    Parameters:
    - df: pandas.DataFrame containing MACD histogram data.
    - window: int, number of recent points to consider.
    - tolerance: float, minimum required decrease over the window.
    
    Returns:
    - bool: True if decreasing beyond tolerance, else False.
        """
        if "MACD_histogram" in df.columns and len(df["MACD_histogram"]) >= window:
            recent_vals = df["MACD_histogram"].iloc[-window:].values
            total_change = recent_vals[-1] - recent_vals[0]
    ...
    def is_macd_histogram_increasing(df, window=3, tolerance=0.0):
        """
        Return True if the MACD histogram has an overall rise
        in the last window points. Tolerance can ignore tiny fluctuations.
        """
        if "MACD_histogram" in df.columns and len(df["MACD_histogram"]) >= window:
            recent_vals = df["MACD_histogram"].iloc[-window:].values
    ...
    def is_macd_histogram_decreasing(df, window=3, tolerance=0.0):
        """
        Return True if the MACD histogram has an overall drop
        in the last window points. Tolerance can ignore tiny fluctuations.
        """
        if "MACD_histogram" in df.columns and len(df["MACD_histogram"]) >= window:
            recent_vals = df["MACD_histogram"].iloc[-window:].values
    ...
    if trend_type == "Bullish":
        conditions = {
            ...
            "Breakout": (
                ...
                # Avoid triggers when MACD is overextended (potential reversal)
                & (df_today["MACD"] < macd_extreme_upper_limit)
                & (df_today["close"] > df_today["20_SMA"])
                & (df_today["Stochastic"] > 50)
                & (df_today["Stochastic"].diff() > 0)
                & (df_today["ADX"] > 35)
                # You could optionally require a rising MACD histogram:
        ...
        conditions = {
            ...
            "Breakdown": (
                ...
                & (df_today["Stochastic"].diff() < 0)
                & (df_today["ADX"] > 35)
                # Optionally require a falling MACD histogram:
                & is_macd_histogram_decreasing(df_today)
            ),
        }

    # Collect matching rows based on the conditions
    ...
    try:
        with lock:
            ...
            if not rows_to_update.empty:
                # Update 'Entry Signal' and 'CalcFlag' to 'Yes'
                df_main.loc[mask, ["Entry Signal", "CalcFlag"]] = "Yes"

                # Update 'logtime' where missing
...
def find_price_action_entries_for_today(last_trading_day):
    ...
    Utilizes parallel processing to speed up the detection across multiple CSV files.
    """
    st = india_tz.localize(datetime.combine(last_trading_day, dt_time(9, 25)))
    et = india_tz.localize(datetime.combine(last_trading_day, dt_time(14, 5)))
