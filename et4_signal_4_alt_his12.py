...
def check_bullish_pullback_advanced(
    df: pd.DataFrame, 
    idx: int,
    pivot_lookback: int = 10,
    min_pullback_pct: float = 0.02,     # 2%
    max_pullback_pct: float = 0.10,     # 10%
    rsi_oversold: float = 40,
    adx_threshold: float = 20,
    volume_dryup_factor: float = 0.6,
    volume_spike_factor: float = 1.5
) -> bool:
    """
    Checks if we're in a strong bullish trend that has recently pulled back 
    and is now resuming upwards. Returns True if conditions meet.
    
    Conditions:
      1) Trend Confirmation:
         - Price > 200_SMA
         - 20_SMA > 50_SMA (or 20_EMA > 50_EMA) to confirm a bullish stack
         - ADX above a threshold (e.g., 20) to ensure non-choppy market
      2) Pullback Criteria:
         - Identify pivot high over the last pivot_lookback bars
         - Ensure price is below that pivot by at least min_pullback_pct 
           but no more than max_pullback_pct
         - Volume declines during the pullback (dry-up)
         - RSI dips near or below a rsi_oversold threshold
      3) Reversal Confirmation:
         - A bullish candlestick bar at idx
         - Volume spike relative to average of last few bars
         - MACD or Stochastic turning back up, etc. (illustrative here)
    """

    # Basic index check
    if idx < pivot_lookback or idx >= len(df):
        return False
    
    # Required columns check
    required_cols = [
        'close', 'high', 'low', 'open', 'volume', 
        'RSI', 'ADX', '20_SMA', '50_SMA', '200_SMA', 
        'MACD', 'Signal_Line'
    ]
    ...
    # 1) Trend Confirmation
    # Price above 200_SMA
    if row['close'] <= row['200_SMA']:
        return False
    
    # 20_SMA > 50_SMA
    if row['20_SMA'] <= row['50_SMA']:
        return False
    ...
    # 2a) Identify pivot high over last pivot_lookback bars
    recent_data = df.iloc[idx - pivot_lookback:idx]
    pivot_high = recent_data['high'].max()
    if pivot_high <= 0:
        return False
    
    # 2b) Calculate pullback percentage
    current_pullback_pct = (pivot_high - row['close']) / pivot_high
    if not (min_pullback_pct <= current_pullback_pct <= max_pullback_pct):
        return False
    
    # 2c) Volume analysis: ensure volume has dried up during the pullback
    #    Compare average volume over the last pivot_lookback bars to 
    #    average volume over the pullback bars.
    pullback_vol_avg = recent_data['volume'].mean()
    
    # We can also check the volume just before the pivot to see if
    # it was higher. For simplicity, let's just ensure the 
    # most recent bar's volume is lower than some factor of that average.
    if row['volume'] > volume_dryup_factor * pullback_vol_avg:
        # means the current bar volume isn't that "dry." It's too high (yet).
        return False
    
    # 2d) RSI check: near oversold or at least below some threshold
    if row['RSI'] > rsi_oversold:
        return False
    
    # 3) Reversal Confirmation
    
    # 3a) Candle is bullish
    if row['close'] <= row['open']:
        return False
    
    # 3b) Volume spike compared to the last pivot_lookback bars?
    #    Optionally, you can require the *next* bar to have the volume spike.
    #    For demonstration, let's require the current bar to show a pick-up 
    #    from the *immediate prior bar* (or prior average).
    if idx < 1:
        return False
    prev_vol = df.iloc[idx - 1]['volume']
    if prev_vol == 0:
        return False
    if (row['volume'] < volume_spike_factor * prev_vol):
        return False
    
    # 3c) MACD turning up (MACD > Signal) or at least hooking upwards
    macd_now = row['MACD']
    sig_now = row['Signal_Line']
    if macd_now < sig_now:
        return False
...
def check_bearish_pullback_advanced(
    df: pd.DataFrame, 
    idx: int,
    pivot_lookback: int = 10,
    min_pullback_pct: float = 0.02,     # 2%
    ...
    """
    Checks if we're in a strong bearish trend that has recently pulled back 
    (a countertrend bounce) and is now resuming downward. Returns True if conditions meet.
    
    Conditions:
      1) Trend Confirmation:
         - Price < 200_SMA
         - 20_SMA < 50_SMA (or 20_EMA < 50_EMA) to confirm a bearish stack
         - ADX above a threshold (e.g., 20) to ensure non-choppy market
      2) Pullback Criteria:
         - Identify pivot low over the last pivot_lookback bars
         - Ensure price is above that pivot by at least min_pullback_pct 
           but no more than max_pullback_pct
         - Volume declines during the bounce (dry-up)
         - RSI goes near or above a rsi_overbought threshold
      3) Reversal Confirmation:
         - A bearish candlestick at idx
         - Volume spike relative to average of last few bars
         - MACD or Stochastic turning back down, etc.
    """
    ...
    # 2) Pullback (Countertrend Bounce) Criteria
    
    # 2a) Identify pivot low over the last pivot_lookback bars
    recent_data = df.iloc[idx - pivot_lookback:idx]
    pivot_low = recent_data['low'].min()
    if pivot_low <= 0:
        return False
    
    # 2b) Calculate bounce percentage
    current_bounce_pct = (row['close'] - pivot_low) / pivot_low
    if not (min_pullback_pct <= current_bounce_pct <= max_pullback_pct):
        return False
    
    # 2c) Volume analysis: ensure volume has dried up during the bounce
    pullback_vol_avg = recent_data['volume'].mean()
    if row['volume'] > volume_dryup_factor * pullback_vol_avg:
        return False
    
    # 2d) RSI check: near overbought or at least above some threshold
    if row['RSI'] < rsi_overbought:
        return False
    
    # 3) Reversal Confirmation
    
    # 3a) Candle is bearish
    if row['close'] >= row['open']:
        return False
    
    # 3b) Volume spike vs. prior bar
    if idx < 1:
        return False
    prev_vol = df.iloc[idx - 1]['volume']
    if prev_vol == 0:
...
def check_bullish_pullback_simple(
    df: pd.DataFrame,
    idx: int,
    require_vwap_above_price: bool = True,
    # Slightly stricter: default a 3% margin above VWAP
    extra_vwap_margin_pct: float = 0.03,
    require_break_prev_high: bool = True,
    # Slightly stricter: must exceed previous high by 0.2%
    break_prev_margin_pct: float = 0.002
) -> bool:
    """
    Slightly Stricter Minimal Bullish Check
    ---------------------------------------
    1) VWAP: 'close' must be >= (VWAP * (1 + extra_vwap_margin_pct)).
    2) Price-action: 'close' must exceed previous high by at least (prev_high * break_prev_margin_pct).

    Returns True if conditions are met; otherwise False.
    """
    # 1) Basic index and data-safety checks
    if idx <= 0 or idx >= len(df):
        return False
    
    required_cols = ["close"]
    if require_break_prev_high:
        required_cols.append("high")
    if require_vwap_above_price:
    ...
    # 2) VWAP Condition
    if require_vwap_above_price and not pd.isna(row.get("VWAP", None)):
        # Price must be at least X% above VWAP
        margin_close = row["VWAP"] * (1.0 + extra_vwap_margin_pct)
        if row["close"] < margin_close:
            return False

    # 3) Price-Action: break previous high by a margin
    if require_break_prev_high:
        strict_break_level = prev_row["high"] * (1.0 + break_prev_margin_pct)
        if row["close"] <= strict_break_level:
            return False

    return True


def check_bearish_pullback_simple(
    df: pd.DataFrame,
    idx: int,
    require_vwap_below_price: bool = True,
    # Slightly stricter: default a 3% margin below VWAP
    extra_vwap_margin_pct: float = 0.03,
    require_break_prev_low: bool = True,
    # Slightly stricter: must drop below previous low by 0.2%
    break_prev_margin_pct: float = 0.002
) -> bool:
    """
    Slightly Stricter Minimal Bearish Check
    ---------------------------------------
    1) VWAP: 'close' must be <= (VWAP * (1 - extra_vwap_margin_pct)).
    2) Price-action: 'close' must be below previous low by at least (prev_low * break_prev_margin_pct).

    Returns True if conditions are met; otherwise False.
    """
    # 1) Basic index/data-safety checks
    if idx <= 0 or idx >= len(df):
        return False
    
    required_cols = ["close"]
    if require_break_prev_low:
        required_cols.append("low")
    if require_vwap_below_price:
        required_cols.append("VWAP")
    ...
    # 2) VWAP Condition
    if require_vwap_below_price and not pd.isna(row.get("VWAP", None)):
        margin_close = row["VWAP"] * (1.0 - extra_vwap_margin_pct)
        if row["close"] > margin_close:
            return False

    # 3) Price-Action: break previous low by a margin
    if require_break_prev_low:
        strict_break_level = prev_row["low"] * (1.0 - break_prev_margin_pct)
        if row["close"] >= strict_break_level:
            return False
...
def detect_signals_in_memory(
    ...
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        if(
            ...
            #and check_bullish_pullback_simple(combined, cidx)
        ):
        ...
        if (
            ...
            #and check_bearish_pullback_simple(combined, cidx)
            ):
...
def run_for_date(date_str):
    ...
    try:
        ...
        papertrade_file = f"papertrade2_{date_label}.csv"
...
if __name__ == "__main__":
    ...
    if not date_args:
        ...
        date_args = [
            ...
            "2025-02-13", "2025-02-17", "2025-02-18", "2025-02-20",
