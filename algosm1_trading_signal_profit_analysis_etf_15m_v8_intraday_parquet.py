...
Trade rules:
- STRICT Stop Loss: 2% above entry (short)
- STRICT Target:    2% below entry (short)
...
def scan_one_day_row_by_row(ticker: str, df_day: pd.DataFrame, day_str: str) -> list[Trade]:
    ...
    while i < len(df_day) - 3:
        ...
        # Preconditions: below AVWAP OR EMA20 < EMA50
        precond_ok = False
        if np.isfinite(avwap1) and close1 < avwap1:
            precond_ok = True
        if np.isfinite(ema20_1) and np.isfinite(ema50_1) and (ema20_1 < ema50_1):
            precond_ok = True
...
def summarize_trades(df: pd.DataFrame) -> dict:
    ...
    - days_target_hit (unique trade_date with >=1 TARGET)
    - trades_target_hit (# trades with outcome TARGET)
    - avg_pnl_pct
    - sum_pnl_pct
    """
    if df.empty:
...
# =============================================================================
def main() -> None:
    print("[RUN] AVWAP SHORT scan — ALL DAYS in data (15m) | strict 2% SL/Target")
