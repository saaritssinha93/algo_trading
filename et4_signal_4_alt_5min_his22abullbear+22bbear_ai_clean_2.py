...
Clean version + ML filter (rewritten & synced):
- Loads latest stamped bundle via manifest (weekly freeze, robust picker).
- Detects intraday signals using CSV-fitted indicator ranges.
- Loads per-ticker ML models and OOF; computes per-ticker thresholds (Youden’s J).
- Applies per-ticker isotonic calibrators (from trainer) before gating.
- Adds a small LIVE_DEFENSIVE_BUMP for live.
- Writes signals and appends first-per-ticker to papertrade CSV.

Key robustness additions:
- Manifest selector accepts bundles up to 7 days ahead and 30 days old.
- Indicators directory resolved to an absolute path (prevents silent empty scans).
- ML shim accepts saved PerTickerModel, raw sklearn pipeline, or constant models.
- If ML scoring fails for a ticker/row, gracefully falls back to CSV-ranges-only.
- Optional relaxed early-session VolRel (min_periods=1) toggle for sanity checks.
- Clear startup diagnostics for paths and file counts; lightweight debug prints.
"""
...
RELAX_EARLY_VOLREL_MIN_PERIODS = True  # set True to use min_periods=1 (early-bars friendly)
VOLREL_GATE_MIN_DEFAULT = 0.90         # relaxed from 1.10 to avoid early-session zeroing
...
FITTED_CSV_MAIN = str((BASE_DIR / "fitted_indicator_ranges_by_ticker.csv").resolve())  # fallback if no manifest
FITTED_CSV_BEAR_ALT = str((BASE_DIR / "fitted_indicator_ranges_by_ticker20_80.csv").resolve())
...
LIVE_DEFENSIVE_BUMP = 0.00      # +2pp safety bump for live
ML_SOFT_MARGIN_BEAR = 0.00      # soft margin for bear admits

# 22b bear union relaxed participation gate
DETECTOR_BEAR_ALT_VOLREL_MIN = 0.85
...
def detect_signals_in_memory(
    ...
    import ast
    from datetime import datetime as _dtloc
    ...
    # --- Optional alt (22b) constraints for bear union ---
    sides_b = {}
    try:
        if os.path.exists(FITTED_CSV_BEAR_ALT):
            ...
            if "Ticker" in cfg_df_b.columns:
                ...
                sides_b = {side: _clean_constraints(rc) for side, rc in raw_b.items()}
    ...
    for _, row_det in df_for_detection.iterrows():
        ...
        if _have_ml_this_row:
            try:
                ...
                if debug:
                    print(f"[{ticker}] {dt} ML={ml_prob:.3f} vs thr={p_thresh_adj + LIVE_DEFENSIVE_BUMP:.3f}")
        ...
        if bull_cfg and allow_bull and _check_side_constraints(r, bull_cfg):
            if volrel_ok_global:
                if _have_ml_this_row:
                    if (ml_prob is not None) and (ml_prob >= (p_thresh_adj + LIVE_DEFENSIVE_BUMP)):
                        ...
                        if sid not in existing_signal_ids:
                            signals.append({
                                ...
                                "Entry Signal": "Yes", "ML_Prob": round(ml_prob, 4), "ML_Thresh": round(p_thresh_adj, 4),
                            })
        ...
        if bear_cfg and allow_bear and _check_side_constraints(r, bear_cfg):
            if volrel_ok_global:
                if _have_ml_this_row:
                    if (ml_prob is not None) and ((ml_prob + ML_SOFT_MARGIN_BEAR) >= (p_thresh_adj + LIVE_DEFENSIVE_BUMP)):
        ...
        # ---- Bearish (22b-style union; relaxed participation) ----
        ...
        if bear_cfg_b and allow_bear:
            if pd.isna(volrel_val) or (volrel_val >= DETECTOR_BEAR_ALT_VOLREL_MIN):
                if _check_side_constraints(r, bear_cfg_b):
                    ok_ml_b = True
                    if _have_ml_this_row:
                        ok_ml_b = (ml_prob is not None) and ((ml_prob + ML_SOFT_MARGIN_BEAR) >= (p_thresh_adj + LIVE_DEFENSIVE_BUMP))
                    if ok_ml_b:
                        ...
                        if sid_b not in existing_signal_ids:
                            signals.append({
                                ...
                                "ML_Thresh": round(p_thresh_adj, 4) if _have_ml_this_row else np.nan,
                            })
...
def find_price_action_entries_for_date(target_date: datetime.date) -> pd.DataFrame:
    ...
    # Path sanity
    if not os.path.exists(FITTED_CSV_MAIN) and os.path.exists(FITTED_CSV_BEAR_ALT):
        print(f"[Detector] Primary ranges CSV missing; falling back to: {FITTED_CSV_BEAR_ALT}")
        # NOTE: we only *read* main path in detect(); keep this print for visibility.
...
if __name__ == "__main__":
    ...
    if not date_args:
        ...
        date_args = [
           "2025-08-25",
           "2025-08-26","2025-08-28","2025-08-29","2025-09-01"
        ]
