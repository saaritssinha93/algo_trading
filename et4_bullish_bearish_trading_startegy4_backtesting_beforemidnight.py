# -*- coding: utf-8 -*-
"""
Created on Mon Dec  2 13:34:51 2024
...
def find_price_action_entry(last_trading_day):
    ...
    for ticker, token in shares_tokens.items():
        try:
            ...
            if data['date'].dt.tz is None:  # If the column is not timezone-aware
            ...
            # Define even stricter screening criteria for Bullish and Bearish trends
            def screen_trend(data, trend_type, change, rsi_threshold, price_condition, volume_multiplier):
                trend_entries = []  # Initialize an empty list for entries
                for i in range(1, len(data)):
                    current_row = data.iloc[i]

                    # Exclude entries after 3 PM
                    if current_row.name.time() >= time(15, 0):
                        continue  # Skip this row

                    # Define Bullish and Bearish entry conditions
                    if trend_type == "Bullish":
                        entry_conditions = {
                            "Pullback": (
                                current_row['low'] >= current_row['VWAP'] * 0.99 and
                                current_row['close'] > current_row['open'] and
                                current_row['MACD'] > current_row['Signal_Line'] and  # MACD Crossover (Bullish)
                                current_row['close'] > data['Lower_Band'].iloc[i] and  # Bollinger Band Reversal (Price above Lower Band)
                                current_row['close'] > current_row['open'] and  # Confirming Bullish reversal from lower band
                                current_row['Stochastic'] > 20 and current_row['Stochastic'] > data['Stochastic'].iloc[i-1]  # Stochastic Bullish
                            ),
                            "Breakout": (
                                current_row['close'] > data['high'].iloc[max(0, i-5):i].max() * 1.03 and
                                current_row['volume'] > volume_multiplier * data['volume'].rolling(window=5).mean().iloc[i] and
                                current_row['MACD'] > current_row['Signal_Line'] and  # MACD Crossover (Bullish)
                                current_row['close'] > data['Lower_Band'].iloc[i] and  # Bollinger Band Reversal (Price above Lower Band)
                                current_row['close'] > current_row['open'] and  # Confirming Bullish reversal from lower band
                                current_row['Stochastic'] > 20 and current_row['Stochastic'] > data['Stochastic'].iloc[i-1]  # Stochastic Bullish
                        ...
                        entry_conditions = {
                            "Pullback": (
                                ...
                                current_row['close'] < current_row['open'] and
                                current_row['MACD'] < current_row['Signal_Line'] and  # Bearish MACD crossover
                                current_row['close'] < data['Upper_Band'].iloc[i] and  # Bollinger Band Reversal (Price below Upper Band)
                                current_row['close'] < current_row['open'] and  # Confirming Bearish reversal from upper band
                                current_row['Stochastic'] < 80 and current_row['Stochastic'] < data['Stochastic'].iloc[i-1]  # Stochastic Bearish
                            ),
                            "Breakdown": (
                                current_row['close'] < data['low'].iloc[max(0, i-5):i].min() * 0.97 and
                                current_row['volume'] > volume_multiplier * data['volume'].rolling(window=5).mean().iloc[i] and
                                current_row['MACD'] < current_row['Signal_Line'] and  # Bearish MACD crossover
                                current_row['close'] < data['Upper_Band'].iloc[i] and  # Bollinger Band Reversal (Price below Upper Band)
                                current_row['close'] < current_row['open'] and  # Confirming Bearish reversal from upper band
            ...
            # Classify based on trend with stricter conditions
            if daily_change > 3.0 and data['RSI'].iloc[-1] > 75 and data['close'].iloc[-1] > data['VWAP'].iloc[-1]:
                # Add the technical conditions for Bullish
                entries.extend(screen_trend(data, "Bullish", daily_change, 75, "> VWAP", 3.0))

            elif daily_change < -2.5 and data['RSI'].iloc[-1] < 25 and data['close'].iloc[-1] < data['VWAP'].iloc[-1]:
                # Add the technical conditions for Bearish
                entries.extend(screen_trend(data, "Bearish", daily_change, 25, "< VWAP", 3.0))
...
def create_papertrade_file(input_file, output_file, last_trading_day):
    ...
    try:
        ...
        threshold_time = pd.Timestamp(
            datetime.combine(last_trading_day, time(9, 20)),
            tz="Asia/Kolkata"
        )
...
def run_for_last_30_trading_days():
    ...
    for day in range(60):  # Iterate over the last 30 trading days
