...
SIGNALS_DB = "generated_signals_historicaln2.json"
...
def check_bullish_breakout(daily_change, adx, price, high_10, vol, macd, roll_vol, macd_above_signal_now, stoch_val, stoch_diff, slope_okay_for_bullish, is_ich_bull, vwap, rsi):
    # Strictly follow user's breakout criteria:
    return (
        (25 <= adx <= 50)
        and (1.0 <= daily_change <= 7.0)
...
def check_bearish_breakdown(daily_change, adx, price, low_10, vol, roll_vol, macd, macd_above_signal_now, stoch_val, stoch_diff, slope_okay_for_bearish, is_ich_bear, vwap, rsi):
    # Strictly follow user's breakdown criteria:
    return (
        (25 <= adx <= 50)
        and (-6.0 <= daily_change <= -1.0)
        and (vol >= 1.5 * roll_vol)
        and (stoch_val < 50)
        and (stoch_diff < 0)
        #and (rsi > 50)
...
def detect_signals_in_memory(
    ...
    """
    Ultimate strictness: Only the strongest, most textbook breakouts/breakdowns.
    """
    import pandas as pd
    ...
    combined['Recent_High'] = combined['high'].rolling(20, min_periods=1).max()
    combined['Recent_Low']  = combined['low'].rolling(20, min_periods=1).min()
    combined['volume_mean_20'] = combined['volume'].rolling(20, min_periods=1).mean()
    ...
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        cidx = match.index[0]
        if cidx == 0:
            continue  # Need previous bar for shift

        row  = combined.iloc[cidx]
        prev = combined.iloc[cidx-1]
        price = row['close']

        # Precompute buffer for EMA_50 breakout
        buffer = 0.001 * row['EMA_50'] if valid(row, 'EMA_50') else 0
        
        # Precompute buffer for EMA_50 breakout
        buffer = 0.001 * row['EMA_50'] if valid(row, 'EMA_50') else 0

        # ── SINGLE NEW INDICATOR: ATR Wide-Range Bar confirmation ──
        WRB_MULT = 1.30   # 1.25–1.50 typical; higher = stricter
        CLOSE_LOC = 0.80  # 0.75–0.85 typical; higher = stricter

        if valid(row, 'high', 'low', 'close'):
            bar_range = max(1e-9, row['high'] - row['low'])
            pos_in_range = (row['close'] - row['low']) / bar_range     # 0..1
            neg_in_range = (row['high'] - row['close']) / bar_range    # 0..1
        else:
            bar_range = 0
            pos_in_range = 0
            neg_in_range = 0

        wrb_bull = valid(row, 'ATR') and (bar_range >= WRB_MULT * row['ATR']) and (pos_in_range >= CLOSE_LOC)
        wrb_bear = valid(row, 'ATR') and (bar_range >= WRB_MULT * row['ATR']) and (neg_in_range >= CLOSE_LOC)
         

        # ========== INSTITUTIONAL-STRICT BULLISH BREAKOUT ========== #
        if (
            valid(row, 'close', 'VWAP', 'ADX', 'Stoch_%K', 'Stoch_%D', 'volume', 'RSI', 'MACD_Hist', 'EMA_50') and
            valid(prev, 'Recent_High', 'volume_mean_20') and
            row['close'] > prev['Recent_High'] and
            row['close'] > row['VWAP'] and
            row['close'] > row['EMA_50'] + buffer and
            row['ADX'] > 30 and
            row['Stoch_%K'] > 60 and
            (row['Stoch_%K'] > row['Stoch_%D'] + 5) and
            row['volume'] > 2 * row['volume_mean_20'] and
            50 < row['RSI'] < 68 and
            row['MACD_Hist'] > 0.25 and
            prev['volume'] <= 2 * prev['volume_mean_20'] and
            wrb_bull                         # ← only new requirement
        ):
            signal_id = f"{ticker}-{dt.isoformat()}-BULL_BREAKOUT"
            if signal_id not in existing_signal_ids:
                signals_detected.append({
                    'Ticker': ticker, 'date': dt, 'Entry Type': 'Bullish Breakout (Ultimate-Strict)',
                    'Trend Type': 'Bullish', 'Price': round(price, 2),
                    'Daily Change %': round(row_detect.get('Daily_Change', 0), 2),
                    'VWAP': round(row['VWAP'], 2),
                    'ADX': round(row['ADX'], 2),
                    'RSI': round(row['RSI'], 2),
                    'Stoch_%K': round(row['Stoch_%K'], 2),
                    'Stoch_%D': round(row['Stoch_%D'], 2),
        ...
        # ========== INSTITUTIONAL-STRICT BEARISH BREAKDOWN ========== #
        if (
            valid(row, 'close', 'VWAP', 'ADX', 'Stoch_%K', 'Stoch_%D', 'volume', 'RSI', 'MACD_Hist', 'EMA_50') and
            valid(prev, 'Recent_Low', 'volume_mean_20') and
            row['close'] < prev['Recent_Low'] and
            row['close'] < row['VWAP'] and
            row['close'] < row['EMA_50'] - buffer and
            row['ADX'] > 30 and
            row['Stoch_%K'] < 40 and
            ...
            row['MACD_Hist'] < -0.25 and
            prev['volume'] <= 2 * prev['volume_mean_20'] and
            wrb_bear                         # ← only new requirement
        ):
            signal_id = f"{ticker}-{dt.isoformat()}-BEAR_BREAKDOWN"
            if signal_id not in existing_signal_ids:
                signals_detected.append({
                    'Ticker': ticker, 'date': dt, 'Entry Type': 'Bearish Breakdown (Ultimate-Strict)',
                    'Trend Type': 'Bearish', 'Price': round(price, 2),
                    'Daily Change %': round(row_detect.get('Daily_Change', 0), 2),
                    'VWAP': round(row['VWAP'], 2),
                    'ADX': round(row['ADX'], 2),
                    'RSI': round(row['RSI'], 2),
