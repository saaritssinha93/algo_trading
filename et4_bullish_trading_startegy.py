...
def get_last_trading_day():
    ...
    """
    today = dt.date.today()
    logging.info(f"Checking for the last trading day. Today is: {today}.")
    print(f"Today: {today}")
    while today.weekday() >= 5 or today in market_holidays:  # 5=Saturday, 6=Sunday
        logging.debug(f"Today ({today}) is a holiday or weekend.")
        today -= timedelta(days=1)
    logging.info(f"Last trading day determined as: {today}")
    print(f"Last Trading Day: {today}")
    return today
...
'''
def fetch_intraday_data(token, last_trading_day, interval="5minute"):
    """
    Fetch intraday data for a given token and last trading day.

    Args:
        token (int): Instrument token of the stock.
        last_trading_day (datetime.date): The last valid trading day.
        interval (str): Timeframe interval for the data (default: "15minute").

    Returns:
        pd.DataFrame: DataFrame containing intraday data with proper timestamps.
    """
    try:
        from_date = last_trading_day - timedelta(days=2)  # Include one day range
        to_date = last_trading_day

        logging.info(f"Fetching data from {from_date} to {to_date} for token: {token}")
        ...
        print(f"Raw data for token {token}: {raw_data}")  # Debugging: Inspect raw response
        data = pd.DataFrame(raw_data)

        # Ensure 'date' column exists in the response
        if 'date' not in data.columns:
            logging.error(f"Timestamp column 'date' missing from Kite API response for token {token}.")
            raise ValueError("Timestamp column 'date' missing from Kite API response.")

        # Convert 'date' to datetime and set as index
        data['date'] = pd.to_datetime(data['date'])  # Convert to datetime
        if data['date'].dt.tz is None:  # If timestamps are naive
        ...
        logging.error(f"ValueError for token {token}: {ve}")
        return pd.DataFrame()  # Return empty DataFrame on error

    except Exception as e:
        logging.error(f"Error fetching intraday data for token {token}: {e}")
        return pd.DataFrame()  # Return empty DataFrame on error
'''
        ...
        try:
            ...
            data = pd.DataFrame(
                kite.historical_data(
                    ...
                    interval="5minute"  # Updated interval
                )
            )
            ...
            if (
                ...
                data['volume'].iloc[-1] > 1.5 * data['volume'].rolling(window=5).mean().iloc[-1] and  # Significant volume spike
                data['ATR'].iloc[-1] > 1.1  # Minimum volatility
            ):
