...
def is_ichimoku_bullish(df: pd.DataFrame, idx: int, tol: float = 0.01) -> bool:
    """
...
def is_ichimoku_bearish(df: pd.DataFrame, idx: int, tol: float = 0.01) -> bool:
    """
...
def detect_signals_in_memory(
    ...
    def valid_adx_for_breakout(adx_val: float) -> bool:
        ...
        return (not pd.isna(adx_val)) and (adx_val >= 25) and (adx_val <= 100)
    ...
    def is_bullish_range_breakout(df: pd.DataFrame, idx: int, range_bars: int = 3) -> bool:
        """
        Checks if the current candle (idx) is a bullish breakout from a 2–3 bar range.
          - The close is above the highest high of the last 'range_bars'.
          - The candle body >= 50% of its own range.
          - The candle's close is in the top 20% of its range.
          - Volume is >= 1.5× or 2× the recent average volume.
        """
        if idx < range_bars:
            return False

        row = df.iloc[idx]
        # Identify the "range" from prior bars
        prior_slice = df.iloc[idx-range_bars:idx]  # last 'range_bars' candles
        highest_high = prior_slice['high'].max()

        # Condition 1: Must close above that highest high
        if row['close'] <= highest_high:
            return False

        # Condition 2: Strong candle body
        candle_range = row['high'] - row['low']
        if candle_range <= 0:
            return False
        body_size = row['close'] - row['open']
        if body_size < 0.5 * candle_range:  # at least 50% of total range
            return False

        # Condition 3: Close is in the top 20% of the candle
        if (row['high'] - row['close']) > 0.2 * candle_range:
            return False

        # Condition 4: Volume spike vs. short-term average (e.g. last 10 bars)
        if 'volume' not in df.columns or idx < 10:
            return False
        avg_volume = df['volume'].iloc[idx-10:idx].mean()
        if avg_volume > 0 and (row['volume'] < 1.5 * avg_volume):  
            # 1.5× or 2× is common; pick what you prefer
            return False

        return True


    def is_bearish_range_breakdown(df: pd.DataFrame, idx: int, range_bars: int = 3) -> bool:
        """
        Checks if the current candle (idx) is a bearish breakdown from a 2–3 bar range.
          - The close is below the lowest low of the last 'range_bars' bars.
          - The candle body >= 50% of its own range.
          - The candle's close is in the bottom 20% of its range.
          - Volume >= 1.5× or 2× the recent average.
        """
        if idx < range_bars:
            return False
        ...
        row = df.iloc[idx]
        # Identify the "range" from prior bars
        prior_slice = df.iloc[idx-range_bars:idx]
        lowest_low = prior_slice['low'].min()

        # Condition 1: Must close below that lowest low
        ...
        # Condition 3: Close in the bottom 20%
        if (row['close'] - row['low']) > 0.2 * candle_range:
            return False

        # Condition 4: Volume spike vs. short-term average
        if 'volume' not in df.columns or idx < 10:
            return False
        avg_volume = df['volume'].iloc[idx-10:idx].mean()
        if avg_volume > 0 and (row['volume'] < 1.5 * avg_volume):
    ...
    VOLUME_MULTIPLIER_BREAKOUT = 3     # higher volume requirement
    ...
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        # Price has pulled back ~3% from the last 5-bar high
        if cidx >= 5:
            recent_high_5 = combined.loc[cidx-5:cidx, 'high'].max()
        else:
            recent_high_5 = combined.loc[:cidx, 'high'].max()
        pullback_pct = (recent_high_5 - combined.loc[cidx, 'close']) / recent_high_5 if recent_high_5 else 0    
        
        
        # Price has bounced ~3% from the last 5-bar low
        if cidx >= 5:
            recent_low_5 = combined.loc[cidx-5:cidx, 'low'].min()
        else:
            recent_low_5 = combined.loc[:cidx, 'low'].min()
        bounce_pct = (combined.loc[cidx, 'close'] - recent_low_5) / recent_low_5 if recent_low_5 else 0
        ...
        if row_trend == "Bullish":
            ...
            bullish_breakout = (
                ...
                #and is_adx_increasing(combined, cidx, bars_ago=3)
                #and is_rsi_increasing(combined, cidx, bars_ago=3)
                # NEW: OBV and ROC check for bullish trend:
                #and is_obv_increasing(combined, cidx, bars_ago=3)
                #and is_roc_positive(combined, cidx)
                #and is_bullish_breakout_price_action(combined, cidx, daily_change)
                and is_ich_bull
                and sq_on
                and is_bullish_range_breakout(combined, cidx, range_bars=3)
            )
            ...
            bullish_pullback = (
                ...
                #and is_obv_increasing(combined, cidx, bars_ago=1)
                #and is_roc_positive(combined, cidx)
                #and is_bullish_pullback_price_action(combined, cidx, daily_change)
                and (pullback_pct >= 0.03)   # at least 3% off recent high
            )
            ...
            bearish_pullback = (
                ...
                and (bounce_pct >= 0.025)  # 3% bounce from recent low
            )
...
def run_for_date(date_str):
    ...
    try:
        ...
        papertrade_file = f"papertrade_n_{date_label}.csv"
