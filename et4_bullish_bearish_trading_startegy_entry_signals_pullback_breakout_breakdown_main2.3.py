...
from et4_filtered_stocks_market_cap import selected_stocks  # Example import
...
file_handler = TimedRotatingFileHandler(
    "trading_script_signal1.log", when="M", interval=30,
...
def find_price_action_entries_for_today(last_trading_day):
    ...
    et = india_tz.localize(datetime.combine(last_trading_day, dt_time(17, 30)))
...
def main():
    ...
    # 1) Detect signals for today (CalcFlag="No" by default)
    ...
    confirmed_signals = []  # Will hold only those signals that become CalcFlag="Yes"

    # 2) For each newly detected signal, try to set CalcFlag="Yes"
    for signal_row in raw_signals_df.to_dict("records"):
        ...
        # A) Mark signals in the main CSV
        mark_signals_in_main_csv(ticker, [signal_row], main_csv_path, india_tz)

        # B) Now re-check the main CSV => see if that row is CalcFlag="Yes"
        sig_id = signal_row["Signal_ID"]
        df_main = load_and_normalize_csv(main_csv_path, tz=india_tz)

        mask = (df_main["Signal_ID"] == sig_id) & (df_main["CalcFlag"] == "Yes")
        if mask.any():
            # The row in the main CSV truly updated => keep the original detection row
            # (This is how we preserve 'Daily Change %' etc. from the detection dictionary)
            confirmed_signals.append(signal_row)

    if not confirmed_signals:
        print("No CalcFlag=Yes signals => nothing goes to papertrade.")
        return

    # 3) Now that we have only CalcFlag=Yes signals, build DataFrame => papertrade
    confirmed_df = pd.DataFrame(confirmed_signals)
    create_papertrade_df(confirmed_df, papertrade_file, last_trading_day)
