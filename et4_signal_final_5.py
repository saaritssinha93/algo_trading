...
- Reads all bars after 9:30 AM from each *_main_indicators_updated.csv
...
def find_price_action_entries_for_today(trading_day, papertrade_file):
    """
    1) For each *_main_indicators_updated.csv:
       - Load entire data.
       - Filter rows from 9:30 to current time (or a default end time) for detection.
    ...
    5) Print line: [ANALYZED] TICKER => used last X of Y rows after 9:30, reason: ...
    """
    ...
    def process_file(file_path):
        ...
        if df_full.empty:
            ...
            print(f"[ANALYZED] {ticker} => used last 0 of 0 rows after 9:30, reason: No data")
            return []

        # All rows after 9:30 (whether or not we go to end_dt)
        df_full_930 = df_full[df_full["date"] >= start_dt]
        count_full_930 = len(df_full_930)

        # For detection, only up to end_dt
        df_today = df_full_930[df_full_930["date"] <= end_dt].copy()
        count_df_today = len(df_today)

        if df_today.empty:
            reason = "No rows after 9:30"
            with lock:
                summary_info[ticker] = reason
            print(f"[ANALYZED] {ticker} => used last {count_df_today} of {count_full_930} rows after 9:30, reason: {reason}")
            return []
