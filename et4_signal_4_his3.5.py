# -*- coding: utf-8 -*-
"""
Created on Thu Feb  6 19:30:30 2025
...
def detect_signals_in_memory(
    ...
    def valid_adx_for_breakout(adx_val: float) -> bool:
        ...
        return (not pd.isna(adx_val)) and (adx_val >= 30) and (adx_val <= 50)

    def valid_adx_for_pullback(adx_val: float) -> bool:
        # Require ADX >= 30 for pullbacks
        return (not pd.isna(adx_val)) and (adx_val >= 25) and (adx_val <= 50)
...
def is_bullish_pullback_price_action(df, idx, daily_change) -> bool:
    """
    Even more relaxed logic for a bullish pullback:
      1. Candle is bullish (close > open).
      2. Lower wick >= 1% of total range (instead of 2–3%).
      3. Upper wick <= 90% of total range (looser than 70–85%).
      4. Low is within 15% of the recent 10-bar low (instead of 8–12%).
      5. Daily change >= -0.3% (allows more negative than -0.2%).
      6. Open is within 8% of the candle's low (instead of 5–6%).
      7. Close >= 95% of its 10-SMA (instead of 97–98%).
      8. Volume >= 25% of the 10-bar average (instead of 30–40%).
      9. Price >= 95% of its 50-SMA for a broader uptrend (instead of 97–98%).
    """
    if idx < 0 or idx >= len(df):
        return False
    ...
    # 2) Lower wick check (>= 1% of total range)
    lower_wick = row['open'] - row['low']
    if lower_wick < 0.01 * range_:
        return False

    # 3) Upper wick check (<= 90% of total range)
    upper_wick = row['high'] - row['close']
    if upper_wick > 0.90 * range_:
        return False

    # 4) Near support: within 15% of recent 10-bar low
    if idx < 9:
        return False
    recent_low = df['low'].rolling(window=10, min_periods=10).min().iloc[idx]
    if recent_low <= 0 or row['low'] > 1.15 * recent_low:
        return False

    # 5) Daily change can be as low as -0.3%
    if daily_change < -0.3:
        return False

    # 6) Open near low (8% gap)
    if row['open'] > 1.08 * row['low']:
        return False
    ...
    # 8) Volume check (>= 25% of avg)
    if 'volume' in df.columns and idx >= 9:
        avg_vol = df['volume'].rolling(window=10, min_periods=10).mean().iloc[idx]
        if avg_vol > 0 and row['volume'] < 0.25 * avg_vol:
            return False

    # 9) Broader uptrend => 50-SMA
    if idx >= 49:
        ma50 = df['close'].rolling(window=50, min_periods=50).mean().iloc[idx]
...
def is_bearish_pullback_price_action(df, idx, daily_change) -> bool:
    """
    Even more relaxed logic for a bearish pullback:
      1. Candle is bearish (close < open).
      2. Upper wick >= 1% of total range (instead of 2–3%).
      3. Lower wick <= 90% of total range (looser than 70–85%).
      4. High is within 15% of the recent 10-bar high (instead of 8–12%).
      5. Daily change <= +0.3% (allows more positive than +0.1–0.2%).
      6. Open is within 8% of the candle's high (instead of 5–6%).
      7. Close <= 105% of its 10-SMA (instead of 102–103%).
      8. Volume >= 25% of the 10-bar average (instead of 30–40%).
      9. Price <= 105% of its 50-SMA for a broader downtrend (instead of 102–103%).
    """
    if idx < 0 or idx >= len(df):
        return False
    ...
    # 2) Upper wick check (>= 1% of range)
    upper_wick = row['high'] - row['open']
    if upper_wick < 0.01 * range_:
        return False

    # 3) Lower wick check (<= 90% of range)
    lower_wick = row['close'] - row['low']
    if lower_wick > 0.90 * range_:
        return False

    # 4) Near resistance: within 15% of recent 10-bar high
    if idx < 9:
        return False
    recent_high = df['high'].rolling(window=10, min_periods=10).max().iloc[idx]
    if recent_high <= 0 or row['high'] < 0.85 * recent_high:
        return False

    # 5) Daily move can be as high as +0.3%
    if daily_change > 0.3:
        return False

    # 6) Open near high (8% gap)
...
if __name__ == "__main__":
    ...
    if not date_args:
        ...
        date_args = ["2025-01-27", "2025-01-28", "2025-01-29", "2025-01-30", "2025-01-31", "2025-02-03", "2025-02-04", "2025-02-05"]
