...
def process_ticker(ticker, from_date_intra, to_date_intra, last_trading_day, india_tz):
    ...
    try:
        ...
        if os.path.exists(main_path):
            ...
            # We do NOT want to re-calc or re-touch older data
            # Just keep it as is. Only normalizing the 'date' column so we can compare times correctly:
        ...
        # Check if 'Signal_ID' is missing
        if 'Signal_ID' not in latest_row.columns:
            logging.error(f"[{ticker}] Latest row missing 'Signal_ID'.")
            return None, []

        # If we have existing data, ensure the new row is truly new
        ...
        # OPTIONAL: compute Entry Signal logic for this single new row
        ...
        # Force 'logtime' just for the new row
        latest_row['logtime'] = datetime.now(india_tz).isoformat()

        # Append to existing
        combined = pd.concat([existing, latest_row], ignore_index=True)

        # Drop duplicates on 'Signal_ID' (keep='first' preserves older rows if conflict)
        if 'Signal_ID' in combined.columns:
            combined.drop_duplicates(subset=['Signal_ID'], keep='first', inplace=True)

        # Sort final by date for cleanliness (though not strictly required)
        combined.sort_values('date', inplace=True)
        ...
        # Return the newly appended row + optional signal
        signals_out = []
...
def find_price_action_entry(last_trading_day):
    ...
    logging.info(f"Hist data: {from_date_hist} to {to_date_hist}")
    logging.info(f"Intraday data: {from_date_intra} to {to_date_intra}")
    ...
    # Combine all signals in a DataFrame
    all_entries = []
    for tkr, ents in entries_dict.items():
        for entry in ents:
            all_entries.append({
    ...
    entries_df = normalize_time(entries_df, timezone='Asia/Kolkata')
    entries_df = entries_df.sort_values('date').reset_index(drop=True)

    # -----------------------------------------------
    # OPTIONAL: Filter out signals that aren’t in the
    # latest 15-minute candle
    # -----------------------------------------------
    latest_candle = get_latest_expected_interval(datetime.now(), tz='Asia/Kolkata')
    entries_df['candle_floor'] = entries_df['date'].dt.floor('15min')
    entries_df = entries_df[entries_df['candle_floor'] == latest_candle].copy()
    entries_df.drop(columns='candle_floor', inplace=True)

    # Ensure standard columns
    expected_cols = ["Ticker","date","Entry Type","Trend Type","Price","Daily Change %","VWAP","ADX","Signal_ID","logtime"]
    for c in expected_cols:
        if c not in entries_df.columns:
            entries_df[c] = ""
    entries_df = entries_df[expected_cols]
...
def create_papertrade_file(input_file, output_file, last_trading_day):
    """
    Reads 'price_action_entries' from CSV, then writes only signals
    belonging to the *latest* 15-minute candle to 'papertrade.csv'.
    """
    ...
    # Convert 'date' column to Asia/Kolkata time
    price_action_entries = normalize_time(price_action_entries, timezone='Asia/Kolkata')
    price_action_entries = price_action_entries.sort_values('date').reset_index(drop=True)

    # -------------------------------------------------------
    # NEW: Filter to the most recent 15-min candle
    # -------------------------------------------------------
    # 1) Figure out the current "expected" candle floor
    latest_candle = get_latest_expected_interval(
        current_time=datetime.now(), 
        tz='Asia/Kolkata'
    )

    # 2) Floor each entry’s time to 15 minutes
    price_action_entries['candle_floor'] = price_action_entries['date'].dt.floor('15min')

    # 3) Keep only rows that match `latest_candle` 
    latest_entries = price_action_entries[
        (price_action_entries['candle_floor'] == latest_candle)
        & (price_action_entries['date'].dt.date == last_trading_day)
    ].copy()

    # If no rows for the latest candle, skip
    if latest_entries.empty:
        print(f"No valid entries found for the latest 15-min candle: {latest_candle}.")
        return

    # Now proceed with the original logic on `latest_entries`
    required_cols = [
    ...
    for col in required_cols:
        if col not in latest_entries.columns:
            latest_entries[col] = ""

    # Sort them by date
    latest_entries = latest_entries.sort_values('date').reset_index(drop=True)

    # ----------------------------------------------------------------
    # The next block is your existing logic for target price, quantity, etc.
    # ----------------------------------------------------------------
    if "Trend Type" in latest_entries.columns:
        bullish = latest_entries[latest_entries["Trend Type"]=="Bullish"].copy()
        bearish = latest_entries[latest_entries["Trend Type"]=="Bearish"].copy()
        ...
        combined_entries = latest_entries.copy()

    # Sort by date, re-normalize time if needed
    ...
    combined_entries = combined_entries.sort_values('date').reset_index(drop=True)

    # If 'logtime' doesn't exist, set it
    if 'logtime' not in combined_entries.columns:
        combined_entries['logtime'] = datetime.now(india_tz).isoformat()

    # Merge into or write a fresh 'papertrade.csv'
    if os.path.exists(output_file):
        ...
        all_papertrade = pd.concat([existing, combined_entries], ignore_index=True)
        all_papertrade = all_papertrade.drop_duplicates(subset=['Ticker','date'], keep='last')
        all_papertrade = all_papertrade.sort_values('date').reset_index(drop=True)
        all_papertrade.to_csv(output_file, index=False)
    else:
        combined_entries.to_csv(output_file, index=False)

    print(f"Filtered latest 15-min candle data saved to '{output_file}'")

    # (Optional) Quick read-back
    try:
        papertrade_data = pd.read_csv(output_file)
        print("\nPapertrade Data for the Latest Candle:")
        print(papertrade_data)
    except Exception as e:
        logging.error(f"Error reading papertrade file {output_file}: {e}")
        print(f"Error reading {output_file}: {e}")

    # You can still do final validations here
    logging.info("Validating the combined CSV files for 'Entry Signal' ...")
    validate_entry_signals(selected_stocks, last_trading_day, india_tz)
    logging.info("Validation completed.")
    print("Validation of the combined CSV files completed.")
...
def main():
    ...
    # After market close, run validation job
    ...
    while True:
        ...
        # Optionally, after each schedule run (or after some condition),
        # call the missing signals function:
...
if __name__=="__main__":
    # Before starting main process, ensure main indicators have 'Entry Signal' column if not present
    validate_and_correct_main_indicators(selected_stocks, india_tz)
    main()
    main1()
