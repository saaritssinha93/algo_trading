...
# Create handlers
file_handler = logging.FileHandler("trading_script_log.log")
...
logging.basicConfig(
    ...
    format=log_format,
    handlers=[
        logging.FileHandler("trading_script_log.log", mode='a'),
...
def normalize_time(df, timezone='Asia/Kolkata'):
    """
    Normalize the 'date' column to ensure it's datetime and then localize/convert timezones.
    """
    ...
    # At this point, ensure 'date' is actually datetime
    if not pd.api.types.is_datetime64_any_dtype(df['date']):
        # If it's still not datetime, raise an error or handle it
        raise TypeError("The 'date' column could not be converted to datetime.")

    # Check if tz-naive and localize if needed
    # Before using .dt, we know 'date' is a datetime column
...
def sort_dataframe_by_date(df):
    """
    Sort the DataFrame by the 'date' column without SettingWithCopyWarning.
    Uses the normalize_time function which returns a copy.
    """
    # normalize_time returns a copy, so df is safe to sort
    df = normalize_time(df)
    # Sort and reset index, no inplace
...
def store_latest_data(ticker, df):
    DATA_CACHE[ticker] = df.copy()  # keep a copy in memory
...
def fetch_intraday_data_with_historical(
    ...
    to_date_intra,
    interval="15minute",
    historical_days=7,         # <-- Pull 7 days of history for better warm-up
    recalc_lookback=500,       # <-- Recalculate indicators on the last 500 rows
    cache_dir=CACHE_DIR,
    freshness_threshold=timedelta(minutes=15),
    bypass_cache=False
):
    """
    Fetches intraday data for the specified interval and merges it with a larger 
    chunk of historical data (by default 7 days). Only new rows are appended to 
    the main indicators CSV. This allows better indicator warm-up by using a 
    bigger 'recalc_lookback'.

    Steps:
    1) Fetch intraday data (cache updated).
    2) Fetch historical data (cache updated).
    3) Combine & drop duplicates.
    4) Recalculate indicators with a larger tail (e.g., 500 rows).
    5) Ensure necessary columns (Signal_ID, Entry Signal, logtime).
    6) Append only missing rows to the main indicators CSV.

    Parameters
    ----------
    ticker : str
        The trading symbol/ticker.
    token : int
        Instrument token for the ticker (from the Kite API).
    from_date_intra : datetime
        Start datetime for intraday data fetch.
    to_date_intra : datetime
        End datetime for intraday data fetch.
    interval : str, optional
        Data interval (default "15minute").
    historical_days : int, optional
        Number of past days to fetch for historical data (default 7).
    recalc_lookback : int, optional
        Number of rows used in recalc_indicators' tail for better warm-up (default 500).
    cache_dir : str, optional
        Directory for cache.
    freshness_threshold : timedelta, optional
        Threshold for cache freshness.
    bypass_cache : bool, optional
        Whether to bypass cache logic and fetch fresh data.

    Returns
    -------
    pd.DataFrame
        The combined DataFrame of historical+intraday data with recalculated indicators.
        May be empty if no data was fetched.
    """
    ...
    def fetch_intraday(ticker):
        try:
            ...
            if new_intraday_df.empty:
                ...
                intraday_data['date'] = (
                    pd.to_datetime(intraday_data['date'], utc=True)
    ...
    def fetch_historical(ticker):
        # Use "historical_days" to get enough data for indicators
        from_date_hist = from_date_intra - timedelta(days=historical_days)
        ...
        try:
            ...
            if historical_df.empty:
                ...
                historical_data['date'] = (
                    pd.to_datetime(historical_data['date'], utc=True)
                      .dt.tz_convert('Asia/Kolkata')
                )
    ...
    # Convert timezones (if not already) & clean
    combined_data = normalize_time(combined_data)

    # -------------------------------
    # Step 4: Recalculate Indicators
    # -------------------------------
    if not combined_data.empty:
        combined_data = sort_dataframe_by_date(combined_data)
        # Pass a bigger "lookback" to recalc_indicators for better warm-up
        combined_data = recalc_indicators(combined_data, lookback=recalc_lookback)
        logging.info(f"Indicators recalculated for {ticker} with lookback={recalc_lookback}.")
    else:
        logging.warning(f"Combined data is empty after merging historical and intraday for {ticker}.")
        return combined_data

    # Ensure the existence of essential columns
    if 'Signal_ID' not in combined_data.columns:
        ...
        # Assign missing Signal_IDs if needed
        combined_data['Signal_ID'] = combined_data.apply(
            lambda row: row['Signal_ID'] if pd.notnull(row['Signal_ID'])
            else f"{ticker}-{row['date'].isoformat()}", axis=1
        )
    ...
    if 'logtime' not in combined_data.columns:
        combined_data['logtime'] = datetime.now(pytz.timezone('Asia/Kolkata')).isoformat()

    # -------------------------------
    # (Optional) Mark 'Entry Signal' = 'Yes' Based on Conditions
    # -------------------------------
    ...
    today_data = combined_data[
        (combined_data['date'] >= today_start_time) &
        (combined_data['date'] <= today_end_time)
    ].copy()
    ...
    if not today_data.empty:
        ...
        if ticker_entries:
            entry_times = [
                pd.to_datetime(entry['date']).astimezone(india_tz)
    ...
    if os.path.exists(main_indicators_path):
        ...
        # Ensure 'logtime' col if missing
        if 'logtime' not in existing_indicators.columns:
            existing_indicators['logtime'] = ""
    else:
        existing_indicators = pd.DataFrame()

    # Identify new rows by 'Signal_ID'
    if not existing_indicators.empty:
        new_rows = combined_data[
            ~combined_data['Signal_ID'].isin(existing_indicators['Signal_ID'])
        ].copy()
    else:
        new_rows = combined_data.copy()

    # If new rows exist, append them
    if not new_rows.empty:
        current_logtime = datetime.now(pytz.timezone('Asia/Kolkata')).isoformat()
        new_rows.loc[:, 'logtime'] = current_logtime
        ...
        logging.info(
            f"Appended {len(new_rows)} new rows of indicator data for {ticker}, "
            f"with logtime={current_logtime}."
        )

    # Finally, return the combined DataFrame
    combined_data = sort_dataframe_by_date(combined_data)
    
    store_latest_data(ticker, combined_data)
...
def recalc_indicators(data: pd.DataFrame, lookback: int = 500) -> pd.DataFrame:
    """
    Recalculate all required indicators using a larger lookback window to improve accuracy.

    We fetch up to 'lookback' rows from the tail, compute indicators, then only 
    update those rows in the final DataFrame.
    """
    import logging
    logging.debug(f"Recalculating indicators with a tail of {lookback} rows.")

    data = data.reset_index(drop=True)
    data = data.copy()

    # Take a bigger slice
    slice_df = data.tail(lookback).copy()

    # Check for NaNs in critical columns
    critical_cols = ['close', 'high', 'low', 'volume']
    if slice_df[critical_cols].isnull().any().any():
        slice_df[critical_cols] = slice_df[critical_cols].fillna(method='ffill')

    # --- RSI Calculation (example) ---
    slice_df['RSI'] = calculate_rsi(slice_df['close'], timeperiod=14)

    # --- ATR ---
    slice_df['ATR'] = calculate_atr(slice_df, timeperiod=14)

    # --- MACD ---
    macd_values = calculate_macd(slice_df, fastperiod=12, slowperiod=26, signalperiod=9)
    slice_df['MACD']        = macd_values['MACD']
    slice_df['Signal_Line'] = macd_values['Signal_Line']
    slice_df['Histogram']   = macd_values['Histogram']

    # --- Bollinger Bands ---
    bb = calculate_bollinger_bands(slice_df, timeperiod=20, nbdevup=2, nbdevdn=2)
    slice_df['Upper_Band'] = bb['Upper_Band']
    slice_df['Lower_Band'] = bb['Lower_Band']

    # --- Stochastic ---
    slice_df['Stochastic'] = calculate_stochastic(slice_df, fastk_period=14, slowk_period=3)

    # --- VWAP ---
    slice_df['VWAP'] = (slice_df['close'] * slice_df['volume']).cumsum() / slice_df['volume'].cumsum()

    # --- 20_SMA ---
    slice_df['20_SMA'] = slice_df['close'].rolling(window=20, min_periods=1).mean()

    # --- Recent High/Low ---
    slice_df['Recent_High'] = slice_df['high'].rolling(window=5, min_periods=1).max()
    slice_df['Recent_Low']  = slice_df['low'].rolling(window=5, min_periods=1).min()

    # --- ADX ---
    slice_df['ADX'] = calculate_adx(slice_df, period=14)

    # Merge back into full data
    data.update(slice_df)  # only updates rows that changed

    # optional forward/backward fill
...
def process_ticker(ticker, from_date_intra, to_date_intra, last_trading_day, india_tz):
    try:
        with api_semaphore:
            ...
            combined_data = normalize_time(combined_data, timezone=india_tz)   
...
#            daily_change = calculate_daily_change(today_data)
            ...
            ticker_entries_df = normalize_time(ticker_entries_df, timezone=india_tz)
            ...
            # After merging, you have a list of dicts:
            ticker_entries_with_id = merged_df.to_dict('records')

            # Convert the list of dicts back to a DataFrame
            entries_df = pd.DataFrame(ticker_entries_with_id)

            # Now normalize the time on the DataFrame
            entries_df = normalize_time(entries_df, timezone=india_tz)

            # Convert back to a list of dicts if needed
            ticker_entries_with_id = entries_df.to_dict('records')
...
def mark_entry_signals(data, entry_times, india_tz):
    ...
    Parameters:
        data (pd.DataFrame): The DataFrame containing stock data.
        entry_times (list of datetime): List of entry times.
        india_tz (pytz.timezone): The target timezone.
    ...
    try:
        if entry_times:
            # Convert entry_times to tz-aware
            entry_times = [et if et.tzinfo else india_tz.localize(et) for et in entry_times]
            ...
            # Mark entry signals
            data['Entry Signal'] = 'No'  # Default value
...
def apply_trend_conditions(ticker: str, last_trading_day: pd.Timestamp, india_tz) -> list:
    """
    Detects Bullish or Bearish trend entries based on data in the *_main_indicators.csv file for a given ticker.

    Parameters:
        ticker (str): The stock ticker symbol.
        last_trading_day (pd.Timestamp): The last trading day.
        india_tz (pytz.timezone): Timezone object for 'Asia/Kolkata'.
    ...
    # Normalize 'date' column - this ensures tz-awareness
    df = normalize_time(df, timezone=india_tz)

    # No need to tz_localize again since normalize_time already handled that
    ...
    # Proceed with trend conditions (unchanged logic)
    try:
...
def combine_and_validate_data(intraday_data, ticker, india_tz):
    ...
    # Normalize and sort the intraday data
    intraday_data = normalize_time(intraday_data, timezone=india_tz)
    intraday_data = sort_dataframe_by_date(intraday_data)
    ...
    try:
        ...
        if 'Signal_ID' not in df.columns:
            df['Signal_ID'] = df.apply(
                lambda row: f"{row['Ticker']}-{row['date'].isoformat()}" if 'Ticker' in df.columns else f"UNKNOWN-{row['date'].isoformat()}",
                axis=1
            )
        else:
            # For rows with missing Signal_ID, assign one
            missing_signal_ids = df['Signal_ID'].isna() | (df['Signal_ID'] == "")
            if missing_signal_ids.any():
                df.loc[missing_signal_ids, 'Signal_ID'] = df[missing_signal_ids].apply(
                    lambda row: f"{row['Ticker']}-{row['date'].isoformat()}" if 'Ticker' in df.columns else f"UNKNOWN-{row['date'].isoformat()}",
                    axis=1
                )
...
def create_papertrade_file(input_file, output_file, last_trading_day):
    ...
    try:
        ...
        if "Trend Type" in earliest_entries.columns:
            ...
            current_logtime = datetime.now(india_tz).isoformat()
            # Convert 'logtime' to string type
            combined_entries['logtime'] = combined_entries['logtime'].astype(str)

            # Now assign the string timestamp
            combined_entries.loc[:, 'logtime'] = current_logtime
...
def prepare_time_ranges(last_trading_day, india_tz):
    ...
    past_n_days = last_trading_day - timedelta(days=10)
...
def setup_logging(log_filename="trading_script.log", rotation_interval_minutes=30):
    """
    Sets up logging so that logs are rotated every 'rotation_interval_minutes'.
    If you prefer hourly, set `when="H", interval=1`.
    """
    logger = logging.getLogger()   # or getLogger(__name__) if you prefer
    ...
    handler = TimedRotatingFileHandler(
        ...
        when="M",                  # 'M' for minutes; for hourly, use 'H'
        interval=rotation_interval_minutes,
        backupCount=0             # 0 means no old logs are kept
    )
    formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
...
if __name__ == "__main__":    
    ...
    setup_logging("trading_script_log.log", 60)  # Rotate every 60 minutes
