...
SIGNALS_DB     = "generated_signals_historical5minv2dd.json"
ENTRIES_DIR    = "main_indicators_history_entries_5minv2dd"  # where day's detected signals are saved
...
file_handler = TimedRotatingFileHandler(
    "logs\\signalnewn5minv2dd.log", when="M", interval=30, backupCount=5, delay=True
)
...
def detect_signals_in_memory(
    ...
    debug: bool = False,
    # --- NEW global gates / knobs ---
    volrel_gate_min: float = 1.10,    # raise to 1.22 to cut more signals
    use_vwap_side_gate: bool = False, # set True to enable side-aware VWAP gating
    vwap_long_min: float = 0.001,     # +0.10% above VWAP required for Bullish (when side gate on)
    vwap_short_max: float = -0.001,   # -0.10% below VWAP required for Bearish (when side gate on)
) -> list:
    """
    ...
    Additions:
      - Global participation brake via VolRel >= volrel_gate_min (side-agnostic).
      - Optional side-aware VWAP gate (Bullish must be above, Bearish below).
    """
    ...
    for _, row_det in df_for_detection.iterrows():
        ...
        # --- NEW: Global participation brake (VolRel = volume / rolling_vol_10) ---
        volrel_val = r.get("VolRel", np.nan)
        if pd.isna(volrel_val) or volrel_val < volrel_gate_min:
            if debug:
                # Count as both sides failing this bar due to participation
                fail_counts["Bullish"] += 1
                fail_counts["Bearish"] += 1
            continue

        # --- NEW (optional): side-aware VWAP gate ---
        # When enabled, skip evaluating a side if VWAP distance disagrees.
        allow_bull = True
        allow_bear = True
        if use_vwap_side_gate:
            vdist = r.get("VWAP_Dist", np.nan)
            if pd.isna(vdist):
                # If we can't compute VWAP_Dist, skip both sides
                if debug:
                    fail_counts["Bullish"] += 1
                    fail_counts["Bearish"] += 1
                continue
            # Bullish: require price sufficiently above VWAP
            if vdist < vwap_long_min:
                allow_bull = False
            # Bearish: require price sufficiently below VWAP
            if vdist > vwap_short_max:
                allow_bear = False

        bull_cfg = sides.get("Bullish")
        if bull_cfg and allow_bull:
...
def find_price_action_entries_for_date(target_date: datetime.date) -> pd.DataFrame:
    ...
    def process_file(file_path: str) -> list:
        ...
        new_signals = detect_signals_in_memory(
            ...
            fitted_csv_path="fitted_indicator_ranges_by_ticker25_75.csv",
...
def run_for_date(date_str: str) -> None:
    ...
    try:
        ...
        papertrade_file = f"papertrade_5min_v2d_{date_label}.csv"
