# -*- coding: utf-8 -*-
"""
Created on Sun Aug 24 13:53:00 2025
...
2) Run batched backtests (v3a) with SL/TP logic and skip-if-results-exist.
...
PAPERTRADE_PREFIX     = "papertrade_5min_v3_"
RESULT_PREFIX         = "result_papertradesl_v3a_"      # per-trade rows
RESULT_FINAL_PREFIX   = "result_finalsl_5min_v3a_"      # per-day net % summary
...
# Trade params
BULL_TARGET_PCT = 0.02
BEAR_TARGET_PCT = 0.02
STOP_LOSS_PCT   = 0.02
...
SLEEP_SECONDS   = 120 * 60   # 5 minutes

# Logging
logging.basicConfig(
    filename="trading_log_batched_v3a.log",
    level=logging.INFO,
...
def simulate_long(
    ...
    stop_px   = buy_price * (1 - STOP_LOSS_PCT)
...
def simulate_short(
    ...
    stop_px   = sell_price * (1 + STOP_LOSS_PCT)
...
def process_multiple_dates(
    ...
    output_csv_path: str = "summary_5min_resultsv3a.csv",
    pretty_csv_path: Optional[str] = "summary_5min_results_prettyv3a.csv",
    ...
    for date in dates:
        ...
        total_net_percentage = pd.to_numeric(merged.loc[~mask_overall, 'Net Percentage'], errors='coerce').fillna(0).sum()
    ...
    _f2 = lambda x: f"{float(x):,.2f}" if pd.notna(x) and isinstance(x,(int,float,np.floating)) else x
    _fi = lambda x: f"{int(x):,d}" if pd.notna(x) and float(x).is_integer() else x

    for c in ["Total Value","Total Net %","Avg %","Profit/Loss","Profit/Loss (Margins)",
              "Profit/Loss (Bull)","Profit/Loss (Margins) (Bull)",
              "Profit/Loss (Bear)","Profit/Loss (Margins) (Bear)"]:
        df_pretty[c] = df_pretty[c].apply(_f2)
    df_pretty["Ticker Count"] = df_pretty["Ticker Count"].apply(_fi)
...
def run_cycle():
    ...
    # Backtest
    logging.info("Starting batched backtest (v3a)…")
    ...
    process_multiple_dates(
        ...
        output_csv_path="summary_5min_resultsv3a.csv",
        pretty_csv_path="summary_5min_results_prettyv3a.csv",
        print_console=True
    )
