# -*- coding: utf-8 -*-
"""
Created on Fri Feb 21 11:03:47 2025
...
def recalc_indicators_for_latest_row(data: pd.DataFrame) -> pd.DataFrame:
    """
    Recalculate and append indicators for the latest rows.
    We increased the lookback to 250 rows so we can compute up to a 200-SMA.
    """
    if data.empty:
        return pd.DataFrame()

    df = data.copy().reset_index(drop=True)
    # Extended lookback window to cover up to 200-SMA
    lookback_window = 250
    last_portion = df.tail(lookback_window).copy()

    try:
        df.loc[last_portion.index, 'RSI'] = calculate_rsi(last_portion['close'])
        df.loc[last_portion.index, 'ATR'] = calculate_atr(last_portion)

        macd_df = calculate_macd(last_portion)
        df.loc[last_portion.index, ['MACD','Signal_Line','Histogram']] = macd_df

        boll_df = calculate_bollinger_bands(last_portion)
        df.loc[last_portion.index, ['Upper_Band','Lower_Band']] = boll_df

        stoch = calculate_stochastic(last_portion)
        df.loc[last_portion.index, 'Stochastic'] = stoch

        vwap_series = (last_portion['close'] * last_portion['volume']).cumsum() / last_portion['volume'].cumsum()
        df.loc[last_portion.index, 'VWAP'] = vwap_series

        # Existing 20_SMA plus new 50_SMA & 200_SMA
        df.loc[last_portion.index, '20_SMA'] = last_portion['close'].rolling(window=20, min_periods=1).mean()
        df.loc[last_portion.index, '50_SMA'] = last_portion['close'].rolling(window=50, min_periods=1).mean()
        df.loc[last_portion.index, '200_SMA'] = last_portion['close'].rolling(window=200, min_periods=1).mean()

        df.loc[last_portion.index, 'Recent_High'] = last_portion['high'].rolling(window=5, min_periods=1).max()
        df.loc[last_portion.index, 'Recent_Low']  = last_portion['low'].rolling(window=5, min_periods=1).min()

        adx_series = calculate_adx(last_portion, period=14)
        df.loc[last_portion.index, 'ADX'] = adx_series

    except Exception as e:
        logging.error(f"Indicator calculation error in recalc_indicators_for_latest_row: {e}")
        raise

    # Forward/backward fill any missing values, then return the latest row
    df = df.ffill().bfill()
