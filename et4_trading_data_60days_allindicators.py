...
Single-run script to replicate the live 15-min scenario for the past 2 months:
  ...
  2) For each interval (call it t_candle), we fetch historical data from
     (t_candle - 10 days) to t_candle in 15-min interval, i.e. the continuity window.
...
INDICATORS_DIR = "main_indicators_new"
...
def setup_kite_session() -> KiteConnect:
    ...
    try:
        ...
        kite = KiteConnect(api_key=key_data[0])
...
###############################################################################
# 5) Time Utility: Generate 15-min intervals for the past 2 months
###############################################################################
def generate_15min_time_slots(days_back=60, interval_minutes=15):
    ...
    This version generates times labeled from 9:30 up to 15:15, matching
    how many brokers label the candle by its close time (first bar 9:15-9:30 => 9:30).
    """
...
def fetch_single_candle_data(ticker: str, token: int, candle_end_dt: datetime) -> pd.DataFrame:
    ...
    We fetch from (candle_end_dt - CONTINUITY_DAYS) up to `candle_end_dt`
    with 15-min intervals, then re-calc the indicators and extract the row
    that matches `candle_end_dt`.
    """
    # Convert to strings
    ...
    # Recalc indicators on entire data range
    df = recalc_indicators_for_entire_df(df)

    # Then find the final row that matches the candle_end_dt's minute block
    target_minute_block = candle_end_dt.replace(second=0, microsecond=0)
    df_candidate = df[df['date'] == target_minute_block]
    if df_candidate.empty:
        # If exact match not found, pick the last row with date <= target_minute_block
        df_candidate = df[df['date'] <= target_minute_block]
        if df_candidate.empty:
            return pd.DataFrame()
...
def recalc_indicators_for_entire_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Also sets 'Daily_Change' by comparing each day's open with previous day's close.
    """
    if df.empty:
        return df
...
def simulate_2month_live_for_ticker(ticker: str):
    ...
    For the given ticker:
      ...
      - For each time slot, fetch data from (slot - 10 days) to slot
...
def main_simulation_run():
    ...
    WARNING: This will produce a large number of API calls.
    """
