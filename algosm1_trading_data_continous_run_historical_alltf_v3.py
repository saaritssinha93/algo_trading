...
- Fetch & print ONLY missing files and missing evaluation rows.
  * Missing file  => output CSV does not exist (per mode).
  * Missing rows  => output exists but is not "fresh" vs expected_last_stamp().

IMPORTANT:
- Data fetching logic remains the same.
- Indicator calculations remain the same.
- Still uses per-ticker freshness skip (FASTSKIP), but now:
  * It prints only the missing items (not the "fresh skip" noise)
  * It writes optional reports for missing files and newly appended rows.
"""
...
def missing_spec(mode: str, out_path: str, now_ist: datetime, context: str,
    ...
    """
    Returns a dict explaining what is missing for this ticker/mode.
    - If file missing: kind='file_missing'
    - If file exists and fresh: kind='fresh'
    - If file exists but stale: kind='rows_missing', includes last_ts and expected
    """
    if not os.path.exists(out_path):
        spec = expected_last_stamp(mode, now_ist, context, holidays, force_today_daily, intraday_ts)
...
def process_ticker(mode: str, ticker: str, token: int, kite: KiteConnect,
    ...
    last_before_ts = _read_last_ts_from_csv(out_path) if existed_before else None
    ...
    # Safety: if fresh, do nothing (but do NOT spam logs)
    ...
    try:
        ...
        # ---- Compute "missing evaluation rows" = newly appended rows (after last_before_ts) ----
        ...
        new_last = None
        if new_rows_count > 0:
            new_first = pd.to_datetime(new_rows["date"], errors="coerce").dropna().min()
            new_last = pd.to_datetime(new_rows["date"], errors="coerce").dropna().max()
            new_first = new_first.strftime("%Y-%m-%d %H:%M:%S") if pd.notna(new_first) else None
            new_last = new_last.strftime("%Y-%m-%d %H:%M:%S") if pd.notna(new_last) else None

        # Save newly appended rows report (optional but useful)
        new_rows_path = None
        if new_rows_count > 0:
            rep_dir = os.path.join(report_dir, "missing_rows", mode)
            _safe_mkdir(rep_dir)
            new_rows_path = os.path.join(rep_dir, f"{ticker}_missing_rows_{mode}.csv")
            new_rows.to_csv(new_rows_path, index=False)

            # Optional small preview print (capped)
...
def run_mode(mode: str, context: str, max_workers: int, force_today_daily: bool,
    ...
    # ---- Pre-scan: decide missing files + missing rows (no Kite calls) ----
    missing_files: list[str] = []
    missing_rows: list[str] = []   # tickers with stale rows
    fresh: list[str] = []

    if skip_if_fresh:
        for t in tickers:
            t = t.upper()
            out_path = os.path.join(DIRS[mode]["out"], f"{t}_main_indicators_{mode}.csv")
            ...
            if ms["kind"] == "fresh":
                ...
                missing_rows.append(t)  # also needs work
            else:
                missing_rows.append(t)
    else:
        # If skip disabled, treat everything as "missing rows" (work required)
        missing_rows = [t.upper() for t in tickers]

    # ---- Print ONLY missing items (as requested) ----
    if skip_if_fresh:
        ...
        logger.info("[%s] Missing evaluation rows (stale tickers): %d", mode.upper(), len(missing_rows))
    else:
        logger.info("[%s] no-skip enabled => processing all tickers: %d", mode.upper(), len(missing_rows))

    if not missing_rows:
        logger.info("[%s] Nothing missing — all tickers fresh.", mode.upper())
        return

    # ---- Start Kite ONLY if there is missing work ----
    ...
    logger.info("[%s] Processing ONLY missing tickers=%d with max_workers=%d ...", mode.upper(), len(work_items), max_workers)
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                if rep.status == "failed":
                    failed += 1
                # Keep only meaningful reports for printing (created/updated)
    ...
    # ---- Final print: ONLY what changed (missing files / missing rows) ----
    if updated_reports:
        logger.info("[%s] Updated tickers: %d", mode.upper(), len(updated_reports))
        for r in sorted(updated_reports, key=lambda x: x.ticker):
            logger.info(
                "[%s] %s %s | last_before=%s | expected=%s | new_rows=%d | new_range=%s → %s | new_rows_csv=%s",
                mode.upper(),
                r.ticker,
                r.status,
                r.last_before,
    ...
    if failed:
        logger.warning("[%s] Failed tickers: %d (see fetcher_run.log)", mode.upper(), failed)
...
def parse_args():
    ...
    # NEW: reporting / printing controls
    p.add_argument("--report-dir", default="missing_reports",
                   help="Directory to write missing-files and missing-rows reports")
    p.add_argument("--print-missing-rows", action="store_true",
                   help="Print a small preview of newly appended rows per ticker")
    p.add_argument("--print-missing-rows-max", type=int, default=5,
                   help="Max rows to print per ticker when --print-missing-rows is enabled")
