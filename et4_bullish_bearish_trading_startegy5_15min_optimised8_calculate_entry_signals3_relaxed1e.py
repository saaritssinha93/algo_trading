...
def detect_signals_in_memory(ticker: str, df_today: pd.DataFrame) -> list:
    """
    Signal detection with moderately relaxed thresholds focusing on momentum stocks.
    ...
    # -- 2) Check if ADX indicates momentum (e.g., > 28 for moderately relaxed focus)
    latest_row = df_today.iloc[-1]
    adx_val = latest_row.get("ADX", 0)
    if adx_val <= 28:  # Relaxed ADX threshold
        return signals_detected

    # Decide if it's Bullish or Bearish scenario:
    if (
        daily_change > 1.0  # Relaxed for bullish
        and latest_row.get("RSI", 0) > 30  # Moderate RSI threshold
        and latest_row.get("close", 0) > latest_row.get("VWAP", 0) * 1.01  # Relaxed VWAP multiplier
    ):
        trend_type = "Bullish"
    elif (
        daily_change < -1.0  # Relaxed for bearish
        and latest_row.get("RSI", 0) < 70  # Relaxed RSI for bearish
    ...
    volume_multiplier = 1.2  # Relaxed volume multiplier for momentum
    rolling_window = 8  # Moderate rolling window for momentum detection
    MACD_BULLISH_OFFSET = -0.1  # Relaxed MACD offset for bullish
    MACD_BEARISH_OFFSET = 0.1  # Relaxed MACD offset for bearish

    if trend_type == "Bullish":
        conditions = {
            "Pullback": (
                (df_today["low"] >= df_today["VWAP"] * 0.975)  # Relaxed VWAP condition
                ...
                & (df_today["Stochastic"] < 50)  # Relaxed stochastic threshold
