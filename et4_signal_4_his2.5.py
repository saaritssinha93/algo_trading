# -*- coding: utf-8 -*-
"""
Created on Tue Feb  4 17:42:24 2025
...
def detect_signals_in_memory(
    ...
    def slope_okay_for_bullish(row) -> bool:
        ...
        if pd.isna(slope_20) or pd.isna(sma_20) or sma_20 == 0:
            return False
        return (slope_20 > 0) and ((slope_20 / sma_20) >= 0.002)
    ...
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        if row_trend == "Bullish":
            ...
            bullish_breakout = (
                (2.0 <= daily_change)
                and (price > 1.04 * vwap)  # equivalent to: VWAP < 0.9434 * price
            ...
            bearish_breakdown = (
                (daily_change <= -2.0)
                and (price < 0.96 * vwap)  # equivalent to: VWAP > 1.0638 * price
                and valid_adx_for_breakout(adx_val)
...
def is_bullish_breakout_price_action(df, idx, daily_change) -> bool:
    """
    Stricter Price Action logic for a bullish breakout:
      - The current candle is bullish (close > open).
      - The candle's body is large (at least 60% of the candle's range).
      - The candle's close is very near the highest high of the last 10 bars (within 0.5%).
      - The move is strong: daily_change is at least +3%.
    """
    ...
    if range_ <= 0 or body < 0.6 * range_:
        return False
    ...
    # Current close must be within 0.5% of the recent high.
    if row['close'] < 0.995 * recent_high:
        return False

    # Require a strong move.
    if daily_change < 3.0:
        return False
...
def is_bearish_breakdown_price_action(df, idx, daily_change) -> bool:
    """
    Stricter Price Action logic for a bearish breakdown:
      - The current candle is bearish (close < open).
      - The candle's body is large (at least 60% of the candle's range).
      - The candle's close is very near the lowest low of the last 10 bars (within 0.5%).
      - The move is strong: daily_change is at most -3%.
    """
    if idx < 0 or idx >= len(df):
        return False
    ...
    # Current close must be within 0.5% above the recent low.
    if row['close'] > 1.005 * recent_low:
        return False
...
def is_bullish_pullback_price_action(df, idx, daily_change) -> bool:
    """
    Lenient Price Action logic for a bullish pullback:
      - The current candle is bullish (close > open).
      - The candle shows evidence of a pullback by having a long lower wick 
        (at least 30% of the candle's range).
...
def is_bearish_pullback_price_action(df, idx, daily_change) -> bool:
    """
    Lenient Price Action logic for a bearish pullback:
      - The current candle is bearish (close < open).
      - The candle shows evidence of a pullback by having a long upper wick 
        (at least 30% of the candle's range).
...
if __name__ == "__main__":
    ...
    if not date_args:
        ...
        date_args = ["2025-01-27", "2025-01-28", "2025-01-29", "2025-01-30", "2025-01-31", "2025-02-03", "2025-02-04" ]
