# -*- coding: utf-8 -*-
"""
Example script to read only the last row of each *_main_indicators.csv,
detect signals, mark CalcFlag, and update papertrade file.
Now runs continuously every 20 seconds.
"""
...
file_handler = TimedRotatingFileHandler(
    "logs\\signal1.log", when="M", interval=30, backupCount=5, delay=True
)
...
#    (Reading only the last row from each CSV)
###########################################################
...
'''
def detect_signals_in_memory(ticker, df_today, existing_signal_ids, prev_day_close=None):
    """Detect signals for the last (single) row in df_today."""
    signals_detected = []
    ...
    # Example ADX threshold
    if adx_val <= 25:
        return signals_detected

    # Decide if bullish vs. bearish
    if (
        daily_change > 1.0
        and latest_row.get("RSI", 0) > 20
        and latest_row.get("close", 0) > latest_row.get("VWAP", 0) * 1.01
...
###########################################################
# 7) find_price_action_entries_for_today (Last Row Only)
###########################################################
def find_price_action_entries_for_today(trading_day):
    ...
    CHANGED:
      Instead of reading only the last row from each CSV file,
      we now read the last TWO rows (df_recent = df_full.iloc[-2:]).
      This can help catch signals triggered by small changes over
      the most recent few rows.
    """
    # Example: If you want only rows from 9:25 to 14:05, define st & et:
    # (You can remove this time-based filtering if you want *all* rows)
    st = india_tz.localize(datetime.combine(trading_day, datetime.min.time()))
    et = india_tz.localize(datetime.combine(trading_day, datetime.max.time()))
    ...
    def find_prev_day_close_for_ticker(df_full, date_of_interest):
        """
        Find the last close from the day BEFORE `date_of_interest`.
        Return None if not found.
        """
        prev_day = date_of_interest - timedelta(days=1)
        df_before = df_full[df_full["date"].dt.date < date_of_interest]
        if df_before.empty:
            return None
        # We'll take the last row from that previous day
    ...
    def process_file(file_path):
        ...
        # Filter by today's range (optional; remove if you want last 2 rows from the entire CSV)
        ...
        # Instead of picking just 1 row, we pick the last 2 rows
        # If df_today has fewer than 2, it just returns however many it has.
        df_recent = df_today.iloc[-2:]

        # For daily_change reference: we find the previous day's close
        prev_day_close = find_prev_day_close_for_ticker(df_full, trading_day)

        # Pass these 2 rows to detect_signals_in_memory
    ...
    if not all_signals:
        logging.info("No new signals detected (last 2-row approach).")
        return pd.DataFrame()
...
def create_papertrade_df(df_entries_yes, output_file, trading_day):
    ...
    # If you want a daily intraday window, e.g., entire day
...
def main():
    ...
    try:
        ...
        if raw_signals_df.empty:
            logging.info("No signals found this cycle (last-row approach).")
            return
