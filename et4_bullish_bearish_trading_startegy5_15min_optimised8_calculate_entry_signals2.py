...
def detect_signals_in_memory(ticker: str, df_today: pd.DataFrame) -> list:
    """
    A slightly relaxed version of signal detection. It identifies bullish/bearish
    pullback and breakout/breakdown signals while reducing strict thresholds.

    Steps:
      1) Compute daily_change from the day’s first open & last close
      2) Check ADX > 30 for sufficient momentum
    ...
    # -- 2) Check if ADX is "strong enough" (e.g., > 30 for relaxed conditions)
    latest_row = df_today.iloc[-1]
    adx_val    = latest_row.get("ADX", 0)
    if adx_val <= 30:  # Lower threshold for ADX
        return signals_detected

    # Decide if it's Bullish or Bearish scenario:
    if (
        daily_change > 1.0  # Lower daily change threshold
        and latest_row.get("RSI", 0) > 40  # Slightly lower RSI threshold
        and latest_row.get("close", 0) > latest_row.get("VWAP", 0) * 1.01  # Relaxed VWAP condition
    ):
        trend_type = "Bullish"
    elif (
        daily_change < -1.5  # Slightly relaxed for bearish
        and latest_row.get("RSI", 0) < 65  # Higher RSI for bearish
        and latest_row.get("close", 0) < latest_row.get("VWAP", 0) * 0.99  # Relaxed VWAP condition
    ):
        trend_type = "Bearish"
    else:
    ...
    volume_multiplier   = 1.5  # Slightly reduced
    rolling_window      = 8    # Shorter rolling window
    MACD_BULLISH_OFFSET = -0.2  # Relaxed MACD offset
    MACD_BEARISH_OFFSET = 0.2   # Relaxed MACD offset

    if trend_type == "Bullish":
        conditions = {
            "Pullback": (
                (df_today["low"] >= df_today["VWAP"] * 0.99)  # Slightly less strict
                ...
                & (df_today["Stochastic"] < 50)  # Higher threshold for stochastic
                & (df_today["Stochastic"].diff() > 0)
                & (df_today["ADX"] > 30)
            ),
            "Breakout": (
                (df_today["close"] > df_today["high"].rolling(window=rolling_window).max() * 1.005)
                & (df_today["volume"] > volume_multiplier * df_today["volume"].rolling(window=rolling_window).mean())
                & (df_today["MACD"] > df_today["Signal_Line"])
                & (df_today["MACD"] > MACD_BULLISH_OFFSET)
                & (df_today["close"] > df_today["20_SMA"])
                & (df_today["Stochastic"] > 55)
                & (df_today["Stochastic"].diff() > 0)
                & (df_today["ADX"] > 30)
            ),
        }
        ...
        conditions = {
            ...
            "Breakdown": (
                (df_today["close"] < df_today["low"].rolling(window=rolling_window).min() * 0.995)
                & (df_today["volume"] > volume_multiplier * df_today["volume"].rolling(window=rolling_window).mean())
                & (df_today["MACD"] < df_today["Signal_Line"])
                & (df_today["MACD"] < MACD_BEARISH_OFFSET)
                & (df_today["close"] < df_today["20_SMA"])
