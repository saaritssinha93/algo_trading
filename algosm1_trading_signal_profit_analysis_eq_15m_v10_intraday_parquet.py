...
UPDATED: Option A + Option E + Option B
------------------------------------------------
...
4) Also require close < AVWAP (strict)

OPTION E (Time-of-day filter):
- Only allow signals/entries in selected windows:
    09:15–11:30 and 13:00–14:30

OPTION B (AVWAP “rejection” rule + distance from AVWAP):
- Don’t short just because price is under AVWAP; require evidence of rejection:
   Either:
   (1) Touch/approach AVWAP but fail: high >= AVWAP and close < AVWAP (any bounce candle)
   OR
   (2) Close stays below AVWAP for N consecutive candles (default N=2) between impulse and entry
- Require distance from AVWAP at entry:
      (AVWAP - close_at_entry_candle) >= AVWAP_DIST_ATR_MULT * ATR15_at_entry
  default AVWAP_DIST_ATR_MULT = 0.25
...
CORRECTION (kept):
- ADX twice INCREASING at impulse: ADX[i] > ADX[i-1] > ADX[i-2]
- RSI twice REDUCING:             RSI[i] < RSI[i-1] < RSI[i-2]
- Stochastic bearish + K twice reducing:
...
AVWAP_REJECT_MIN_CONSEC_CLOSES = 2  # set 0 to disable this branch

# Distance from AVWAP at entry (avoid mid-zone chop)
AVWAP_DIST_ATR_MULT = 0.25  # require (AVWAP - close_entry_candle) >= 0.25 * ATR15
...
def _max_consecutive_true(flags: np.ndarray) -> int:
    """Return max length of consecutive True in a boolean array."""
    best = 0
    run = 0
    for x in flags:
        if bool(x):
...
def avwap_rejection_ok(df_day: pd.DataFrame, impulse_idx: int, entry_idx: int) -> bool:
    """
    OPTION B: Require AVWAP rejection evidence between impulse and entry.
    We consider candles from (impulse_idx+1 .. entry_idx) as "bounce/transition" candles.
    If entry_idx == impulse_idx+1, this window is just the entry candle.
    Criteria (either passes):
      - TOUCH rule: any candle has high >= AVWAP and close < AVWAP
      - CONSEC rule: at least N consecutive closes below AVWAP (can include impulse candle too)
    """
    ...
    # Window A: bounce/transition candles (impulse_idx+1 .. entry_idx)
    win_a = df_day.iloc[i0 + 1 : e0 + 1]
    if win_a.empty:
        return False
    ...
    # CONSEC closes rule (use a slightly wider window including the impulse candle)
    consec_ok = False
    n = int(AVWAP_REJECT_MIN_CONSEC_CLOSES)
    if n > 0:
        win_b = df_day.iloc[i0 : e0 + 1]  # include impulse candle for "stays below" logic
...
def avwap_distance_ok(df_day: pd.DataFrame, idx: int) -> bool:
    """
    OPTION B: Require distance from AVWAP at entry candle:
      (AVWAP - close) >= AVWAP_DIST_ATR_MULT * ATR15
    Uses the entry candle close and ATR15.
    """
    if AVWAP_DIST_ATR_MULT is None:
        return True
    mult = float(AVWAP_DIST_ATR_MULT)
    if mult <= 0:
        return True
    ...
    while i < len(df_day) - 3:
        ...
        if impulse == "MODERATE":
            ...
            if float(c2["low"]) < (low1 - buf1):
                ...
                # OPTION E: entry must be inside allowed windows
                if not in_signal_window(entry_ts):
                    i += 1
                    continue

                # OPTION B: AVWAP rejection + distance check at entry candle
                if not avwap_rejection_ok(df_day, i, entry_idx):
                    i += 1
                    continue
                if not avwap_distance_ok(df_day, entry_idx):
                    i += 1
                    continue
            ...
            if c2_small_green and c2_below_avwap:
                ...
                if float(c3["low"]) < (low2 - buf2):
                    ...
                    # OPTION B: AVWAP rejection + distance check
                    if not avwap_rejection_ok(df_day, i, entry_idx):
                        i += 1
                        continue
                    if not avwap_distance_ok(df_day, entry_idx):
        ...
        if impulse == "HUGE":
            ...
            # Existing logic: ALL bounce closes < AVWAP counts as failed bounce
            fail_avwap_all = bool((closes < avwaps).fillna(False).all())

            # OPTION B touch rejection: at least one bounce candle touches/approaches AVWAP but closes below
            fail_avwap_touch = False
            if REQUIRE_AVWAP_REJECTION and AVWAP_REJECT_TOUCH:
                fail_avwap_touch = bool(((highs >= avwaps) & (closes < avwaps)).fillna(False).any())

            # Keep mid-body failure as an alternate style of failure (existing)
            fail_mid = bool((highs < mid_body).fillna(False).all())

            # With Option B enabled, for AVWAP-based failure we accept either:
            # - all closes below AVWAP (strong failure), OR
            # - touch-and-fail (clean rejection)
            if REQUIRE_AVWAP_REJECTION:
                fail_avwap = bool(fail_avwap_all or fail_avwap_touch)
            else:
                fail_avwap = bool(fail_avwap_all)
            ...
            for j in range(start_j, len(df_day)):
                ...
                # OPTION B: distance from AVWAP at entry candle
                if not avwap_distance_ok(df_day, j):
                    continue
                ...
                if float(df_day.at[j, "low"]) < (bounce_low - buf):
                    ...
                    # OPTION B: also require rejection evidence between impulse and entry (extra guard)
                    if not avwap_rejection_ok(df_day, i, entry_idx):
                        continue
...
def main() -> None:
    ...
    if REQUIRE_AVWAP_REJECTION:
        print(f"[INFO] AVWAP rejection enabled: touch={AVWAP_REJECT_TOUCH}, consec_closes>={AVWAP_REJECT_MIN_CONSEC_CLOSES}, dist={AVWAP_DIST_ATR_MULT}*ATR")
