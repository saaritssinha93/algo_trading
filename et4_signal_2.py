# -*- coding: utf-8 -*-
"""
Created on Wed Jan  8 11:51:31 2025
...
file_handler = TimedRotatingFileHandler(
    "logs\\signal2.log", when="M", interval=30,
...
def detect_signals_in_memory_row_by_row(ticker, df_today, existing_signal_ids):
    """
    Detect signals for the 'latest' (or entire) df_today.
    Relies on daily_change, ADX, RSI, MACD conditions,
    skipping duplicates found in existing_signal_ids.
...
def update_entry_signals_in_file(file_path, tz_obj):
    """
    For a given CSV file at file_path, analyze each row to detect entry signals 
    based on specified technical conditions and update the 'Entry Signal' column.
    """
    # Load and normalize the CSV file
    df = load_and_normalize_csv(file_path, tz=str(tz_obj))
    if df.empty:
        print(f"{file_path}: No data to process.")
        return

    # Ensure the 'Entry Signal' column exists; initialize to 'No'
    if "Entry Signal" not in df.columns:
        df["Entry Signal"] = "No"
    else:
        df["Entry Signal"] = "No"  # Clear previous marks for re-evaluation

    try:
        # Calculate necessary metrics from the data
        ...
        # Skip processing if ADX threshold is not met
        if adx_val <= 20:
            return

        # Decide trend type based on latest data
        if (daily_change > 1.0 and latest_row.get("RSI", 0) > 30 
            and latest_row.get("close", 0) > latest_row.get("VWAP", 0) * 1.02):
            trend_type = "Bullish"
        elif (daily_change < -1.0 and latest_row.get("RSI", 0) < 70 
              and latest_row.get("close", 0) < latest_row.get("VWAP", 0) * 0.98):
            trend_type = "Bearish"
        else:
            # If neither condition is met, no rows will be marked
            return

        # Define thresholds and conditions for analysis
        volume_multiplier = 1.5
        rolling_window = 10
        MACD_BULLISH_OFFSETp = -2
        MACD_BEARISH_OFFSETp = 2

        # Define conditions for marking entry signals based on trend type
        if trend_type == "Bullish":
            conditions = {
                "Pullback": (
                    (df["low"] >= df["VWAP"] * 0.98) &
                    ...
                    (df["MACD"] > MACD_BULLISH_OFFSETp) &
                    (df["close"] > df["20_SMA"]) &
                    (df["Stochastic"] < 60) &
                    (df["Stochastic"].diff() > 0) &
                    (df["ADX"] > 20)
                ),
                "Breakout": (
                    (df["close"] > df["high"].rolling(window=rolling_window).max() * 0.98) &
                    (df["volume"] > volume_multiplier * df["volume"].rolling(window=rolling_window).mean()) &
                    (df["MACD"] > df["Signal_Line"]) &
                    (df["MACD"] > 0) &
                    (df["close"] > df["20_SMA"]) &
                    (df["Stochastic"] > 60) &
            ...
            conditions = {
                "Pullback": (
                    (df["high"] <= df["VWAP"] * 1.02) &
                    (df["close"] < df["open"]) &
                    (df["MACD"] < df["Signal_Line"]) &
                    (df["MACD"] < MACD_BEARISH_OFFSETp) &
                    (df["close"] < df["20_SMA"]) &
                    (df["Stochastic"] > 40) &
                    (df["Stochastic"].diff() < 0) &
                    (df["ADX"] > 20)
                ...
                "Breakdown": (
                    (df["close"] < df["low"].rolling(window=rolling_window).min() * 1.02) &
                    (df["volume"] > volume_multiplier * df["volume"].rolling(window=rolling_window).mean()) &
                    (df["MACD"] < df["Signal_Line"]) &
                    (df["MACD"] < 0) &
                    (df["close"] < df["20_SMA"]) &
        ...
        # Apply each condition to update 'Entry Signal' for matching rows
        for cond in conditions.values():
            df.loc[cond, "Entry Signal"] = "Yes"

        # Save the updated DataFrame back to the CSV
        df.to_csv(file_path, index=False)
        print(f"Updated Entry Signal for file {file_path}")
    except Exception as e:
        logging.error(f"Error updating entry signals in {file_path}: {e}")

def analyze_all_files_for_entry_signals(indicators_dir, tz_obj):
    pattern = os.path.join(indicators_dir, "*_main_indicators.csv")
    files = glob.glob(pattern)
    for file_path in files:
        update_entry_signals_in_file(file_path, tz_obj)
...
# Define timezone and directory settings
india_tz = pytz.timezone('Asia/Kolkata')
INDICATORS_DIR = "main_indicators"

# --- Assume other necessary functions are defined: load_generated_signals, save_generated_signals,
# load_and_normalize_csv, normalize_time, detect_signals_in_memory, etc. ---

def process_single_file(file_path, st, et, existing_ids):
    """
    Process a single main_indicator CSV file:
    - Load and normalize CSV.
    - Print rows with entry signal "yes" for today's date.
    - Filter data for the specified time range.
    - Detect signals row by row.
    Returns:
        List of detected signals for this file.
    """
    ...
    # Ensure 'Entry Signal' and 'date' columns exist
    if 'Entry Signal' in df.columns and 'date' in df.columns:
        # Determine today's date in the specified timezone
        india_tz = pytz.timezone('Asia/Kolkata')
        today_date = datetime.now(india_tz).date()

        # Filter rows where Entry Signal is "yes" and date is today's date
        yes_signal_rows = df[
            (df['Entry Signal'].astype(str).str.lower() == 'yes') &
            (df['date'].dt.date == today_date)
        ]

        for index, row in yes_signal_rows.iterrows():
            print(f"File: {file_path}, Row {index}, Date: {row.get('date', 'N/A')}, Entry Signal: {row['Entry Signal']}")
    else:
        missing_cols = []
        if 'Entry Signal' not in df.columns:
            missing_cols.append('Entry Signal')
        if 'date' not in df.columns:
            missing_cols.append('date')
        print(f"{file_path}: Missing columns: {', '.join(missing_cols)}.")

    # Filter data for the relevant time range
    df_today = df[(df['date'] >= st) & (df['date'] <= et)].copy()

    # Use row-by-row signal detection
    new_signals = detect_signals_in_memory_row_by_row(ticker, df_today, existing_ids)
    return new_signals
...
def find_price_action_entries_for_today(last_trading_day):
    ...
    # Analyze and update entry signals for all indicator files
    analyze_all_files_for_entry_signals("main_indicators", india_tz)
    ...
    max_workers = 50  # Tune based on your system and file I/O performance

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {
            executor.submit(process_single_file, file, st, et, existing_ids): file 
            for file in files
        }
...
def schedule_jobs():
    ...
    interval_minutes = 5
    total_cycles = 300  # From 09:30 to 15:30 inclusive
