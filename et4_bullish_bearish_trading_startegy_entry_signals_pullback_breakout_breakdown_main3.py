...
file_handler = RotatingFileHandler("trading_script_signal.log", maxBytes=5*1024*1024, backupCount=5)  # 5 MB per file, keep 5 backups
...
# Ensure proper cleanup of logging on exit
import atexit
...
def detect_signals_in_memory(ticker: str, df_today: pd.DataFrame, existing_signal_ids: set) -> list:
    ...
    for entry_type, cond in conditions.items():
        if cond.iloc[-1]:  # Check only the latest row
...
def create_papertrade_file(input_file, output_file, last_trading_day):
    ...
    Adds a 'time' column recording the 'logtime' time component.
    """
    india_tz = pytz.timezone('Asia/Kolkata')
    ...
    try:
        ...
        # Filter strictly to today's (9:40–14:20)
        start_time = india_tz.localize(datetime.combine(last_trading_day, datetime_time(9,45)))
        ...
        # Merge into existing papertrade CSV with file locking
...
if __name__=="__main__":
    ...
    # Acquire a global lock to prevent multiple instances
    global_lock = acquire_lock("trading_script_signal.lock")
