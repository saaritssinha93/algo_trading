# -*- coding: utf-8 -*-
"""
Created on Wed Aug 27 15:59:14 2025

@author: Saarit

Combined runner:
1) Normalize Quantity & Total value in papertrade files.
2) Run batched backtests (v7) with SL/TP logic and skip-if-results-exist.
...
PAPERTRADE_PREFIX     = "papertrade_5min_v7_"
RESULT_PREFIX         = "result_papertradesl_v7_"      # per-trade rows
RESULT_FINAL_PREFIX   = "result_finalsl_5min_v7_"      # per-day net % summary
...
# Trade params (existing)
BULL_TARGET_PCT = 0.03
BEAR_TARGET_PCT = 0.03
BULL_STOP_LOSS_PCT = 0.04  # % cap for long (upper bound distance); can be None
BEAR_STOP_LOSS_PCT = 0.04  # % cap for short (upper bound distance); can be None

# --- Dynamic SL knobs ---
USE_DYNAMIC_SL = True

# Choose how to pick the initial SL from multiple candidates
# "tight" -> tightest (closest) valid stop (min risk)  [DEFAULT]
# "wide"  -> widest valid stop (more room)
# "percent_only" -> disable dynamics; only use % caps above
DYN_SL_MODE = "percent_only"

# Volatility & structure
ATR_LEN = 10              # ATR on your backtest interval
ATR_MULT_BULL = 1.0       # initial ATR multiple for long SL
ATR_MULT_BEAR = 1.0       # initial ATR multiple for short SL
SWING_LOOKBACK = 5        # use swing low/high over this window as one candidate

# Trailing stop
USE_TRAILING_SL = True
CHANDELIER_MULT_BULL = 1.5   # trail for long: HighestHighSinceEntry - k * ATR
CHANDELIER_MULT_BEAR = 1.5   # trail for short: LowestLowSinceEntry  + k * ATR
TRAIL_AFTER_R = 0.50         # start trailing after +0.5R move
BREAKEVEN_AFTER_R = 1.00     # move stop to breakeven after +1.0R

# Market microstructure
TICK_SIZE = 0.05  # clamp stops to be at least one tick beyond entry/price
...
# Logging
logging.basicConfig(
    filename="trading_log_batched_v7.log",
    level=logging.INFO,
...
def _compute_atr_from_df(df: pd.DataFrame, period: int) -> np.ndarray:
    """Simple ATR (SMA of TR). Assumes columns: high, low, close."""
    ...
    atr = tr.rolling(int(max(1, period)), min_periods=1).mean().to_numpy(dtype=float)
    # ensure first value exists
    if np.isnan(atr[0]):
        atr[0] = float(tr.iloc[0])
    return atr

def _pick_initial_stop_long(entry: float, low0: float, atr0: float) -> float:
    candidates = []
    # % cap
    if BULL_STOP_LOSS_PCT is not None:
        candidates.append(entry * (1 - float(BULL_STOP_LOSS_PCT)))
    if USE_DYNAMIC_SL and DYN_SL_MODE != "percent_only":
        # ATR based
        if atr0 > 0:
            candidates.append(entry - float(ATR_MULT_BULL) * float(atr0))
        # swing low (at entry bar)
        candidates.append(low0 - 2 * TICK_SIZE)

    # Filter valid (must be below entry by at least a tick)
    valid = [lvl for lvl in candidates if lvl < (entry - TICK_SIZE)]
    if not valid:
        return entry - TICK_SIZE

    if DYN_SL_MODE == "wide":
        # give more room: choose lowest level
        base = min(valid)
    else:
        # "tight" default: choose highest (closest) valid level
        base = max(valid)

    # never above entry - tick
    return min(base, entry - TICK_SIZE)

def _pick_initial_stop_short(entry: float, high0: float, atr0: float) -> float:
    candidates = []
    # % cap
    if BEAR_STOP_LOSS_PCT is not None:
        candidates.append(entry * (1 + float(BEAR_STOP_LOSS_PCT)))
    if USE_DYNAMIC_SL and DYN_SL_MODE != "percent_only":
        # ATR based
        if atr0 > 0:
            candidates.append(entry + float(ATR_MULT_BEAR) * float(atr0))
        # swing high (at entry bar)
        candidates.append(high0 + 2 * TICK_SIZE)

    # Filter valid (must be above entry by at least a tick)
    valid = [lvl for lvl in candidates if lvl > (entry + TICK_SIZE)]
    if not valid:
        return entry + TICK_SIZE

    if DYN_SL_MODE == "wide":
        # give more room: choose highest level
        base = max(valid)
    else:
        # "tight" default: choose lowest (closest) valid level
        base = min(valid)

    # never below entry + tick
    return max(base, entry + TICK_SIZE)
...
def simulate_long(
    ...
    openp = df["open"].to_numpy(dtype=float) if "open" in df.columns else close
    times = df["date"].to_numpy()

    atr = _compute_atr_from_df(df, ATR_LEN)
    target_px = buy_price * (1 + BULL_TARGET_PCT)

    # --- Initial SL (tight/wide/percent_only) ---
    stop = _pick_initial_stop_long(buy_price, low0=low[0], atr0=atr[0])
    R = max(buy_price - stop, TICK_SIZE)  # initial risk in price units

    highest_since = high[0]
    exit_reason = "EOD"
    ...
    for i in range(len(df)):
        ...
        # Trailing rules
        if USE_TRAILING_SL:
            mfe = highest_since - buy_price  # move favorable excursion
            # Start trailing after TRAIL_AFTER_R * R
            trail_stop = stop
            if mfe >= TRAIL_AFTER_R * R:
                # chandelier trail
                trail_chand = highest_since - CHANDELIER_MULT_BULL * atr[i]
                trail_stop = max(trail_stop, trail_chand)

            # Breakeven after 1R
            if mfe >= BREAKEVEN_AFTER_R * R:
                trail_stop = max(trail_stop, buy_price + 0.0)  # BE

            # Always keep stop below current price by at least a tick
            trail_stop = min(trail_stop, close[i] - TICK_SIZE)
            # Never loosen stop
            stop = max(stop, trail_stop)

        # Check events on bar i
        hit_tp = high[i] >= target_px
        hit_sl = low[i]  <= stop

        if hit_tp and hit_sl:
            # Ambiguity within same bar: be conservative -> assume SL hits first if open is closer to stop
            # (you can flip this logic to be optimistic)
            if abs(openp[i] - stop) <= abs(target_px - openp[i]):
                exit_idx, exit_px, exit_reason = i, float(stop), "TSL" if USE_TRAILING_SL else "SL"
            else:
                exit_idx, exit_px, exit_reason = i, float(target_px), "TP"
            break
        elif hit_sl:
            exit_idx, exit_px, exit_reason = i, float(stop), "TSL" if USE_TRAILING_SL else "SL"
            break
...
def simulate_short(
    ...
    # --- Initial SL (tight/wide/percent_only) ---
    stop = _pick_initial_stop_short(sell_price, high0=high[0], atr0=atr[0])
    R = max(stop - sell_price, TICK_SIZE)  # initial risk
    ...
    for i in range(len(df)):
        ...
        # Trailing rules
        if USE_TRAILING_SL:
            mfe = sell_price - lowest_since  # favorable for short
            trail_stop = stop
            if mfe >= TRAIL_AFTER_R * R:
                # chandelier trail for shorts
                trail_chand = lowest_since + CHANDELIER_MULT_BEAR * atr[i]
                trail_stop = min(trail_stop, trail_chand)

            # Breakeven after 1R
            if mfe >= BREAKEVEN_AFTER_R * R:
                trail_stop = min(trail_stop, sell_price - 0.0)  # BE

            # Always keep stop above current price by at least a tick
            trail_stop = max(trail_stop, close[i] + TICK_SIZE)
            # Never loosen stop
            stop = min(stop, trail_stop)

        # Check events on bar i
        hit_tp = low[i]  <= target_px
        hit_sl = high[i] >= stop

        if hit_tp and hit_sl:
            # Ambiguity within same bar: conservative -> assume SL first if open closer to stop
            if abs(openp[i] - stop) <= abs(target_px - openp[i]):
                exit_idx, exit_px, exit_reason = i, float(stop), "TSL" if USE_TRAILING_SL else "SL"
            else:
                exit_idx, exit_px, exit_reason = i, float(target_px), "TP"
...
def process_multiple_dates(
    ...
    output_csv_path: str = "summary_5min_resultsv7.csv",
    pretty_csv_path: Optional[str] = "summary_5min_results_prettyv7.csv",
...
def run_cycle():
    ...
    # Backtest
    logging.info("Starting batched backtest (v7)…")
    ...
    process_multiple_dates(
        ...
        output_csv_path="summary_5min_resultsv7d.csv",
        pretty_csv_path="summary_5min_results_prettyv7d.csv",
        print_console=True
    )
