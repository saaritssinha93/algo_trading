...
SLEEP_SECONDS = 1 * 30  # 5 minutes


def _read_result_df(date: str) -> Optional[pd.DataFrame]:
    filename = f"result_finalsl_5min_v2a_{date}.csv"
    try:
...
def _read_papertrade_df(date: str) -> Optional[pd.DataFrame]:
    filename = f"papertrade_5min_v2a_{date}.csv"
    try:
        df = pd.read_csv(filename)
    except FileNotFoundError:
...
def process_multiple_dates(
    ...
    for date in dates:
        ...
        # robust: handle case/whitespace + non-numeric values
    ...
    merged.loc[~mask_overall, 'Net Percentage'], errors='coerce'
        ...
        ticker_count = max(merged['Ticker'].nunique() - 1, 0)
...
def run_once():
    ...
    process_multiple_dates(
        ...
        output_csv_path="summary_5min_resultsv2a.csv",
        pretty_csv_path="summary_5min_results_prettyv2a.csv",
        print_console=True
    )
