...
ENTRIES_DIR    = "main_indicators_history_entries_5minv4"
...
GLOBAL_MIN_PROB  = 0.35      # relaxed floor
GLOBAL_MAX_PROB  = 0.70      # relaxed ceiling
ML_SOFT_MARGIN   = 0.03      # lets near-threshold signals pass

RANGE_PADDING_FRAC = 0.05    # widen each numeric constraint by ±5% (range/min/max)
...
file_handler = TimedRotatingFileHandler(
    "logs\\signalnewn5minv4d.log", when="M", interval=30, backupCount=5, delay=True
)
...
def detect_signals_in_memory(
    ...
    fitted_csv_path: str = "fitted_indicator_ranges_by_ticker20_80.csv",  # permissive
    volrel_window: int = 10,
    volrel_min_periods: int = 1,      # relaxed
    debug: bool = False,
    volrel_gate_min: float = 0.95,    # relaxed
...
def find_price_action_entries_for_date(target_date: datetime.date) -> pd.DataFrame:
    ...
    DETECTOR_VOLREL_MIN_PERIODS = 1   # relaxed
