...
#    combined_cache_exists = os.path.exists(combined_indicators_file)
    ...
    def fetch_historical(ticker):
        from_date_hist = from_date_intra - timedelta(days=7)  # buffer for historical
    ...
    # Fetch intraday and historical data (code omitted for brevity)   
    ...
    if not today_data.empty:
        ...
        if ticker_entries:
            entry_times = [pd.to_datetime(entry['date']).astimezone(india_tz) for entry in ticker_entries]
    ...
    if not existing_indicators.empty:
        new_rows = combined_data[~combined_data['Signal_ID'].isin(existing_indicators['Signal_ID'])].copy()
    else:
    ...
    # Normalize and sort the combined data
    combined_data = sort_dataframe_by_date(combined_data)
    return combined_data
...
def recalc_indicators(data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_rows = data.tail(200).copy()  # Ensure new_rows is a copy
...
if __name__ == "__main__":    
    # First, ensure CalcFlag is initialized in all main_indicator CSVs
    initialize_calc_flag_in_main_indicators(INDICATORS_DIR)        
    # Perform validation before running the main processes
    validate_and_correct_main_indicators(selected_stocks, india_tz)
