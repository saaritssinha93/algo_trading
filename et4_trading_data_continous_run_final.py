...
Added `Daily_Change` column for each 15-min candle. The formula is:
    Daily_Change = ((close(15min candle) - prev_close) / prev_close) * 100

Where `prev_close` is the last 15min candle close from the previous trading day (i.e., skipping weekends/holidays).
"""
...
def normalize_time(df, timezone='Asia/Kolkata'):
    ...
    if 'date' not in df.columns:
        ...
        return pd.DataFrame()  # or decide to raise an error
...
def cache_update(ticker, data_type, fetch_function, freshness_threshold=None, bypass_cache=False):
    ...
    if not bypass_cache and is_cache_fresh(cache_path, freshness_threshold):
        try:
            ...
            if os.path.exists(cache_path) and os.path.getsize(cache_path) > 0:
                ...
                if 'date' not in df.columns:
                    logging.warning(f"Existing cache file missing 'date' column for {ticker}. Deleting file.")
                    os.remove(cache_path)
                    raise ValueError("Cache file missing 'date' column")
                df['date'] = pd.to_datetime(df['date'])
                df = normalize_time(df)
                return df
            else:
                raise ValueError("Cache file is empty or missing")
        except Exception as e:
    ...
    if os.path.exists(cache_path):
        ...
        if 'date' in existing.columns:
            ...
            logging.warning(f"Existing cache file missing 'date' column for {ticker}. It will be overwritten.")
...
def fetch_intraday_data_with_historical(ticker, token, from_date_intra, to_date_intra, interval="15minute"):
    """
    Fetch intraday + some historical data for indicator continuity.
    Then compute the latest row with indicators, including Daily_Change.

    Daily_Change = ((close of latest 15min candle) - previous day's close) / previous day's close * 100
    """
    ...
    # Fetch fresh intraday data (bypass cache) + update historical from cache
    ...
    # Calculate indicators for the latest row
    latest_row = recalc_indicators_for_latest_row(combined)
    if latest_row.empty:
        return pd.DataFrame()

    # Compute Daily Change % = ((close - prev_close) / prev_close) * 100
    # where `prev_close` is from the last trading day's final 15-min candle.
    try:
        # Filter out today's intraday portion, pick last row from older data
...
def process_ticker(ticker, from_date_intra, to_date_intra, last_trading_day, india_tz):
    """
    Fetches the new 15-min candle with updated indicators (and Daily_Change),
    then appends that row to the CSV for the ticker.
    """
    try:
        with api_semaphore:
        ...
        # Always set Entry Signal to "No" (just to be sure)
...
def run():
    ...
    - For each ticker, fetches new row with updated indicators (and daily change) and appends to CSV
    """
...
def run_periodically():
    ...
    start_dt = datetime.combine(today, datetime_time(9, 15, 15))
...
if __name__ == "__main__":
    run_periodically()
    # Optionally call main1 at the very end if desired:
    # main1()
