# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 22:47:51 2025
...
def monitor_ticker_in_window(ticker, buy_price, quantity, total_value_bought, trade_time, kite, instrument_df, target_percentage, interval, trend_type):
    ...
    first_target_price = buy_price * (1 + 0.027)  # First target at 1.5% above buy price
    ...
    end_time = dt.datetime(2025, 1, 10, tzinfo=local_tz)
    ...
    for _, price_row in historical_prices.iterrows():
        ...
        if not first_execution_done:
            if current_price >= first_target_price:
                ...
                second_stop_loss_price = first_target_price * (1 - 0.00375)  # 1% below the current price
                second_target_price = first_target_price * (1 + 0.0075)  # 1% above the current price
...
def monitor_ticker_in_window_bear(ticker, sell_price, quantity, total_value_sold, trade_time, kite, instrument_df, interval, trend_type):
    ...
    first_target_price = sell_price * (1 - 0.027)  # First target at 2% below sell price
    ...
    for _, price_row in historical_prices.iterrows():
        ...
        if not first_execution_done:
            if current_price <= first_target_price:
                ...
                second_target_price = first_target_price * (1 - 0.0075)  # 1% below the current target
                second_stop_loss_price = first_target_price * (1 + 0.00375)  # 1% above the current target
                first_execution_done = True
