...
def detect_signals_in_memory(
    ...
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        if (
            ...
            row['ADX'] > 40 and
            row['Stoch_%K'] > 85 and
            (row['Stoch_%K'] > row['Stoch_%D'] + 10) and
            row['volume'] > 3 * row['volume_mean_20'] and
            60 < row['RSI'] < 68 and
            row['MACD_Hist'] > 0.25 and
            prev['volume'] <= 3 * prev['volume_mean_20']
        ):
...
if __name__ == "__main__":
    ...
    print(f"Processing {len(date_args)} dates in parallel using {max_workers} workers...\n")

    results = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_date = {executor.submit(run_for_date, dstr): dstr for dstr in date_args}
        for future in tqdm(as_completed(future_to_date), total=len(future_to_date), desc="Dates done"):
            dstr = future_to_date[future]
            try:
                result = future.result()
                results.append((dstr, "OK"))
            except Exception as exc:
                print(f"[{dstr}] generated an exception: {exc}")
                results.append((dstr, "ERROR"))
