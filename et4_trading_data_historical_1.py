...
Now, for each row of the day the indicator values are calculated using
a rolling window of up to the last 100 rows.
"""
...
def fetch_intraday_data_with_historical(ticker, token, from_dt, to_dt, interval="15minute"):
    ...
    for context, combine, then return the full DataFrame.
    """
...
def recalc_indicators_for_data(df: pd.DataFrame) -> pd.DataFrame:
    """
    Recalculates all indicators for every row in the DataFrame (15min candles)
    using a rolling window of up to the last 100 rows (or all available rows if fewer)
    ...
    # Prepare empty lists to hold indicator values for each row.
    RSI_values = [None] * n
    ATR_values = [None] * n
    MACD_values = [None] * n
    Signal_Line_values = [None] * n
    Histogram_values = [None] * n
    Upper_Band_values = [None] * n
    Lower_Band_values = [None] * n
    Stochastic_values = [None] * n
    VWAP_values = [None] * n
    SMA20_values = [None] * n
    Recent_High_values = [None] * n
    Recent_Low_values = [None] * n
    ADX_values = [None] * n

    for i in range(n):
        window = out.iloc[max(0, i-99): i+1].copy()
        # RSI calculation
        try:
            rsi_series = calculate_rsi(window['close'], timeperiod=14)
            RSI_values[i] = rsi_series.iloc[-1]
        except Exception:
            RSI_values[i] = None
        # ATR calculation
        try:
            atr_series = calculate_atr(window)
            ATR_values[i] = atr_series.iloc[-1]
        except Exception:
            ATR_values[i] = None
        # MACD, Signal_Line, Histogram
        try:
            macd_df = calculate_macd(window, fast=12, slow=26, signal=9)
            MACD_values[i] = macd_df['MACD'].iloc[-1]
            Signal_Line_values[i] = macd_df['Signal_Line'].iloc[-1]
            Histogram_values[i] = macd_df['Histogram'].iloc[-1]
        except Exception:
            MACD_values[i] = Signal_Line_values[i] = Histogram_values[i] = None
        # Bollinger Bands
        try:
            boll_df = calculate_bollinger_bands(window, period=20, up=2, dn=2)
            Upper_Band_values[i] = boll_df['Upper_Band'].iloc[-1]
            Lower_Band_values[i] = boll_df['Lower_Band'].iloc[-1]
        except Exception:
            Upper_Band_values[i] = Lower_Band_values[i] = None
        # Stochastic
        try:
            stoch_series = calculate_stochastic(window, fastk=14, slowk=3)
            Stochastic_values[i] = stoch_series.iloc[-1]
        except Exception:
            Stochastic_values[i] = None
        # VWAP
        try:
            vwap_series = (window['close'] * window['volume']).cumsum() / window['volume'].cumsum()
            VWAP_values[i] = vwap_series.iloc[-1]
        except Exception:
            VWAP_values[i] = None
        # 20 SMA
        try:
            SMA20_values[i] = window['close'].rolling(window=20, min_periods=1).mean().iloc[-1]
        except Exception:
            SMA20_values[i] = None
        # Recent High (max of last 5 rows)
        try:
            Recent_High_values[i] = window['high'].rolling(window=5, min_periods=1).max().iloc[-1]
        except Exception:
            Recent_High_values[i] = None
        # Recent Low (min of last 5 rows)
        try:
            Recent_Low_values[i] = window['low'].rolling(window=5, min_periods=1).min().iloc[-1]
        except Exception:
            Recent_Low_values[i] = None
        # ADX
        try:
            adx_series = calculate_adx(window, period=14)
            ADX_values[i] = adx_series.iloc[-1]
        except Exception:
            ADX_values[i] = None

    # Assign the computed lists as new columns in the DataFrame.
    out['RSI'] = RSI_values
    out['ATR'] = ATR_values
    out['MACD'] = MACD_values
    out['Signal_Line'] = Signal_Line_values
    out['Histogram'] = Histogram_values
    out['Upper_Band'] = Upper_Band_values
    out['Lower_Band'] = Lower_Band_values
    out['Stochastic'] = Stochastic_values
    out['VWAP'] = VWAP_values
    out['20_SMA'] = SMA20_values
    out['Recent_High'] = Recent_High_values
    out['Recent_Low'] = Recent_Low_values
    out['ADX'] = ADX_values
...
def process_ticker_for_date(ticker, day_date):
    ...
    recalc indicators for every row (using a trailing window of up to 100 rows)
    and append the resulting data to the historical indicators CSV.
    Returns the full DataFrame (with indicators) for that ticker.
    """
    try:
        ...
        # 2) Recalculate indicators for every row using a trailing window of up to 100 rows
...
def run_historical(dates_list):
    ...
    fetch 15-minute data, recalc indicators for every row using a trailing window
    of up to 100 rows and store final outputs in 'main_indicators_historical' directory.
    """
    os.makedirs(HIST_INDICATORS_DIR, exist_ok=True)
