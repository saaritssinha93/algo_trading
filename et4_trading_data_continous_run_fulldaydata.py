# -*- coding: utf-8 -*-
"""
Created on Mon Feb 10 11:31:42 2025
...
# =========================================
# Recalculate Indicators for the FULL DataFrame
# =========================================
def recalc_indicators_for_all_rows(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calculates indicators for ALL rows in the DataFrame, not just the last row.
    Returns df with columns: RSI, ATR, MACD, Signal_Line, Histogram,
    ...
    try:
        ...
        # VWAP for entire DataFrame (cumulative)
        ...
        logging.error(f"Indicator calculation error in recalc_indicators_for_all_rows: {e}")
        raise

    # Fill any NaNs from initial rolling
    df = df.ffill().bfill()
    return df
...
# =========================================
# Updated fetch to Return ALL Today's Rows
# =========================================
def fetch_intraday_data_with_historical(ticker, token, from_date_intra, to_date_intra, interval="15minute"):
    """
    Fetches intraday bars for 'today' plus some historical data for context,
    calculates indicators for the FULL DataFrame, 
    then returns only today's bars with indicators (not just the latest row).
    """
    ...
    def fetch_historical(_):
        # fetch ~10 days prior for context
    ...
    # 1) Calculate indicators for the entire combined DataFrame
    full_with_indicators = recalc_indicators_for_all_rows(combined)

    # 2) Filter only today's rows: from from_date_intra..to_date_intra
    mask_today = (full_with_indicators['date'] >= from_date_intra) & (full_with_indicators['date'] <= to_date_intra)
    today_bars = full_with_indicators.loc[mask_today].copy()

    if today_bars.empty:
        logging.warning(f"No intraday bars found for {ticker} in today's timeframe.")
        return pd.DataFrame()

    # Ensure columns
    if 'Entry Signal' not in today_bars.columns:
        today_bars['Entry Signal'] = 'No'
    else:
        today_bars['Entry Signal'] = 'No'

    # Assign 'logtime' to all rows
    now_str = datetime.now(india_tz).isoformat()
    today_bars['logtime'] = now_str

    # Generate a unique Signal_ID for each row
    if 'Signal_ID' not in today_bars.columns:
        today_bars['Signal_ID'] = today_bars.apply(lambda r: build_signal_id(ticker, r), axis=1)
    else:
        mask_missing = today_bars['Signal_ID'].isna() | (today_bars['Signal_ID'] == "")
        today_bars.loc[mask_missing, 'Signal_ID'] = today_bars.loc[mask_missing].apply(
...
def process_ticker(ticker, from_date_intra, to_date_intra, last_trading_day, india_tz):
    """
    Fetches ALL today's intraday bars (not just the latest) with computed indicators
    and appends them into the main_indicators CSV.
    """
    try:
        with api_semaphore:
            ...
            # Now we get *all* today's intraday bars with indicators
            today_bars = fetch_intraday_data_with_historical(
                ticker, token,
                from_date_intra, to_date_intra,
                interval="15minute"
            )
            if today_bars.empty:
                logging.warning(f"[{ticker}] No new data returned for today's timeframe.")
                return None, []
        ...
        # Combine
        combined = pd.concat([existing, today_bars], ignore_index=True)

        # Deduplicate by Signal_ID if present, otherwise by date
        ...
        logging.info(f"[{ticker}] Appended {len(today_bars)} intraday rows to {main_path}.")
        return today_bars, []
    except Exception as e:
...
def run():
    ...
    - For each ticker, fetches new bars (for today) with updated indicators and appends to CSV
    """
...
def initialize_calc_flag_in_main_indicators(indicators_dir="main_indicators_test"):
    import glob
    import os
    import pandas as pd
    import logging
...
def run_periodically():
    ...
    while True:
        ...
        # But if next_run is beyond 3:30:15, the while loop will end next iteration.
