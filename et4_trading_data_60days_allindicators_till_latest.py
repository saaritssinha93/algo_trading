...
###############################################################################
# 11) Missing Data Check for Existing CSVs
###############################################################################
def simulate_missing_candles_for_ticker(ticker: str):
    """
    For a given ticker, read its CSV file and check if the most recent candle is up-to-date.
    If not, generate missing 15-min time slots (from the last candle +15min until now)
    and for each missing slot, fetch the simulated candle data and print it.
    """
    csv_path = os.path.join(INDICATORS_DIR, f"{ticker}_main_indicators.csv")
    if not os.path.exists(csv_path):
        logging.info(f"[{ticker}] => CSV file does not exist. Skipping missing data simulation.")
        return
    try:
        df = pd.read_csv(csv_path)
    except Exception as e:
        logging.error(f"[{ticker}] => Error reading CSV: {e}")
        return
    if df.empty:
        logging.info(f"[{ticker}] => CSV is empty. Skipping.")
        return
    try:
        df['date'] = pd.to_datetime(df['date'])
    except Exception as e:
        logging.error(f"[{ticker}] => Error parsing date in CSV: {e}")
        return
    last_date = df['date'].max()
    if last_date.tzinfo is None:
        last_date = last_date.tz_localize(INDIA_TZ)
    now = datetime.now(INDIA_TZ)
    if now - last_date <= timedelta(minutes=15):
        logging.info(f"[{ticker}] => CSV is up-to-date. Last candle: {last_date}.")
        return
    else:
        logging.info(f"[{ticker}] => CSV is outdated. Last candle: {last_date}. Generating missing candles.")
    # Generate missing time slots from (last_date + 15 min) until now
    missing_start = last_date + timedelta(minutes=15)
    missing_slots = generate_time_slots_from(missing_start, now, interval_minutes=15)
    if not missing_slots:
        logging.info(f"[{ticker}] => No missing time slots found.")
        return
    token = shares_tokens.get(ticker)
    if not token:
        logging.warning(f"[{ticker}] => No instrument token found for missing data simulation.")
        return
    for slot in missing_slots:
        candle_row = fetch_single_candle_data(ticker, token, slot)
        if candle_row.empty:
            logging.info(f"[{ticker}] => No candle data found for time slot {slot}.")
        else:
            logging.info(f"[{ticker}] => Missing candle data for {slot}:")
            print(candle_row)
