...
def fetch_intraday_data_with_historical(
    ...
    # The rest of your code to save combined_data, handle Signal_ID, etc. remains unchanged
    ...
    if os.path.exists(main_indicators_path):
        ...
        existing_indicators['date'] = existing_indicators['date'].dt.tz_localize('Asia/Kolkata') if existing_indicators['date'].dt.tz is None else existing_indicators['date'].dt.tz_convert('Asia/Kolkata')
    else:
    ...
    if not new_rows.empty:
        ...
        logging.info(f"Appended {len(new_rows)} new rows of indicator data for {ticker}.")
    else:
        logging.info(f"No new rows to append for {ticker}.")
...
def find_price_action_entry(last_trading_day):
    ...
    # **No Reassignment of Signal_ID Here**
    # We trust that Signal_ID is correctly inherited from the main_indicators.csv data.
    ...
    def unify_existing_csv(filename):
        if os.path.exists(filename):
            ...
            # No Signal_ID generation here since we rely on existing Signal_ID
...
def analyze_latest_interval(data_dir=INDICATORS_DIR, tz='Asia/Kolkata'):
    ...
    Determine the entry signal ("Yes" or "No") using apply_trend_conditions and screen_trend logic.
    """
    timezone = pytz.timezone(tz)
    current_time = datetime.now(timezone)
    # Determine the most recent 15-minute interval
    ...
    for f in files:
        ...
        try:
            ...
            # Floor the dates to 15 minutes
            df['floored_date'] = df['date'].dt.floor('15min')

            # Filter for the expected interval rows
            latest_rows = df[df['floored_date'] == expected_interval]
            if latest_rows.empty:
                print(f"[{ticker}] No row found for the expected interval ({expected_interval}).")
                continue

            # We'll assume the last appended row is the one we need
            latest_row_index = latest_rows.index[-1]

            # Identify the last trading day (based on the max date in the data)
            last_trading_day = df['date'].dt.date.max()

            # Define today's trading window
            ...
            if today_data.empty:
                # If no today's data, set "No" and continue
            ...
            # Apply trend conditions to find potential entries
            ticker_entries = apply_trend_conditions(today_data, daily_change, ticker, last_trading_day)

            # Convert all entry 'date' fields to proper timezone-aware datetimes
            entry_times = []
            for entry in ticker_entries:
            ...
            # Determine if the expected interval matches any entry time (within a 1-min tolerance)
            # You can adjust the tolerance as needed
            tolerance = timedelta(minutes=1)
            matched_entry = any(abs((expected_interval - t)) <= tolerance for t in entry_times)

            # Set entry signal based on whether we found a matching entry time
...
def create_papertrade_file(input_file, output_file, last_trading_day):
    ...
    try:
        ...
        if not validate_dataframe(earliest_entries, ["Ticker", "date", "Entry Type", "Trend Type", "Price", "Daily Change %", "VWAP", "ADX", "Signal_ID"]):
