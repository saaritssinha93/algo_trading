...
def recalc_indicators(data: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculate 50 SMA
    try:
        sma50 = new_rows['close'].rolling(window=50, min_periods=1).mean()
        logging.debug(f"50_SMA Series length: {len(sma50)}, Expected: {len(new_rows)} rows.")
        assert len(sma50) == len(new_rows), "Mismatch in 50_SMA Series row count."
        data.loc[new_rows.index, '50_SMA'] = sma50
        logging.debug("50_SMA assigned successfully.")
    except AssertionError as ae:
        logging.error(f"Assertion Error: {ae}")
        raise
    except Exception as e:
        logging.error(f"Error calculating 50_SMA: {e}")
        raise        
...
def apply_trend_conditions(ticker: str, last_trading_day: pd.Timestamp, india_tz) -> list:
    """
    Detects Bullish or Bearish trend entries based on stricter breakout, breakdown, 
    and pullback conditions, using the *_main_indicators.csv file.
    """
    main_indicators_path = os.path.join(INDICATORS_DIR, f"{ticker}_main_indicators.csv")
    ...
    try:
        ...
        strong_trend = latest_data["ADX"] > 25  # Stricter ADX threshold for strong trends

        if strong_trend:
            # Bullish conditions
            if (
                daily_change > 1.5  # Stricter daily change
                and latest_data["RSI"] > 30  # RSI showing stronger momentum
            ...
            # Bearish conditions
                ...
                daily_change < -1.5  # Stricter daily change
                and latest_data["RSI"] < 70  # RSI showing weakening momentum
                and latest_data["close"] < latest_data["VWAP"]
            ):
...
def screen_trend(ticker: str, last_trading_day: pd.Timestamp, india_tz, trend_type: str, change: float, today_data: pd.DataFrame) -> list:
    """
    Identifies specific trend entry points (Pullback/Breakout or Pullback/Breakdown) 
    based on stricter thresholds but retains all indicator conditions.
    """
    trend_entries = []
    volume_multiplier = 2.0  # Stricter volume spike requirement

    # ATR for adaptive thresholds
    today_data["ATR"] = today_data["high"] - today_data["low"]
    
    if trend_type == "Bullish":
        conditions = {
            "Pullback": (
                (today_data["close"] > today_data["VWAP"] - (0.5 * today_data["ATR"])) &  # Close to VWAP
                (today_data["close"] > today_data["20_SMA"]) &                           # Above 20-SMA
                (today_data["Stochastic"] < 35) & (today_data["Stochastic"].diff() > 0) & # Stochastic showing momentum
                (today_data["volume"] < today_data["volume"].rolling(window=10).mean() * 0.8) &  # Stricter low volume
                (today_data["ADX"] > 25) &                                               # Stricter ADX
                (today_data["close"] > today_data["50_SMA"])                             # Trend confirmation
            ),
            "Breakout": (
                (today_data["close"] > today_data["high"].rolling(window=10).max() + (2 * today_data["ATR"])) &  # Stricter breakout
                (today_data["volume"] > volume_multiplier * today_data["volume"].rolling(window=10).mean()) &    # Strong volume spike
                (today_data["MACD"] > today_data["Signal_Line"]) &                                              # MACD signal confirmation
                (today_data["RSI"] > 65)                                                                        # Stronger RSI
            ),
        }
    else:  # Bearish
        conditions = {
            "Pullback": (
                (today_data["close"] < today_data["VWAP"] + (0.5 * today_data["ATR"])) &  # Close to VWAP
                (today_data["close"] < today_data["20_SMA"]) &                            # Below 20-SMA
                (today_data["Stochastic"] > 65) & (today_data["Stochastic"].diff() < 0) & # Stochastic showing weakening momentum
                (today_data["volume"] < today_data["volume"].rolling(window=10).mean() * 0.8) &  # Stricter low volume
                (today_data["ADX"] > 25) &                                                # Stricter ADX
                (today_data["close"] < today_data["50_SMA"])                              # Trend confirmation
            ),
            "Breakdown": (
                (today_data["close"] < today_data["low"].rolling(window=10).min() - (2 * today_data["ATR"])) &   # Stricter breakdown
                (today_data["volume"] > volume_multiplier * today_data["volume"].rolling(window=10).mean()) &   # Strong volume spike
                (today_data["MACD"] < today_data["Signal_Line"]) &                                              # MACD signal confirmation
                (today_data["RSI"] < 35)                                                                        # Stricter RSI
            ),
        }
...
def ensure_all_indicators(data: pd.DataFrame, ticker: str) -> pd.DataFrame:
    ...
    required_columns = {
        ...
        '20_SMA', '50_SMA', 'Recent_High', 'Recent_Low', 'Stochastic', 'ADX'
    }
...
def combine_and_validate_data(intraday_data, ticker, india_tz):
    ...
    required_columns = [
        ...
        "VWAP", "20_SMA", "50_SMA", "Recent_High", "Recent_Low"
    ]
    missing_columns = set(required_columns) - set(data.columns)
    if missing_columns:
