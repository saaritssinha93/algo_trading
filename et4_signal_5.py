...
CONFIG = {
    ...
    "LOG_LEVEL": "INFO",       # set to INFO, DEBUG, WARNING, etc.
    "MARKET_START": "09:30",
    "MARKET_END": "15:30",
    "INTERVAL_MINUTES": 15,
...
def find_price_action_entries_for_today(last_trading_day):
    """
    Parallel processing to detect signals in each CSV.
    """
    ...
    if not all_signals:
        logger.info("No new signals detected today.")
        return pd.DataFrame()

    signals_df = pd.DataFrame(all_signals)
    signals_df.sort_values('date', inplace=True)
    signals_df.reset_index(drop=True, inplace=True)
    logger.info(f"Total new signals detected: {len(signals_df)}")
    return signals_df
