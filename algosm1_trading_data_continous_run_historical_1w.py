# -*- coding: utf-8 -*-
"""
Fetch WEEKLY candles for selected_stocks for the LAST 1 YEAR up to now (IST),
compute indicators, and write *_main_indicators_weekly.csv files.

Notes on change features (kept for schema compatibility):
    • Intra_Change  -> % vs previous WEEK's close (week-over-week)
    • Daily_Change  -> % vs previous WEEK's final close (identical to Intra_Change for weekly)
"""
...
CACHE_DIR = "data_cache_weekly"
INDICATORS_DIR = "main_indicators_weekly"
...
# ========= DATA FETCH (WEEKLY) =========
def fetch_historical_weekly_df(token, start_dt_ist):
    """
    Fetch weekly bars from start_dt_ist to now in ~365-day chunks.
    """
    ...
    while s < end:
        ...
        for attempt in range(1, MAX_RETRIES+1):
            try:
                raw = kite.historical_data(token, s, e, "week")
                df = pd.DataFrame(raw)
                if df.empty: break
                # tz-aware IST
...
# ========= PER-TICKER PIPELINE =========
def process_ticker_last_year_weekly(ticker, token):
    print(f"\nFetching {ticker} ...")
    df = fetch_historical_weekly_df(token, START_DATE)
    if df.empty:
...
if __name__ == "__main__":
    print(f"Fetching window (weekly): {START_DATE.strftime('%Y-%m-%d')} to {datetime.now(india_tz).strftime('%Y-%m-%d')} (IST)")
    for ticker in selected_stocks:
        ...
        if not token:
            ...
            continue
        try:
            process_ticker_last_year_weekly(ticker, token)
