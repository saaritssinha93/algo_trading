# -*- coding: utf-8 -*-
"""
avwap_common_v2.py — Shared infrastructure for AVWAP v11 Long + Short strategies
=============================================================================
...
def default_short_config(**overrides) -> StrategyConfig:
    """Factory for the SHORT side with typical v11 defaults (v2 tweaks applied)."""
    base = dict(
        side="SHORT",
        stop_pct=0.0100,
        target_pct=0.0100,
        slippage_pct=0.0005,   # 5 bps one-way
        commission_pct=0.0003, # 3 bps round-trip
        mod_impulse_min_atr=0.45,
        rsi_max_short=55.0,
        stochk_max=75.0,
        # v2: remove early losing bucket + no PM window
        signal_windows=[(dtime(10, 15, 0), dtime(11, 30, 0))],
        # v2: tighter selection
        topn_per_day=8,
    )
...
def default_long_config(**overrides) -> StrategyConfig:
    """Factory for the LONG side with typical v11 defaults (v2 tweaks applied)."""
    base = dict(
        side="LONG",
        stop_pct=0.0100,
        target_pct=0.0200,
        ...
        # v2: tighter selection
        topn_per_day=8,
        # v2: disable the weaker long setup by default
        enable_setup_a_pullback_c2_break=False,
    )
    base.update(overrides)
    return StrategyConfig(**base)
