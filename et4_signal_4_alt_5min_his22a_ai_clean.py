...
SIGNALS_DB     = "generated_signals_historical5minv3.json"
ENTRIES_DIR    = "main_indicators_history_entries_5minv3"  # where day's detected signals are saved
...
file_handler = TimedRotatingFileHandler(
    "logs\\signalnewn5minv3d.log", when="M", interval=30, backupCount=5, delay=True
)
...
def _best_threshold_from_oof(df_tkr: pd.DataFrame) -> float:
    ...
    if np.unique(y).size < 2:
        # Degenerate: only one class in OOF → use mean prob (clipped)
        thr = float(np.clip(np.nanmean(p) if len(p) else GLOBAL_MIN_PROB, GLOBAL_MIN_PROB, GLOBAL_MAX_PROB))
        return thr
    # Candidate thresholds: percentiles of p + some grid
...
def run_for_date(date_str: str) -> None:
    try:
        ...
        papertrade_file = f"papertrade_5min_v3_{date_label}.csv"
