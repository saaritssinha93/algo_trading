...
- POS_ENTRY, POS_EXIT, POS_DIR (+1/-1/0) for swing regime
...
- Exit on ATR stop-loss OR ATR take-profit (uses LOW/HIGH for realism)
...
- No new entry on the last bar of day
...
- Optional ATR-based stop (using ATR_1d if available, else ATR_15m, else intraday ATR estimate)
...
POS_USE_TREND_FLIP_EXIT = True       # exit long if EMA50_1d < EMA200_1d, etc.
POS_USE_ATR_STOP = True
POS_SL_ATR_MULT = 2.5                # swing stop based on ATR_1d if available (else ATR_15m)
...
def _read_etf_file(ticker: str, tf: str) -> pd.DataFrame:
    ...
    df.index.name = "ts"  # standardize index name
    return df
...
def _build_intraday_entries_exits(
    feat: pd.DataFrame,
    has_vwap: bool,
    vwap_col: str | None,
    atr_price: pd.Series
    ...
    Creates:
    - INTRA_DIR (+1/-1/0),
    - INTRA_ENTRY, INTRA_EXIT (0/1),
    - INTRA_REASON_ENTRY, INTRA_REASON_EXIT

    Notes:
    - SL/TP checks use LOW/HIGH (more realistic than close-only)
    - No new entries on last bar of the day
    - Always square-off on last bar if position open
    """
    idx = feat.index
    day = idx.date

    close = pd.to_numeric(feat["close"], errors="coerce").values
    high = pd.to_numeric(feat["high"], errors="coerce").values
    low  = pd.to_numeric(feat["low"], errors="coerce").values

    vwap = None
    if has_vwap and vwap_col and (vwap_col in feat.columns):
        vwap = pd.to_numeric(feat[vwap_col], errors="coerce").values
    ...
    for d in unique_days:
        ...
        for i in day_ix:
            ...
            # --- forced EOD square-off if position open ---
            if i == last_bar_i:
                if pos != 0:
                    exit_[i] = 1
                    ...
                    pos = 0
                    intra_dir[i] = 0
                # do NOT open new trade on last bar
                continue
            ...
            if pos == 0:
                if _is_buy_sig(sig[i]):
                    pos = 1
                    ep = close[i]
                    a = atrp[i] if atrp[i] > 0 else (ep * 0.002)  # fallback ~0.2%
                    ...
                    pos = -1
                    ep = close[i]
                    a = atrp[i] if atrp[i] > 0 else (ep * 0.002)
                    stop = ep + INTRA_SL_ATR_MULT * a
                    tp   = ep - INTRA_TP_ATR_MULT * a
            ...
            # ---------------- EXIT ----------------
            if pos == 1:
                # stop / tp (use LOW/HIGH)
                if (not np.isnan(low[i])) and low[i] <= stop:
                    exit_[i] = 1
                    reason_exit[i] = "SL_ATR"
                    pos = 0
                elif (not np.isnan(high[i])) and high[i] >= tp:
                    exit_[i] = 1
                ...
                if (not np.isnan(high[i])) and high[i] >= stop:
                    exit_[i] = 1
                    reason_exit[i] = "SL_ATR"
                    pos = 0
                elif (not np.isnan(low[i])) and low[i] <= tp:
                    exit_[i] = 1
                    reason_exit[i] = "TP_ATR"
                    pos = 0
                else:
    ...
    for i in range(len(feat)):
        ...
        if pos == 1:
            ...
            if pos == 1 and POS_USE_ATR_STOP:
                a = atrp[i] if atrp[i] > 0 else (close[i] * 0.01)  # fallback 1%
                if np.isnan(stop):
...
def build_features_and_signals(ticker: str) -> StrategyResult:
    ...
    # intraday ATR price series (after VOL_UNIT is stabilized)
    if atr_col is not None and (atr_col in feat.columns) and feat[atr_col].notna().any():
        atr_price_intra = pd.to_numeric(feat[atr_col], errors="coerce").fillna(0)
    else:
        atr_price_intra = (pd.to_numeric(feat["close"], errors="coerce") * (feat["VOL_UNIT"] / 100.0)).fillna(0)
    ...
    if ENABLE_TREND_PULLBACK and has_vwap:
        ...
        if ema_fast is not None and ema_slow is not None and (ema_fast in feat.columns) and (ema_slow in feat.columns):
            ...
            # ATR price units for pullback zone
            atr_price_pb = atr_price_intra.replace(0, np.nan)
            atr_price_pb = atr_price_pb.fillna(pd.to_numeric(feat["close"], errors="coerce") * 0.002)  # fallback

            near_fast = (feat["close"] - ef).abs() <= (PULLBACK_ATR_MULT * atr_price_pb)
    ...
    # Final intraday signal priority
    feat["SIG_INTRA"] = "HOLD"
    ...
    # ---------------- ADD ENTRY/EXIT FLAGS (NEW) ----------------

    feat = _build_intraday_entries_exits(
        feat=feat,
        has_vwap=has_vwap,
        vwap_col=vwap_col,
        atr_price=atr_price_intra
    )

    # positional ATR price series: prefer ATR_1d then ATR_15m then fallback
    atr_pos_col = _pick_col(feat, ["ATR_1d", "ATR_15m", "ATR"]) or _pick_like(feat, "atr_1d")
    ...
    # add ETF input exists flags (constant across rows)
    ...
    summary_cols = [
        "close",
        "SIG_INTRA", "INTRA_ENTRY", "INTRA_EXIT", "INTRA_DIR", "INTRA_REASON_ENTRY", "INTRA_REASON_EXIT",
        "SIG_POS", "POS_ENTRY", "POS_EXIT", "POS_DIR", "POS_REASON_ENTRY", "POS_REASON_EXIT",
