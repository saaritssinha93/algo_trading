...
def detect_signals_in_memory(ticker: str, df_today: pd.DataFrame) -> list:
    ...
    if adx_val <= 42:  # Maintain ADX threshold for general momentum
        return signals_detected
    ...
    if trend_type == "Bullish":
        conditions = {
            "Pullback": (
                (df_today["low"] >= df_today["VWAP"] * 0.98)  # Loosened proximity to VWAP
                ...
                & (df_today["ADX"] > 50)  # Reduced ADX threshold for pullbacks
            ),
            "Breakout": (
        ...
        conditions = {
            "Pullback": (
                (df_today["high"] <= df_today["VWAP"] * 1.02)  # Loosened proximity to VWAP
                & (df_today["close"] < df_today["open"])
                & (df_today["MACD"] < df_today["Signal_Line"])
...
def create_papertrade_file(input_file, output_file, last_trading_day):
    ...
    end_time   = india_tz.localize(datetime.combine(last_trading_day, datetime_time(14,10)))
