...
SIGNALS_DB = "generated_signals_historical1.json"
...
def is_ichimoku_bullish(df: pd.DataFrame, idx: int, tol: float = 0.005) -> bool:
    """
...
def is_ichimoku_bearish(df: pd.DataFrame, idx: int, tol: float = 0.005) -> bool:
    """
...
def check_bullish_breakout(
    ...
    is_ich_bull: bool,
    #sq_on: bool,
    # Possibly pass is_squeeze_on or other checks if you need them
) -> bool:
    """
    Returns True if the bar meets the "Bullish Breakout" criteria.
    """
    # Example sub-check for ADX range (strict breakout ADX >= 35, for example).
    # Adjust to your actual function logic if needed.
    def valid_adx_for_breakout(adx):
        return (adx >= 30) and (adx <= 45)

    # Example volume multiplier for breakout
    VOLUME_MULTIPLIER_BREAKOUT = 3.0

    # The logic below is taken from your original 'bullish_breakout' condition
    # (with the addition of the 'slope_okay_for_bullish' boolean).
    return (
        (2.0 <= daily_change <= 5.0)
        and valid_adx_for_breakout(adx_val)
        and (price >= 0.998 * high_10)
        ...
        and is_ich_bull
        #and sq_on
        # If you want a bullish range breakout pattern:
        # (is_bullish_range_breakout(...) or other similar price-action function)
    )
...
def check_bullish_pullback(
    ...
    """
    Returns True if the bar meets the "Bullish Pullback" criteria.
    """
    def valid_adx_for_pullback(adx):
        return (adx >= 25) and (adx <= 45)

    # The logic below is taken from your original 'bullish_pullback' condition.
    return (
        (daily_change >= -1.0)
        ...
        and (stoch_diff > 0)
        and slope_okay_for_bullish
        and (pullback_pct >= 0.02)   # e.g. at least 2.5–3% off recent high
    )
...
def check_bearish_breakdown(
    ...
    """
    Returns True if the bar meets the "Bearish Breakdown" criteria.
    """
    def valid_adx_for_breakout(adx):
        return (adx >= 30) and (adx <= 45)

    VOLUME_MULTIPLIER_BREAKOUT = 3.00

    return (
        (-5.0 <= daily_change <= -2.0)
        and valid_adx_for_breakout(adx_val)
        and (price <= 1.002 * low_10)
        and (vol >= VOLUME_MULTIPLIER_BREAKOUT * roll_vol)
        ...
        and is_ich_bear
        #and sq_release
        # If you want a bearish range breakdown pattern:
        # (is_bearish_range_breakdown(...) or other similar price-action function)
    )
...
def check_bearish_pullback(
    ...
    """
    Returns True if the bar meets the "Bearish Pullback" criteria.
    """
    def valid_adx_for_pullback(adx):
        return (adx >= 25) and (adx <= 45)
    ...
    return (
        ...
        and (stoch_diff < 0)
        and slope_okay_for_bearish
        and (bounce_pct >= 0.02)  # e.g. at least 2.5–3% bounce from recent low
    )
...
def detect_signals_in_memory(
    ...
    """
    This version checks all four strategies (Bullish Breakout, Bullish Pullback,
    Bearish Breakdown, Bearish Pullback) independently in 'if' statements 
    so that one bar can produce multiple signals if conditions are met.
    """
    signals_detected = []
    ...
    # 4) Create rolling columns (example: 10-bar rolling)
    rolling_window = 10
    ...
    # Helper checks (use or modify as needed)
    ...
    # 6) Loop over the rows we want to detect signals on
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        cidx = match.index[0]
        # We'll collect potential signals for this bar in a list 
        # so we can add them all if multiple conditions pass.
...
def is_bullish_breakout_consolidation(
    df, 
    idx, 
    ...
    """
    Slightly relaxed Bullish Consolidation Breakout Strategy:
      - A consolidation period is defined over the previous `consolidation_period` bars where:
            (highest_high - lowest_low) / average_close < range_threshold.
      - A bullish breakout is signaled if:
          1) The current candle is bullish (close > open).
          2) Its real body is at least min_body_ratio (default 25%) of its total range.
          3) It closes at least breakout_buffer (default 1%) above the consolidation period's highest high.
          4) Its volume is at least `volume_multiplier` times the average volume during consolidation.
    """
    if idx < consolidation_period or idx >= len(df):
        return False

    # Extract consolidation data (previous consolidation_period bars)
    consolidation_data = df.iloc[idx - consolidation_period: idx]
    consolidation_high = consolidation_data['high'].max()
    consolidation_low = consolidation_data['low'].min()
    average_close = consolidation_data['close'].mean()

    # Check for a narrow-range consolidation.
    if (consolidation_high - consolidation_low) / average_close > range_threshold:
        return False
    ...
    # 2) Ensure the candle's real body is significant.
    total_range = row['high'] - row['low']
    if total_range <= 0:
        return False
    body = row['close'] - row['open']
    if body < min_body_ratio * total_range:
        return False

    # 3) The breakout should be decisive: close must exceed consolidation high by breakout_buffer%.
    if row['close'] <= consolidation_high * (1 + breakout_buffer):
        return False

    # 4) Check volume condition (if volume data exists).
    if 'volume' in df.columns:
        average_volume = consolidation_data['volume'].mean()
        if row['volume'] < volume_multiplier * average_volume:
            return False

    return True


def is_bearish_breakdown_consolidation(
    df, 
    idx, 
    consolidation_period=10, 
    volume_multiplier=1.5, 
    ...
    """
    Slightly relaxed Bearish Consolidation Breakdown Strategy:
      - A consolidation period is defined over the previous `consolidation_period` bars where:
            (highest_high - lowest_low) / average_close < range_threshold.
      - A bearish breakdown is signaled if:
          1) The current candle is bearish (close < open).
          2) Its real body is at least min_body_ratio (default 25%) of its total range.
          3) It closes at least breakdown_buffer (default 1%) below the consolidation period's lowest low.
          4) Its volume is at least `volume_multiplier` times the average volume during consolidation.
    """
    if idx < consolidation_period or idx >= len(df):
        return False
    ...
    # Check for a narrow consolidation.
    if (consolidation_high - consolidation_low) / average_close > range_threshold:
        return False
    ...
    # 3) The breakdown should be decisive: close must be below consolidation low by breakdown_buffer%.
    if row['close'] >= consolidation_low * (1 - breakdown_buffer):
        return False
...
def is_bearish_breakdown_price_action(df, idx) -> bool:
    """
    Even more relaxed bearish breakdown:
      1) Candle is bearish (close < open).
      2) Real body >= 20% of total range.
      3) Lower wick <= 60% of total range.
      4) Close below the 3-bar low.
    """
    # Need at least 3 candles of history for the pivot and a valid index
    if idx < 2 or idx >= len(df):
        return False

    row = df.iloc[idx]
    
    # 1) Candle must be bearish
    if row['close'] >= row['open']:
        return False

    # 2) Candle body >= 20% of total range
    total_range = row['high'] - row['low']
    if total_range <= 0:
        return False
    body = row['open'] - row['close']  # since close < open
    if body < 0.20 * total_range:
        return False

    # 3) Lower wick <= 60% of total range
    lower_wick = row['close'] - row['low']
    if lower_wick > 0.60 * total_range:
        return False

    # 4) Must close below the 3-bar low
...
def is_bullish_breakout_price_action(df, idx) -> bool:
    """
    Even more relaxed bullish breakout:
      1) Candle is bullish (close > open).
      2) Real body >= 20% of total range.
      3) Upper wick <= 60% of total range.
      4) Close above the 3-bar high.
    """
    # Need at least 3 candles of history for the pivot and a valid index
    if idx < 2 or idx >= len(df):
        return False
    ...
    if upper_wick > 0.60 * total_range:
        return False

    # 4) Must close above the 3-bar high
...
def is_bullish_pullback_price_action(df, idx, daily_change) -> bool:
    """
    A stricter bullish pullback pattern:
      1) Candle is bullish (close > open).
      2) Body >= 30% of total range (instead of 20%).
      3) Lower wick >= 1% of total range; upper wick <= 85% of total range (tighter than 90%).
      4) Candle low is within 5% of recent 10-bar low (instead of 10%).
      5) daily_change >= +0.5% (instead of +0.25%).
      6) Must close >= 95% of the 10-SMA and 50-SMA (instead of 92%).
      7) Volume >= 80% of the 10-bar average (instead of 50%).
      8) Must have at least 10 bars of data prior (for the rolling checks).
    """
    ...
    # Lower wick >= 1% of range
    if lower_wick < 0.01 * range_:
        return False

    # Upper wick <= 85% of range
    if upper_wick > 0.85 * range_:
        return False

    # Need at least 10 bars for rolling
    if idx < 9:
        return False
    ...
    # 4) Low must be within 5% of the 10-bar low
    ...
    # 8) Must also be >= 95% of the 50-SMA
    if idx >= 49:
...
def is_bearish_pullback_price_action(df, idx, daily_change) -> bool:
    """
    A stricter bearish pullback pattern:
      1) Candle is bearish (close < open).
      2) Body >= 30% of total range (instead of 20%).
      3) Upper wick >= 1% of total range; lower wick <= 85% of total range.
      4) Candle high is within 5% of recent 10-bar high (instead of 10%).
      5) daily_change <= -0.5% (instead of -0.25%).
      6) Must close <= 105% of the 10-SMA and 50-SMA (instead of 108%).
      7) Volume >= 80% of the 10-bar average (instead of 50%).
      8) Must have at least 10 bars prior for rolling checks.
    """
    if idx < 0 or idx >= len(df):
        return False
    ...
    # Upper wick >= 1% of range
    if upper_wick < 0.01 * range_:
        return False

    # Lower wick <= 85% of range
    if lower_wick > 0.85 * range_:
        return False
    ...
    # 5) High must be within 5% of recent 10-bar high
    ...
    # 7) Candle open must not be more than 8% below candle's high (tighter than 10%)
    if row['open'] < 0.92 * row['high']:
        return False

    # 8) Check proximity to the 10-SMA (<= 105%)
    ma10 = df['close'].rolling(window=10, min_periods=10).mean().iloc[idx]
    if row['close'] > 1.05 * ma10:
        return False

    # 9) Volume >= 80% of 10-bar average
    if 'volume' in df.columns and idx >= 9:
        avg_vol = df['volume'].rolling(window=10, min_periods=10).mean().iloc[idx]
        if avg_vol > 0 and row['volume'] < 0.80 * avg_vol:
            return False

    # 10) Must be <= 105% of the 50-SMA
    if idx >= 49:
        ma50 = df['close'].rolling(window=50, min_periods=50).mean().iloc[idx]
