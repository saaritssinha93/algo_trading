...
# Capital sizing
CAPITAL_PER_SIGNAL_RS: float = 10000.0
...
def _build_signals_positional_template(ticker: str, P: dict) -> list[dict]:
    ...
    # Day change booster now uses HOURLY bar-change (Bar_Change_H if present, else computed)
    bar_chg = pd.to_numeric(_col("Bar_Change_H"), errors="coerce").fillna(0.0)
    day_change_min = float(P.get("day_change_min", 1.3))
    day_boost = (bar_chg >= day_change_min).fillna(False)
