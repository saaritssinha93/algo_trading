...
def detect_signals_in_memory(ticker: str, df_today: pd.DataFrame, existing_signal_ids: set) -> list:
    ...
    - df_today (pd.DataFrame): DataFrame containing today's data for the ticker (only the latest row).
    ...
    # -- 2) Check if ADX indicates momentum (e.g., > 25 for general momentum)
    ...
    # Define Pullback & Breakout/Breakdown conditions
    ...
    if trend_type == "Bullish":
        conditions = {
            "Pullback": (
                (latest_row["low"] >= latest_row["VWAP"] * 0.98)
                & (latest_row["close"] > latest_row["open"])
                & (latest_row["MACD"] > latest_row["Signal_Line"])
                & (latest_row["MACD"] > MACD_BULLISH_OFFSET)
                & (latest_row["close"] > latest_row["20_SMA"])
                & (latest_row["Stochastic"] < 50)
                & (latest_row["Stochastic"].diff() > 0)
                & (latest_row["ADX"] > 25)
            ),
            "Breakout": (
                (latest_row["close"] > latest_row["high"] * 0.99)
                & (latest_row["volume"] > volume_multiplier * latest_row["volume"].rolling(window=rolling_window).mean())
                & (latest_row["MACD"] > latest_row["Signal_Line"])
                & (latest_row["MACD"] > MACD_BULLISH_OFFSET)
                & (latest_row["close"] > latest_row["20_SMA"])
                & (latest_row["Stochastic"] > 60)
                & (latest_row["Stochastic"].diff() > 0)
                & (latest_row["ADX"] > 37)
            ),
        }
        ...
        conditions = {
            "Pullback": (
                (latest_row["high"] <= latest_row["VWAP"] * 1.02)
                & (latest_row["close"] < latest_row["open"])
                & (latest_row["MACD"] < latest_row["Signal_Line"])
                & (latest_row["MACD"] < MACD_BEARISH_OFFSET)
                & (latest_row["close"] < latest_row["20_SMA"])
                & (latest_row["Stochastic"] > 50)
                & (latest_row["Stochastic"].diff() < 0)
                & (latest_row["ADX"] > 25)
            ...
            "Breakdown": (
                (latest_row["close"] < latest_row["low"] * 1.01)
                & (latest_row["volume"] > volume_multiplier * latest_row["volume"].rolling(window=rolling_window).mean())
                & (latest_row["MACD"] < latest_row["Signal_Line"])
                & (latest_row["MACD"] < MACD_BEARISH_OFFSET)
                & (latest_row["close"] < latest_row["20_SMA"])
                & (latest_row["Stochastic"] < 40)
                & (latest_row["Stochastic"].diff() < 0)
                & (latest_row["ADX"] > 37)
            ),
        }
    ...
    for entry_type, cond in conditions.items():
        if cond:
            ...
            signals_detected.append({
                ...
                "Price": round(latest_row.get("close", 0), 2),
                "Daily Change %": round(daily_change, 2),
                "VWAP": round(latest_row.get("VWAP", 0), 2),
...
def find_price_action_entries_for_today(last_trading_day):
    ...
    for file_path in tqdm(csv_files, desc="Processing CSV Files", unit="file"):
        ...
        try:
            ...
            # Detect signals only on the latest data point
            if not df_today.empty:
                latest_df = df_today.iloc[-1:].copy()  # Select the last row as a DataFrame
...
def main():
    ...
    entries_filename = f"price_action_entries_15min_l_{today_str}.csv"
    papertrade_filename = f"papertrade_l_{today_str}.csv"
