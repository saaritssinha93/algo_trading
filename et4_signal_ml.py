...
This script processes user-specified dates by:
  ...
  5. For each strategy, it applies a per-ticker logistic regression model (named:
     "lr_model_{TICKER}_{strategy}.pkl") to filter candidate signals.
...
LR_MODEL_DIR = "lr_model"  # Directory where lr_model_{TICKER}_{strategy}.pkl reside
...
# -------------------------------------------------------------------
# 6) LR Model Loading for Each Strategy
# -------------------------------------------------------------------
import os
import pickle
import numpy as np
...
############################
# 1) Strategy-Aware Model Loading
############################

def load_lr_model_strategy(ticker: str, strategy: str):
    """
    Loads the logistic regression model from "lr_model_{ticker}_{strategy}.pkl".
    If that file does not exist, tries "lr_model_{ticker}.pkl".
    If both are missing or loading fails, returns None.
    """
    # 1) Try to load the per-strategy model
    strategy_model_path = os.path.join(LR_MODEL_DIR, f"lr_model_{ticker}_{strategy}.pkl")
    ...
    # 2) Fall back to the general ticker model
    general_model_path = os.path.join(LR_MODEL_DIR, f"lr_model_{ticker}.pkl")
    ...
    # 3) If both attempts fail, return None
...
def load_lr_model_strategy(ticker: str, strategy: str):
    ...
    should run conditions-only (no LR filter).
    """
    import os
    import pickle
...
def extract_features(row):
    """
    Features used by the LR models.
    """
    return [
        row.get("close", 0),
        row.get("volume", 0),
...
def predict_signal_probability(model, row):
    features = np.array(extract_features(row)).reshape(1, -1)
    return model.predict_proba(features)[0, 1]

###########################################################
# 8) detect_signals_in_memory (Merged Detection Logic + LR Models)
...
def detect_signals_in_memory(
    ...
    Each strategy is further filtered by a logistic regression model:
      - Per-strategy: lr_model_{ticker}_{strategy}.pkl, 
      - or fallback: lr_model_{ticker}.pkl
    If neither file exists for that strategy, no LR filter is applied.
    ...
    # -------------------------------------------------
    # (1) Load LR Models for Each Strategy or Fallback
    # -------------------------------------------------
    # We assume you have the function load_lr_model_strategy(ticker, strategy)
    # that tries "lr_model_{ticker}_{strategy}.pkl" then fallback "lr_model_{ticker}.pkl"
    # and returns None if neither is found.
    bull_breakout_model  = load_lr_model_strategy(ticker, "bull_breakout")
    bull_pullback_model  = load_lr_model_strategy(ticker, "bull_pullback")
    bear_breakdown_model = load_lr_model_strategy(ticker, "bear_breakdown")
    bear_pullback_model  = load_lr_model_strategy(ticker, "bear_pullback")
    ...
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        # ----------------------------------------------------
        #  if-elif-elif-elif structure for each strategy
        # ----------------------------------------------------
        ...
        if (
            ...
            if signal_id not in existing_signal_ids:
                # Apply LR Model for BULL_BREAKOUT
                # (If bull_breakout_model is None, skip LR filter)
                if bull_breakout_model is not None:
                    lr_prob = predict_signal_probability(bull_breakout_model, row_c)
                    if lr_prob < lr_threshold:
                        # LR probability not enough => skip
                        continue
                # If model is None => no filter => pass
        ...
        if check_bullish_pullback(daily_change, adx_val, macd_above_now,
            ...
            if signal_id not in existing_signal_ids:
                # Apply LR Model for BULL_PULLBACK
                if bull_pullback_model is not None:
                    lr_prob = predict_signal_probability(bull_pullback_model, row_c)
                    if lr_prob < lr_threshold:
                        continue
                signals_for_this_bar.append({
                    'Ticker': ticker,
        ...
        if (
            ...
            if signal_id not in existing_signal_ids:
                # Apply LR Model for BEAR_BREAKDOWN
                if bear_breakdown_model is not None:
                    lr_prob = predict_signal_probability(bear_breakdown_model, row_c)
                    if lr_prob < lr_threshold:
                        continue
                signals_for_this_bar.append({
                    'Ticker': ticker,
        ...
        if check_bearish_pullback(daily_change, adx_val, macd_above_now,
            ...
            if signal_id not in existing_signal_ids:
                # Apply LR Model for BEAR_PULLBACK
                if bear_pullback_model is not None:
                    lr_prob = predict_signal_probability(bear_pullback_model, row_c)
                    if lr_prob < lr_threshold:
                        continue
                signals_for_this_bar.append({
                    'Ticker': ticker,
...
def find_price_action_entries_for_day(trading_day: date, papertrade_file: str):
    ...
    For each ticker in the main_indicators_test folder:
       ...
       2) For each of the 4 strategies, loads "lr_model_{TICKER}_{strategy}.pkl"
