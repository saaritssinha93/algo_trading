...
Notes (rewrite, same parameters):
- Robust CSV loader:
  • Coalesces any date-like columns (date, date.1, timestamp…) into a single tz-aware IST 'date'
  • Ensures required columns exist (adds if missing), and coerces numerics
- Keeps your exact ENTRY_PARAMS_DEFAULT thresholds/values
- Optional lightweight diagnostics helpers (off by default)

Outputs:
- entries_2pct_results.csv
- fitted_indicator_ranges_by_ticker.json/.csv
"""
...
def _coalesce_date_columns(df: pd.DataFrame) -> pd.Series:
    ...
    # Prefer 'date', then other date.*, then 'timestamp'
    def _rank(c):
        cl = c.lower()
    ...
    # row-wise first non-null (avoid combine_first deprecation warnings)
    dt_series = parsed[0]
    for s in parsed[1:]:
...
def _ensure_columns(df: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
    """Ensure required columns exist; if missing, add NaNs."""
    for c in cols:
...
def _coerce_numeric(df: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
    """Coerce listed columns to numeric where possible."""
    for c in cols:
        if c in df.columns:
...
def _safe_read_csv(path: str, usecols: Optional[List[str]] = None) -> pd.DataFrame:
    """Robust reader: coalesces date columns, enforces IST, date window, required cols, numerics."""
    ...
    try:
        ...
        # Fallback to a single 'date' parse if present
    ...
    # Drop original date-like columns and insert canonical one
    ...
    # Ensure required columns exist, then coerce numerics
...
# ============================
# ENTRY LOGIC (Step 1) — unchanged thresholds/values
# ============================
...
def _row_is_bear_entry(df: pd.DataFrame, i: int, p: Dict[str, Any]) -> bool:
    ...
    if r["Stoch_%D"] > p["stochd_bear_max"]:
        return False
    # vwap_bear_min == 0 means "no VWAP distance check", exactly as before
    ...
    for _, r in fwd.iterrows():
        if side == "bull":
            if stop is not None and r["low"] <= stop:
                hit_target_path = False; exit_px = stop; exit_time = r["date"]; break
            if r["high"] >= target:
                hit_target_path = True;  exit_px = target; exit_time = r["date"]; break
        else:
...
def generate_intraday_entries_for_file(ticker: str,
    ...
    # Derived features (computed once on full DF)
    eps = 1e-9
...
# ============================
# STEP 2b: FIT PER-TICKER RANGES (winners-only, shrinkage)
# ============================
...
def update_fitted_ranges_by_ticker_from_entries_csv(entries_csv_path: str,
    ...
    # Avoid double-read of entire file twice
    head = pd.read_csv(entries_csv_path, nrows=1)
    parse_cols = [time_col] if time_col in head.columns else None
    df = pd.read_csv(entries_csv_path, parse_dates=parse_cols)
    if df.empty:
    ...
    if last_epoch > 0 and time_col in df.columns:
...
if __name__ == "__main__":
    # Step 1: scan & simulate 2% target (parameters unchanged)
    entries = run_step1_scan(INDICATORS_DIR, ENTRY_PARAMS_DEFAULT, out_csv="entries_2pct_results.csv")

    if not entries.empty:
        # FULL recompute (winners-only with shrinkage to global) — same values
        fit_indicator_ranges_by_ticker(entries,
                                       out_json="fitted_indicator_ranges_by_ticker.json",
                                       out_csv="fitted_indicator_ranges_by_ticker.csv",
