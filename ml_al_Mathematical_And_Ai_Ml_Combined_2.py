...
This version:
- Uses explicit START_DATE and END_DATE (no rolling 90d).
- Trains per-ticker KNN regressors with group-wise CV by day.
- Produces out-of-fold (OOF) predictions and per-ticker isotonic calibrators.
- Fits winners-only per-ticker indicator ranges (multiple quantile bands).
- Writes a stamped manifest so the live detector can load a weekly-frozen bundle.

Notes:
- Set START_DATE / END_DATE below to define the training window.
- “Embargo” (if you see it mentioned in other scripts) means keeping a gap
  between the last training date and the date you trade on, to avoid leakage.
  Here, you control the window explicitly with END_DATE.
"""
...
# === Explicit training window (EDIT THESE) ===
START_DATE = "2025-07-01"   # inclusive
END_DATE   = "2025-08-28"   # inclusive
...
# Default paths for Step-2 (stamped)
...
def run_ai_ml_pipeline(data_path: str = DATA_PATH_DEFAULT,
    ...
    # --- Calibrate probabilities per ticker using OOF (isotonic) ---
    calibrators = {}
    try:
    ...
    if save_models:
        joblib.dump({"models": models, "features": feats, "calibrators": calibrators}, models_path)
...
if __name__ == "__main__":
    ...
    if not entries.empty:
        ...
        run_step2_from_entries_csv(
            ...
            out_json=f"models/fitted_indicator_ranges_by_ticker10_90_{STAMP}.json",
            out_csv=f"models/fitted_indicator_ranges_by_ticker10_90_{STAMP}.csv"
        )
        run_step2_from_entries_csv(
            entries_csv=DATA_PATH_DEFAULT,
            q_low=0.15, q_high=0.85, shrink_k=200,
            out_json=f"models/fitted_indicator_ranges_by_ticker15_85_{STAMP}.json",
            out_csv=f"models/fitted_indicator_ranges_by_ticker15_85_{STAMP}.csv"
        )
        run_step2_from_entries_csv(
            entries_csv=DATA_PATH_DEFAULT,
            q_low=0.20, q_high=0.80, shrink_k=200,
            out_json=f"models/fitted_indicator_ranges_by_ticker20_80_{STAMP}.json",
            out_csv=f"models/fitted_indicator_ranges_by_ticker20_80_{STAMP}.csv"
        )
        run_step2_from_entries_csv(
            entries_csv=DATA_PATH_DEFAULT,
            q_low=0.25, q_high=0.75, shrink_k=200,
            out_json=f"models/fitted_indicator_ranges_by_ticker25_75_{STAMP}.json",
            out_csv=f"models/fitted_indicator_ranges_by_ticker25_75_{STAMP}.csv"
        )
