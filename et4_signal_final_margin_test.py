# -*- coding: utf-8 -*-
"""
Created on Fri Mar  7 12:39:21 2025
...
# -*- coding: utf-8 -*-
"""
Merged/Unified "Code 2" with "Code 1"-style logic:
- Reads all bars after 9:15 AM from each *_main_indicators.csv
- Uses the existing 'Daily_Change' column in each CSV instead of recalculating it
- Performs advanced breakout/breakdown checks, Ichimoku, TTM Squeeze, etc.
- Marks signals in main CSV (CalcFlag='Yes'), just like Code 1
- Only the *first* signal per ticker each day is appended to papertrade CSV
- Runs continuously every 20 seconds

NOTE: We have removed the step that calculates the Daily_Change from previous day's close;
      now we rely solely on the 'Daily_Change' column already present in each CSV.
"""
...
# This JSON tracks which signal IDs have already been generated
SIGNALS_DB = "generated_signals_unified2.json"

# Example: if you want to skip certain holidays, populate here (as date objects)
market_holidays = [
    datetime(2025, 1, 26).date(),   # Republic Day
    datetime(2025, 2, 26).date(),   # Maha Shivratri
    datetime(2025, 3, 14).date(),    # Holi
    datetime(2025, 4, 11).date(),   # Eid ul-Fitr (approx.)
    datetime(2025, 8, 15).date(),   # Independence Day
    datetime(2025, 10, 2).date(),   # Gandhi Jayanti
    datetime(2025, 10, 17).date(),  # Dussehra (approx.)
    datetime(2025, 11, 5).date(),   # Diwali (approx.)
...
###########################################################
def detect_signals_in_memory(ticker: str, df_for_rolling: pd.DataFrame,
                             df_for_detection: pd.DataFrame, existing_signal_ids: set) -> list:
    signals_detected = []
...
def append_signals_to_papertrade(signals_list, output_file, trading_day):
    ...
    # Compute base quantity and adjust by margin allowed
    for idx, row in df_entries.iterrows():
        ...
        if price > 0:
            ...
            base_qty = int(20000 / price) if price != 0 else 1
            
            # Retrieve margin allowed from the row and convert to float (default to 1.0 if missing)
            margin_str = row.get("Margin allowed", "")
            try:
                margin_value = float(margin_str) if margin_str != "" else 1.0
            except:
                margin_value = 1.0
            
            # Multiply the base quantity by the margin allowed
            qty = int(base_qty * margin_value)
            total_val = qty * price
    ...
    # === NEW CODE: Add Margin allowed for each ticker from mis_sheet.csv ===
    try:
        mis_df = pd.read_csv("mis_sheet.csv")
        # Assume mis_sheet.csv has two columns: first column is ticker and second is "Margin allowed"
        mis_df = mis_df.iloc[:, :2]
        mis_df.columns = ["Ticker", "Margin allowed"]
        margin_dict = mis_df.set_index("Ticker")["Margin allowed"].to_dict()
    except Exception as e:
        logging.error("Error reading mis_sheet.csv: " + str(e))
        margin_dict = {}
    df_entries["Margin allowed"] = df_entries["Ticker"].apply(lambda x: margin_dict.get(x, ""))
    # === END NEW CODE ===
...
###########################################################
# 11) find_price_action_entries_for_today (Detection for Today)
###########################################################
def find_price_action_entries_for_today(trading_day, papertrade_file):
    start_dt = india_tz.localize(datetime.combine(trading_day, dt_time(9, 15)))
    now_ist = datetime.now(india_tz)
    if now_ist.date() == trading_day:
        end_dt = now_ist
    else:
        end_dt = india_tz.localize(datetime.combine(trading_day, dt_time(14, 45)))
    ...
    def process_file(file_path):
        ...
        if df_full.empty:
            ...
            print(f"[ANALYZED] {ticker} => used last 0 of 0 rows after 9:15, reason: No data")
            return []
        df_full_915 = df_full[df_full["date"] >= start_dt]
        count_full_915 = len(df_full_915)
        df_today = df_full_915[df_full_915["date"] <= end_dt].copy()
        count_df_today = len(df_today)
        if df_today.empty:
            reason = "No rows after 9:15"
            with lock:
                summary_info[ticker] = reason
            print(f"[ANALYZED] {ticker} => used last {count_df_today} of {count_full_915} rows after 9:15, reason: {reason}")
            return []
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for fut in tqdm(as_completed(future_to_path), total=len(future_to_path), desc="Processing CSVs"):
    ...
    for t, stat in summary_info.items():
        print(f"{t}: {stat}")
    if not all_signals:
        logging.info("No new signals overall.")
        return pd.DataFrame()
...
def main():
    try:
        ...
        papertrade_file = f"papertrade2_{today_str}.csv"
        signals_df = find_price_action_entries_for_today(trading_day, papertrade_file)
        if signals_df.empty:
            logging.info(f"No signals found for {today_str}.")
        else:
            logging.info(f"Detected {len(signals_df)} signals for {today_str}.")
    except Exception as e:
        logging.error(f"Error in main execution: {e}")
        logging.error(traceback.format_exc())

###########################################################
# 13) Continuous Run Every 20 Seconds
###########################################################
...
def append_signals_to_papertrade(signals_list, output_file, trading_day):
    """
    Appends new signals to papertrade CSV, ensuring "Quantity Margin" column is retained.
    """

    if not signals_list:
        return

    # 1) Convert signals into a DataFrame
    df_entries = pd.DataFrame(signals_list)
    if df_entries.empty:
        return

    # 2) Normalize 'date' and filter rows for the given trading day
    df_entries = normalize_time(df_entries, tz='Asia/Kolkata')
    df_entries.sort_values('date', inplace=True)
    start_dt = india_tz.localize(datetime.combine(trading_day, datetime.min.time()))
    end_dt   = india_tz.localize(datetime.combine(trading_day, datetime.max.time()))
    ...
    # 3) Ensure these columns exist in df_entries
    columns_needed = ["Target Price", "Quantity", "Quantity Margin", "Total value"]
    for col in columns_needed:
        if col not in df_entries.columns:
            df_entries[col] = ""

    # 4) Load the MIS sheet and attach "Margin allowed" to df_entries
    try:
        mis_df = pd.read_csv("mis_sheet.csv")
        # mis_sheet.csv must have two columns: Ticker, Margin allowed
        mis_df = mis_df.iloc[:, :2]
        mis_df.columns = ["Ticker", "Margin allowed"]
        margin_dict = mis_df.set_index("Ticker")["Margin allowed"].to_dict()
    except Exception as e:
    ...
    # Default to '1x' if no margin is found
    df_entries["Margin allowed"] = df_entries["Ticker"].apply(
        lambda x: margin_dict.get(x, "1x")
    )

    # 5) Compute final quantity using the updated "Margin allowed"
    for idx, row in df_entries.iterrows():
        price = float(row.get("Price", 0) or 0)
        trend = row.get("Trend Type", "")
        ...
        # Base quantity calculation, e.g. 20,000 / price
        base_qty = int(20000 / price) if price != 0 else 1

        margin_str = row.get("Margin allowed", "1x").strip()
        # Remove trailing 'x' or 'X' if present
        if margin_str.lower().endswith("x"):
            margin_str = margin_str[:-1]
        try:
            margin_val = float(margin_str)
        except:
            margin_val = 1.0

        # Final quantity after applying margin
        final_qty = int(base_qty * margin_val)
        total_val = final_qty * price

        df_entries.at[idx, "Target Price"]      = round(target, 2)
        df_entries.at[idx, "Quantity"]          = base_qty
        df_entries.at[idx, "Quantity Margin"]   = final_qty
        df_entries.at[idx, "Total value"]       = round(total_val, 2)

    # Fill time column if missing
    if 'time' not in df_entries.columns:
        df_entries['time'] = datetime.now(india_tz).strftime('%H:%M:%S')
    else:
        blank_mask = (df_entries['time'].isna()) | (df_entries['time'] == "")
    ...
    # 6) Merge with existing papertrade CSV, overriding old rows
    lock_path = output_file + ".lock"
    lock = FileLock(lock_path, timeout=10)
    ...
    with lock:
        if os.path.exists(output_file):
            ...
            # Ensure existing also has the needed columns
            for col in columns_needed:
                if col not in existing.columns:
                    existing[col] = ""
            if "Margin allowed" not in existing.columns:
                existing["Margin allowed"] = ""

            # Merge existing + new, so that new entries override old
            existing["Day"] = existing["date"].dt.date
            df_entries["Day"] = df_entries["date"].dt.date

            combined = pd.concat([existing, df_entries], ignore_index=True)
            # Keep the last occurrence of each (Ticker, Day)
            combined.sort_values(by=["Ticker","date"], inplace=True)
            combined.drop_duplicates(subset=["Ticker","Day"], keep="last", inplace=True)

            # Clean up
            combined.drop(columns=["Day"], errors="ignore", inplace=True)
            combined.sort_values("date", inplace=True)
            combined.to_csv(output_file, index=False)
            print(f"[Papertrade] Updated => {output_file} with {len(df_entries)} row(s).")

        else:
            # No existing file => write df_entries as new
            df_entries.to_csv(output_file, index=False)
            print(f"[Papertrade] Created => {output_file} with {len(df_entries)} rows.")
