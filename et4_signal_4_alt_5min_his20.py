...
def detect_signals_in_memory(
    ...
    for _, row_detect in df_for_detection.iterrows():
        ...
        if (
            daily_change > 2
            and has_intra_streak(cidx, n, t, bullish=True)
            ...
            and stoch_val > 60
            and stoch_diff > 0
            and stoch_bull_ok(combined, cidx, zone=30)   # <-- df then index
            and price > vwap
        ):
        ...
        if (
            daily_change < -2
            and has_intra_streak(cidx, n, t, bullish=False)
            and is_adx_increasing(combined, cidx, bars_ago=1)
            and 25 < adx_val < 45
            ...
            and stoch_val < 40
            and stoch_diff < 0
            and stoch_bear_ok(combined, cidx, zone=70)   # <-- df then index
            and price < vwap
        ):
