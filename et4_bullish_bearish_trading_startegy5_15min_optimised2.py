# -*- coding: utf-8 -*-
"""
Created on Wed Dec 11 12:04:12 2024
...
def ensure_all_indicators(data, ticker):
    ...
    If any are missing, calculate and add them. Also ensure each row has a stable Signal_ID.
    ...
    Returns:
        pd.DataFrame: DataFrame with all required indicators and Signal_ID or None if failure.
    """
    required_columns = {
    ...
    # If there are missing indicators, recalculate them
    if missing_columns:
        ...
        try:
            ...
            # Recheck for missing columns after calculations
            missing_columns = required_columns - set(data.columns)
            if missing_columns:
    ...
    # After ensuring indicators, ensure Signal_ID is stable
    if 'Signal_ID' not in data.columns:
        # Assign a unique Signal_ID to each row if it doesn't exist
        # The Signal_ID is based on ticker and the UTC isoformat of the date
        data['Signal_ID'] = data.apply(
            lambda row: f"{ticker}-{row['date'].astimezone(pytz.UTC).isoformat()}",
            axis=1
        )
        logging.info(f"Assigned new Signal_ID for all rows of {ticker}.")
    else:
        # If Signal_ID already exists, do not modify it - it remains stable
        logging.debug(f"Signal_ID already present for {ticker}. Not modifying existing Signal_IDs.")
...
def apply_trend_conditions(data, daily_change, ticker, last_trading_day):
    ...
    try:
        ...
        if (
            daily_change > 1.25  # Increased from 1.0 to 1.5%
...
def main():
    ...
    end_time_naive = datetime.combine(now.date(), datetime_time(15, 30, 0))
