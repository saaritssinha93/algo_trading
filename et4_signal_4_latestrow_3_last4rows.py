...
Now also prints a summary of any tickers whose rows weren't analyzed.
...
file_handler = TimedRotatingFileHandler(
    "logs\\signal5.log", when="M", interval=30, backupCount=5, delay=True
)
...
def normalize_time(df, tz='Asia/Kolkata'):
    ...
    if df['date'].dt.tz is None:
        # If no timezone, assume UTC (adjust if your data is local time)
...
def find_price_action_entries_for_today(trading_day):
    ...
    Also tracks which tickers/rows are or aren't analyzed (analysis_info).
    """
    ...
    # Dictionary to store summary of analysis
    # e.g. analysis_info[ticker] = {
    #    'rows_after_930': <int>,
    #    'rows_used': <int>,
    #    'reason': <str if rows_used == 0>
    # }
    analysis_info = {}
    lock = threading.Lock()
    ...
    def process_file(file_path):
        ...
        if df_full.empty:
            ...
            with lock:
                analysis_info[ticker] = {
                    'rows_after_930': 0,
                    'rows_used': 0,
                    'reason': "File empty or not found"
                }
            print(f"[INFO] {ticker} => no data in CSV.")
            return []
        ...
        if df_today.empty:
            with lock:
                analysis_info[ticker] = {
                    'rows_after_930': 0,
                    'rows_used': 0,
                    'reason': "No data after 9:30 AM"
                }
            print(f"[INFO] {ticker} => no rows after 9:30.")
            return []
        ...
        with lock:
            analysis_info[ticker] = {
                ...
                'rows_used': rows_used,
                'reason': "" if rows_used > 0 else "No usable rows"
            }
    ...
    # 9) Print summary of analysis_info
    print("\n=== ANALYSIS SUMMARY ===")
    all_tickers = sorted(analysis_info.keys())
    for t in all_tickers:
        info = analysis_info[t]
        if info['rows_used'] == 0:
            print(f"[NOT ANALYZED] {t} => reason: {info['reason']}")
        else:
            unused = info['rows_after_930'] - info['rows_used']
            print(f"[ANALYZED] {t} => used {info['rows_used']} of {info['rows_after_930']} rows after 9:30 "
                  f"({unused} not used).")

    # 10) Convert all_signals to DataFrame, sort, and return
    if not all_signals:
