...
def detect_signals_in_memory(
    ...
    def valid_adx_for_breakout(adx_val: float) -> bool:
        ...
        return (not pd.isna(adx_val)) and (adx_val >= 25) and (adx_val <= 35)
    ...
    def slope_okay_for_bullish(row) -> bool:
        ...
        if pd.isna(slope_20) or pd.isna(sma_20) or sma_20 == 0:
            return False
        return (slope_20 > 0) and ((slope_20 / sma_20) >= 0.002)  # >= 0.1%
    ...
    def slope_okay_for_bearish(row) -> bool:
        ...
        if pd.isna(slope_20) or pd.isna(sma_20) or sma_20 == 0:
            return False
        return (slope_20 < 0) and ((abs(slope_20) / sma_20) >= 0.002)
    ...
    def is_adx_increasing(df: pd.DataFrame, current_idx: int, bars_ago: int = 1) -> bool:
        ...
        ending at `current_idx`. That is, for bars_ago=3, we require:
            ADX[current_idx - 3] < ADX[current_idx - 2] < ADX[current_idx - 1] < ADX[current_idx]

        Returns False if there is insufficient data or any ADX value is NaN.
        """
        # We need at least `bars_ago` bars behind `current_idx` to compare.
        start_idx = current_idx - bars_ago
        if start_idx < 0:
            return False  # Not enough data to check

        # Extract ADX values for the range [current_idx - bars_ago ... current_idx]
        adx_values = df.loc[start_idx:current_idx, 'ADX'].to_numpy()

        # If any are NaN, return False
        if any(pd.isna(val) for val in adx_values):
            return False

        # Check strictly increasing: adx_values[i] < adx_values[i+1] for all i
    ...
    def is_rsi_increasing(df: pd.DataFrame, current_idx: int, bars_ago: int = 1) -> bool:
        ...
        For example, if bars_ago == 3, then we require:
            RSI[current_idx - 3] < RSI[current_idx - 2] < RSI[current_idx - 1] < RSI[current_idx]
      
        Returns False if there is insufficient data or any RSI value is NaN.
        """
        start_idx = current_idx - bars_ago
        if start_idx < 0:
            return False  # Not enough data to check

        # Extract the RSI values for the specified interval.
        ...
        # Check that each value is strictly less than the next.
    ...
    def is_rsi_decreasing(df: pd.DataFrame, current_idx: int, bars_ago: int = 3) -> bool:
        ...
        # Check that each value is strictly greater than the next.
        for i in range(len(rsi_values) - 1):
            if rsi_values[i] <= rsi_values[i + 1]:
                return False
...
def is_bullish_breakout(df, idx, daily_change) -> bool:
    """
    Example logic for a bullish breakout:
        - Price near or above rolling high
        - Strong daily_change (>= +2%)
        - Volume above some rolling average
    """
    ...
    rolling_high = row['rolling_high_10']    # last 10-bar high
    avg_volume   = row['rolling_vol_10']       # 10-bar average volume

    return (
        (daily_change >= 2.0) and
        (row['close'] >= 0.99 * rolling_high) and
        (row['volume'] >= 2.0 * avg_volume)   # 2x the average volume
    )


def is_bearish_breakdown(df, idx, daily_change) -> bool:
    """
    Example logic for a bearish breakdown:
        - Price near or below rolling low
        - Strong negative daily_change (<= -2%)
    ...
    row = df.iloc[idx]
    rolling_low = row['rolling_low_10']
    avg_volume  = row['rolling_vol_10']

    return (
        (daily_change <= -2.0) and
        (row['close'] <= 1.01 * rolling_low) and
        (row['volume'] >= 2.0 * avg_volume)
    )


def is_bullish_pullback(df, idx, daily_change) -> bool:
    """
    Example logic for a bullish pullback:
        - Overall uptrend or slope > 0
        - daily_change is still somewhat positive or small (e.g. >= +0.5)
        - Price retraced to a moving average, then bounces back
    """
    if idx < 0 or idx >= len(df):
        return False

    row = df.iloc[idx]
    # For example, ensure price is above 20_SMA (bullish environment)
    if '20_SMA' not in row or pd.isna(row['20_SMA']):
        return False

    return (
        (daily_change >= 0.5) and
        (row['close'] >= 0.98 * row['20_SMA'])
    )


def is_bearish_pullback(df, idx, daily_change) -> bool:
    """
    Example logic for a bearish pullback:
        - Overall downtrend or slope < 0
        - daily_change is somewhat negative (e.g., <= -0.5)
        - Price popped up to near a moving average, then turned back down
    """
    if idx < 0 or idx >= len(df):
        return False
    ...
    return (
        (daily_change <= -0.5) and
        (row['close'] <= 1.02 * row['20_SMA'])
    )



    # 6) Parameter constants for stricter logic
    VOLUME_MULTIPLIER_BREAKOUT = 2.0     # higher volume requirement
    MACD_BULLISH_OFFSET_BREAKOUT = 2.0
    MACD_BULLISH_OFFSET_BREAKOUT_max = 20
    MACD_BULLISH_OFFSET_PULLBACK = 1
    MACD_BEARISH_OFFSET_BREAKOUT = -2.0
    ...
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        if row_trend == "Bullish":
            ...
            bullish_breakout = (
                ...
                and is_bullish_breakout(combined, cidx, daily_change)
            )
            ...
            bullish_pullback = (
                ...
                #and is_roc_positive(combined, cidx)
                and is_bullish_pullback(combined, cidx, daily_change)
            )
            ...
            bearish_breakdown = (
                ...
                #and (MACD_BEARISH_OFFSET_BREAKOUT_min <= macd_val <= MACD_BEARISH_OFFSET_BREAKOUT)
                and (stoch_val < 30)
                and (stoch_diff < 0)
                ...
                # NEW: *increasing* ADX check:
                and is_adx_increasing(combined, cidx, bars_ago=3)
                and is_rsi_decreasing(combined, cidx, bars_ago=3)
                # NEW: OBV and ROC check for bearish trend:
                and is_obv_decreasing(combined, cidx, bars_ago=3)
                and is_roc_negative(combined, cidx)
                and is_bearish_breakdown(combined, cidx, daily_change)
            )
            ...
            bearish_pullback = (
                ...
                #and is_roc_negative(combined, cidx)
                and is_bearish_pullback(combined, cidx, daily_change)
            )
...
if __name__ == "__main__":
    ...
    if not date_args:
        ...
        date_args = ["2025-01-27", "2025-01-28", "2025-01-29", "2025-01-30", "2025-01-31", "2025-02-03"]
        #date_args = ["2025-01-31"]
