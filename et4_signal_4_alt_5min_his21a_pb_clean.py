...
def detect_signals_in_memory(
    ...
    """
    Generate pullback entries:
      Bullish: VWAP touch (wick) + reclaim; trend up; mid-zone momentum; light volume confirm.
      Bearish: VWAP touch (wick) + reject; trend down; mid-zone momentum; light volume confirm.
    """
    ...
    # --- Minimal features actually used ---
    ...
    def find_cidx(ts):
        ...
        # first bar of that day if exists
        mask2 = (combined["_date_only"] == t.date())
        if mask2.any(): return int(combined.index[mask2][0])
        return None

    # --- Tunables (pullback behavior) ---
    TOUCH_BUF   = 0.0015   # 0.15% proximity to VWAP for wick "touch"
    RECLAIM_BUF = 0.0010   # 0.10% close above/below VWAP for confirm
    VOL_MIN_X   = 1.2      # >= 1.2× rolling(10) volume
    WICK_MIN    = 0.25     # prefer visible tails
    PD_SAFETY   = 0.0030   # avoid trading into PDH/PDL within 0.30%
    ...
    for _, row_det in df_for_detection.iterrows():
        ...
        if pd.isna(daily_change): continue
        ...
        vol     = row_c.get("volume", np.nan)
        rvol    = row_c.get("rolling_vol_10", np.nan)
        ...
        # ------------------------------
        # Bullish pullback: buy near lows
        # ------------------------------
        bull_ok = False
        if (
            daily_change > 1.0                               # day with some positive tone
            and not is_midday_ist(dt)                        # avoid lunch chop
            and pd.notna(vwap) and pd.notna(rsi) and pd.notna(stoch_k) and pd.notna(stoch_d) and pd.notna(adx_val)
            and vwap_slope_up(cidx, bars=2)                  # session bias up
            and (ema20 > ema50) and (row_c["EMA20_Slope"] > 0) and (row_c["EMA50_Slope"] > 0)
            and (40 <= rsi <= 60)                            # pullback zone (not OB)
            and (15 <= stoch_k <= 45) and (stoch_d <= 55)    # %K at lows, %D not OB
            and (row_c["low"] <= vwap * (1 + TOUCH_BUF))     # wick tags VWAP
            and (price >= vwap * (1 + RECLAIM_BUF))          # close reclaims over VWAP
            and (lower_wick_ratio(row_c) >= WICK_MIN)        # demand tail
            and (vol >= VOL_MIN_X * rvol)                    # participation
            and (25 <= adx_val <= 45)                        # "healthy" trend strength
        ):
            # avoid buying into PDH
            pdh = prev_day_high(combined, daykey)
            near_pdh = (pdh is not None) and (abs(price - pdh) / pdh <= PD_SAFETY)
            # ATR sanity: skip if a vanilla 2% stop is inside ~1.2×ATR noise
            atr_bad = (pd.notna(atr) and atr > 0 and (0.02 <= 1.2 * (atr / price)))
        ...
        if bull_ok:
            ...
            if signal_id not in existing_signal_ids:
                entry_price = round(float(vwap * (1 + RECLAIM_BUF)), 2)  # limit near VWAP, not bar high
                signals.append({
                    "Ticker": ticker,
                    "date": dt,
        ...
        # ------------------------------
        # Bearish pullback: sell near highs
        # ------------------------------
        bear_ok = False
        if (
            daily_change < -1.0
            ...
            and pd.notna(vwap) and pd.notna(rsi) and pd.notna(stoch_k) and pd.notna(stoch_d) and pd.notna(adx_val)
            and vwap_slope_down(cidx, bars=2)                 # session bias down
            and (ema20 < ema50) and (row_c["EMA20_Slope"] < 0) and (row_c["EMA50_Slope"] < 0)
            and (45 <= rsi <= 65)                             # pullback zone (not OS)
            and (55 <= stoch_k <= 85) and (stoch_d >= 45)     # %K at highs, %D not OS
            and (row_c["high"] >= vwap * (1 - TOUCH_BUF))     # wick tags VWAP
            and (price <= vwap * (1 - RECLAIM_BUF))           # close rejects under VWAP
            and (upper_wick_ratio(row_c) >= WICK_MIN)         # supply tail
            and (vol >= VOL_MIN_X * rvol)
            and (25 <= adx_val <= 45)
        ):
            # avoid shorting into PDL
        ...
        if bear_ok:
            ...
            if signal_id not in existing_signal_ids:
                entry_price = round(float(vwap * (1 - RECLAIM_BUF)), 2)  # limit near VWAP, not bar low
                signals.append({
                    "Ticker": ticker,
                    "date": dt,
