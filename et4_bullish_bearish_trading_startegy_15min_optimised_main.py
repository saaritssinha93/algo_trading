...
def main():
    ...
    - Executes 'job_run_twice' immediately if within the trading window (9:30:15 AM to 3:30:15 PM).
    - Validates and corrects main indicators.
    - Continuously checks for missing signals every second until the end of the trading window.
    """
    now = datetime.now(india_tz)
    
    # Define the trading window: 9:30:15 AM to 3:30:15 PM
    start_time_naive = datetime.combine(now.date(), datetime_time(9, 30, 15))
    end_time_naive = datetime.combine(now.date(), datetime_time(23, 30, 15))
    start_time = india_tz.localize(start_time_naive)
    end_time = india_tz.localize(end_time_naive)

    # Determine the current status relative to the trading window
    if now > end_time:
    ...
    # Execute initial run of trading strategy if within trading window
