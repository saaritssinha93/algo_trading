...
def detect_signals_in_memory(ticker, df_today):
    """
    This function replicates the "Bullish/Bearish pullback" logic from
    apply_trend_conditions() and screen_trend_for_bullish()/screen_trend_for_bearish(),
    but does everything IN MEMORY. It does NOT update the CSV.

    Steps:
    1) Check daily_change: (latest_close - first_open)/first_open * 100
    2) If ADX>24, decide if we are "Bullish" or "Bearish" scenario.
    3) For Bullish, use the 'pullback' conditions (MACD>Signal_Line, Stochastic <48, etc.)
    4) For Bearish, use the 'pullback' conditions (MACD<Signal_Line, Stochastic >53, etc.)
    5) Return a list of dictionaries, each containing the row's info from df_today.
    """

    signals_detected = []

    if df_today.empty:
        return signals_detected  # no data, so no signals

    # Attempt daily change calc
    try:
        first_open = df_today['open'].iloc[0]
        latest_close = df_today['close'].iloc[-1]
        daily_change = ((latest_close - first_open) / first_open) * 100
    except Exception as e:
        logging.error(f"[{ticker}] Error calculating daily_change in detect_signals_in_memory: {e}")
        return signals_detected

    # We'll look at the LAST row to see if there's a "strong trend" (e.g. ADX>24)
    latest_row = df_today.iloc[-1]
    strong_trend = latest_row.get('ADX', 0) > 24

    if not strong_trend:
        # No signals if trend is not strong
        return signals_detected

    # Decide if daily_change suggests "Bullish" or "Bearish"
    # (same logic from apply_trend_conditions)
    if (daily_change > 0.9) and (latest_row.get("RSI", 0) > 18):
        # BULLISH scenario, so do 'pullback' conditions
        # (This replicates screen_trend_for_bullish)
        cond = (
            (df_today['MACD'] > df_today['Signal_Line']) &
            (df_today['Stochastic'] < 48) &
    ...
    elif (daily_change < -0.9) and (latest_row.get("RSI", 0) < 82):
        # BEARISH scenario, so do 'pullback' conditions
        # (This replicates screen_trend_for_bearish)
        cond = (
            (df_today['MACD'] < df_today['Signal_Line']) &
...
def process_ticker(ticker, from_date_intra, to_date_intra, last_trading_day, india_tz):
    ...
    - Merge it into main CSV without overwriting older rows
    - Possibly detect the Entry Signal for this new row
    """
    try:
        with api_semaphore:
            ...
            # This returns only the single "latest" row with updated indicators:
            latest_row = fetch_intraday_data_with_historical(
                ticker, token, from_date_intra, to_date_intra, interval="15minute"
            )
            if latest_row.empty:
                logging.warning(f"[{ticker}] No new row returned from fetch_intraday_data_with_historical.")
                return None, []
        ...
        # 3) Normalize the latest_row time
        latest_row = normalize_time(latest_row, timezone='Asia/Kolkata')
        
        # 4) Check if this row is actually new
        if 'Signal_ID' not in latest_row.columns:
            logging.error(f"[{ticker}] Missing 'Signal_ID' in latest_row.")
            return None, []
        if not existing.empty and 'Signal_ID' in existing.columns:
            existing_ids = set(existing['Signal_ID'].astype(str).unique())
            latest_id = str(latest_row.iloc[0]['Signal_ID'])
            if latest_id in existing_ids:
                # This row is already in the CSV, skip
                logging.info(f"[{ticker}] Latest row is already present. Skipping.")
                return None, []

        # 5) [Optional] Determine Entry Signal for this single new row
        #    Example: if 'ADX' > 25 and 'MACD' > 'Signal_Line', set 'Entry Signal'='Yes'
        #    This is a sample condition; adapt to your logic:
        row_data = latest_row.iloc[0]
        adx_val = row_data.get('ADX', 0)
        macd_val = row_data.get('MACD', 0)
        sig_line_val = row_data.get('Signal_Line', 0)
        if adx_val > 24 and macd_val > sig_line_val:
            latest_row.at[latest_row.index[0], 'Entry Signal'] = 'Yes'
        else:
            latest_row.at[latest_row.index[0], 'Entry Signal'] = 'No'

        # 6) Assign logtime only for this new row
        latest_row['logtime'] = datetime.now(india_tz).isoformat()

        # 7) Merge + drop duplicates (keep='first' to preserve old row if conflict)
        final_data = pd.concat([existing, latest_row], ignore_index=True)
        if 'Signal_ID' in final_data.columns:
        ...
        logging.info(f"[{ticker}] Appended new row with date={latest_row.iloc[0]['date']} to {main_path}")

        # Return the single row appended + optional signals
        # If you want to pass back a signal dictionary, do so here:
        signal_list = []
