...
"""
Fetch 1-hour historical data for selected_stocks for the LAST 1 YEAR up to now (IST),
compute indicators, and write *_main_indicators_1h.csv files.

Window:
    • From: now - 365 days (IST)
    • To  : now (IST)

Adds (per bar):
    • Daily_Change  -> % vs previous trading day's final close
    • Intra_Change  -> % vs previous 1-hour bar within SAME day
"""
...
# Dynamic 1-year start date  <<< changed
NOW_IST = datetime.now(india_tz)
START_DATE = NOW_IST - timedelta(days=365)
START_DATE = START_DATE.replace(minute=0, second=0, microsecond=0)  # align to hour candle

# Directories <<< changed
CACHE_DIR = "data_cache_1h"
INDICATORS_DIR = "main_indicators_1h"
...
def calculate_vwap(df):
    # Session-agnostic cumulative VWAP
...
# ========= DATA FETCH =========
def fetch_historical_1h_df(token, start_dt_ist):
    """
    Fetch 1-hour bars from start_dt_ist (tz-aware IST) to now in ~60-day chunks. <<< changed
    ...
    while s < end:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                raw = kite.historical_data(token, query_from, query_to, "60minute")  # <<< changed
                df = pd.DataFrame(raw)
                if df.empty:
                    break
                # Ensure tz-aware (IST)
        ...
        # jump to next chunk, avoid overlap
        s = e + timedelta(hours=1)  # <<< changed
...
# ========= PER-TICKER PIPELINE =========
def process_ticker_last_year_1h(ticker, token):
    print(f"\nFetching {ticker} ...")
    df = fetch_historical_1h_df(token, START_DATE)
    if df.empty:
        print(f"  No data for {ticker} in the given 1y window.")
        return
    ...
    # ---- Change features (IST day buckets) ----
    df["date_only"] = df["date"].dt.tz_convert(india_tz).dt.date

    # Intra_Change: % vs previous 1h bar of SAME DAY  <<< wording changed
    df["Intra_Change"] = (
    ...
    # Daily_Change: % vs previous trading day's FINAL 1h close
    ...
    df["Daily_Change"] = (
        ...
        (df["Prev_Day_Close"]) * 100.0
    )

    # ---- Write ----
    out_path = os.path.join(INDICATORS_DIR, f"{ticker}_main_indicators_1h.csv")  # <<< changed
...
if __name__ == "__main__":
    print(
        f"Fetching window: {START_DATE.strftime('%Y-%m-%d %H:%M')} "
    ...
    for ticker in selected_stocks:
        ...
        if not token:
            ...
            continue
        try:
            process_ticker_last_year_1h(ticker, token)
