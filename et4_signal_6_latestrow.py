...
def detect_signals_in_memory(ticker, df_today, existing_signal_ids, prev_day_close=None):
    """
    Simpler detection that still has:
      - Bullish Pullback, Bullish Breakout
      - Bearish Pullback, Bearish Breakdown
    but uses only daily_change, ADX, RSI, VWAP, and rolling highs/lows.
    """
    signals_detected = []

    # If there's nothing to analyze, return
    if df_today.empty:
        return signals_detected

    # 1) Attempt to compute daily_change from prev_day_close
    try:
    ...
    # 2) Grab the latest row; require ADX > 20
    latest_row = df_today.iloc[-1]
    adx_val = latest_row.get('ADX', 0)
    if adx_val <= 20:
        return signals_detected

    # 3) Basic RSI, Price, and VWAP checks
    rsi_val = latest_row.get("RSI", 0)
    close_price = latest_row.get("close", 0)
    open_price = latest_row.get("open", 0)
    vwap_price = latest_row.get("VWAP", 0)

    # 4) Determine if environment is bullish or bearish based on daily_change, RSI, VWAP
    #    (You can adjust thresholds for daily_change & RSI as you like)
    if daily_change > 1.0 and rsi_val > 30 and close_price > vwap_price:
        trend_type = "Bullish"
    elif daily_change < -1.0 and rsi_val < 70 and close_price < vwap_price:
        trend_type = "Bearish"
    else:
        # If neither, skip
        return signals_detected

    # 5) Define minimal "rolling" windows for breakout/breakdown checks
    #    We'll assume a 5-bar rolling window for the day.
    #    If you have fewer rows, rolling(...) will do its best, but you may
    #    want to ensure you have enough history in df_today (like at least 5 bars).
    rolling_window = 5
    df_rolling = df_today.tail(rolling_window)  # last 5 rows
    recent_high = df_rolling["high"].max()
    recent_low  = df_rolling["low"].min()

    # We'll define 2 sub-signals for each environment:
    # Bullish => "Pullback", "Breakout"
    # Bearish => "Pullback", "Breakdown"
    if trend_type == "Bullish":
        # Pullback: e.g., close > open, daily_change>1, RSI>30, ADX>20, etc.
        cond_pullback = (
        ...
        # Breakout: e.g., close > ~recent_high
        cond_breakout = (
            (close_price > (recent_high * 0.99)) &
            (daily_change > 1.0) &
            (rsi_val > 30) &
            (adx_val > 20)
        )

        # Check which sub-signals pass:
        if cond_pullback:
            sub_signal = "Pullback"
        elif cond_breakout:
            sub_signal = "Breakout"
        ...
        # Pullback: e.g., close < open, daily_change< -1, RSI<70, ADX>20, etc.
        cond_pullback = (
            (close_price < open_price) &
            (daily_change < -1.0) &
            (rsi_val < 70) &
        ...
        if cond_pullback:
            ...
            sub_signal = "Breakdown"
        else:
            return signals_detected

    # 6) Build a Signal_ID and avoid duplicates
    sig_id = latest_row.get("Signal_ID", "")
    if not sig_id:
        sig_id = build_signal_id(ticker, latest_row)

    if sig_id not in existing_signal_ids:
        # 7) Construct final signal record
        signal_data = {
            "Ticker": ticker,
            "date": latest_row["date"],
            "Entry Type": sub_signal,      # "Pullback", "Breakout", or "Breakdown"
            ...
            "RSI": round(rsi_val, 2),
            "Signal_ID": sig_id,
            "logtime": latest_row.get("logtime", datetime.now(india_tz).isoformat()),
            "Entry Signal": "Yes"
        }
