...
def fetch_historical_price(ticker, kite, instrument_df, start_time, end_time, interval='5minute'):
    """
...
def monitor_ticker_in_window(ticker, buy_price, quantity, total_value_bought, trade_time, kite, instrument_df, target_percentage, interval, trend_type):
    ...
    first_target_price = buy_price * (1 + 0.005)  # First target at 1.5% above buy price
    first_stop_loss_price = buy_price * (1 - 0.01)  # Initial stop-loss at 1.5% below buy price
...
def monitor_paper_trades_backtest(kite, instrument_df, file_path, target_percentage=1.5, interval='5minute'):
    try:
...
def monitor_ticker_in_window_bear(ticker, sell_price, quantity, total_value_sold, trade_time, kite, instrument_df, interval, trend_type):
    ...
    first_target_price = sell_price * (1 - 0.005)  # First target at 2% below sell price
    first_stop_loss_price = sell_price * (1 + 0.01)  # Stop-loss at 2% above sell price
...
def monitor_paper_trades_backtest_bear(kite, instrument_df, file_path, target_percentage=1.5, interval='5minute'):
    try:
        paper_trades = pd.read_csv(file_path)
        if paper_trades.empty:
            print("No active trades found.")
