...
# Set up logging
logging.basicConfig(filename='trading1.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def monitor_ticker_in_window(ticker, buy_price, quantity, total_value_bought, trade_time, kite, instrument_df, target_percentage, interval, trend_type):
    ...
    first_target_price = buy_price * (1 + 0.004)  # Target at 1% above buy price
    ...
    result_file = f"result_papertrade_5min_{trade_time.strftime('%Y-%m-%d')}.csv"
    ...
    for _, price_row in historical_prices.iterrows():
        ...
        # Exit at market close if still holding
...
def monitor_ticker_in_window_bear(ticker, sell_price, quantity, total_value_sold, trade_time, kite, instrument_df, interval, trend_type):
    ...
    first_target_price = sell_price * (1 - 0.004)  # Target at 1% below sell price
...
def calculate_net_percentage(last_trading_days):
    ...
    Each file will be named as `result_final_5min_{last_trading_day}.csv`.
    ...
    for trading_day in last_trading_days:
        file_path = f"result_papertrade_5min_{trading_day}.csv"
        output_file = f"result_final_5min_{trading_day}.csv"
