...
Why you were seeing "No signals generated":
- Your current script has STRATEGY_PARAMS = {} (empty). That means no strategy rules exist,
  so the loop never evaluates anything and you will always get 0 signals.

This version:
✅ Includes 10 ready-to-run strategies (custom01..custom10)
✅ Validates strategy keys (fails fast if STRATEGY_PARAMS is empty / keys missing)
✅ Breakout is always preferred over Watch if both exist for a ticker
✅ Robust indicator-column fallbacks (EMA_20 vs 20_SMA, MACD_Hist aliases, BandWidth aliases)
✅ Interactive trade-date prompt (works with Spyder/runfile) + optional CLI flags
...
OUTPUT:
  ...
  - One signal per ticker for trade_date
  - If --trade-date is NOT provided, user will be prompted interactively.
"""
...
def _safe_read_tf(path: str, tz=IST) -> pd.DataFrame:
    ...
    try:
        ...
        # Your files often store tz-aware strings; utc=True keeps it safe.
...
def _load_mtf_context_daily_weekly_asof(ticker: str, trade_start_ist: datetime) -> pd.DataFrame:
    ...
    # Weekly NO same-week lookahead: shift weekly marker forward by 1 day, then asof-backward
...
def _build_trade_date_signal_for_ticker(
    ...
    # For WATCH mode, allow slightly fewer boosters
    ...
    breakout_windows = [int(w) for w in P.get("breakout_windows", [20, 50, 100])]
    breakout_windows = [w for w in breakout_windows if w >= 5]
    ...
    if mode in ("watch", "both") and (not is_breakout_today):
        ...
        # don't allow huge reversal candles
...
def _resolve_strategy_keys(args_strats: Optional[str]) -> List[str]:
    ...
    if not args_strats:
        ...
        missing = [k for k in DEFAULT_STRATEGY_KEYS if k not in STRATEGY_PARAMS]
        if missing:
            # Don't crash; just warn and proceed with the ones that exist.
            print(f"! Warning: missing strategy keys in STRATEGY_PARAMS: {missing}")
        if not keys:
...
def _trade_start_from_date_str(trade_date: Optional[str]) -> datetime:
    if not trade_date:
        trade_date = _prompt_trade_date_ist()
    d = pd.to_datetime(trade_date, format="%Y-%m-%d", errors="raise").date()
    return IST.localize(datetime(d.year, d.month, d.day, 0, 0, 0))

def _latest_output_if_exists(strategy_label: str, trade_start_ist: datetime, mode: str) -> Optional[str]:
...
def main():
    ...
    for i, ticker in enumerate(tickers, start=1):
        ...
        # Evaluate all strategies; BREAKOUT always wins over WATCH
        for key in strategy_keys:
            ...
            if sig.get("signal_type") == "BREAKOUT_FIRST":
                best_breakout = sig
                break  # earliest breakout wins immediately
    ...
    # Ensure one row per ticker per trade_date; prefer breakout over watch just in case
