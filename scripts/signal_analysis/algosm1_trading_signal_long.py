...
For EVERY 1H bar (per ticker), we:
    ...
    3) If all align -> emit a LONG signal row
...
def build_signals_for_ticker(ticker: str) -> pd.DataFrame:
    ...
    For a single ticker:
        ...
        • compute LONG signals for EACH 1H bar (further relaxed conditions),
        • return a dataframe with one row per LONG signal.

    SHORT side logic is currently commented out.
    """
    ...
    # FURTHER RELAXED long daily confirmation:
    daily_confirm_long = (
        (D_close > D_EMA50) &
        (D_RSI  > 45) &          # was 48, now 45
        (D_MACD_Hist > 0)
        # still no (D_close > D_20SMA)
    )

    # Short side (still computed but we won't emit short signals)
    daily_confirm_short = (
        (D_close < D_EMA50) &
        (D_RSI  < 50) &
        (D_MACD_Hist < 0) &
    ...
    # ---------- 1H ENTRY TRIGGERS (further relaxed) ----------
    ...
    # Previous 3-bar high/low (was 5 in original strict version)
    ...
    # Momentum & trend strength (further relaxed)
    rsi_ok_long  = rsi_h > 50          # was 52
    rsi_ok_short = rsi_h < 48          # unchanged for now
    adx_strong   = (adx_h > 18)        # was 20

    # Volume expansion vs 10-bar avg (further relaxed)
    vol_ma10 = vol_h.rolling(10, min_periods=1).mean()
    vol_ok   = vol_h >= (0.85 * vol_ma10)   # was 0.9 * vol_ma10
    ...
    # ---------- BUILD OUTPUT ROWS (ONLY LONG SIDE) ----------
    rows = []
