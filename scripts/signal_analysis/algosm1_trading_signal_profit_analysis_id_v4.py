...
1) Generate multi-timeframe positional LONG signals using:
    • Weekly trend filter (NO same-date weekly lookahead)
...
Daily rule uses **previous day's** Daily_Change:
    • On the Daily timeframe we create Daily_Change_prev = Daily_Change.shift(1).
    • After merging, we use Daily_Change_prev_D for the filter
      (so hourly bars on date D see the move of day D-1).
...
Evaluation (per LONG signal):
    • Scan forward 1H data:
        - target = entry_price * TARGET_MULT, check high >= target
    • Record:
        - hit_<TARGET_SUFFIX>pct (True/False)
        - time_to_<TARGET_SUFFIX>pct_days       → trading days
        - days_to_<TARGET_SUFFIX>pct_calendar   → calendar days between entry & 1st hit
        - exit_time
        - exit_price
        - pnl_pct
        - pnl_rs
        ...
        - same_day_<TARGET_SUFFIX>pct_hit   (True/False: target hit on same calendar date)
...
Outputs:
    ...
    • signals/multi_tf_signals_<TARGET_SUFFIX>pct_eval_<YYYYMMDD_HHMM>_IST.csv
"""
...
TARGET_PCT: float = 0.5  # works for 0.5, 1.0, 5.0, 7.5, etc.
TARGET_MULT: float = 1.0 + TARGET_PCT / 100.0

# Suffix / label that work for both integer and non-integer targets
if float(TARGET_PCT).is_integer():
    # e.g. 5.0 -> "5pct", label "5%"
    TARGET_SUFFIX: str = f"{int(TARGET_PCT)}"
    TARGET_LABEL: str = f"{int(TARGET_PCT)}%"
else:
    # e.g. 0.5 -> "0p5pct", label "0.5%"
    TARGET_SUFFIX: str = str(TARGET_PCT).replace(".", "p")
    TARGET_LABEL: str = f"{TARGET_PCT:g}%"

# Dynamic column names based on TARGET_PCT
HIT_COL: str = f"hit_{TARGET_SUFFIX}pct"
TIME_TD_COL: str = f"time_to_{TARGET_SUFFIX}pct_days"
TIME_CAL_COL: str = f"days_to_{TARGET_SUFFIX}pct_calendar"
SAME_DAY_HIT_COL: str = f"same_day_{TARGET_SUFFIX}pct_hit"

# Eval filename suffix based on TARGET_PCT
EVAL_SUFFIX: str = f"{TARGET_SUFFIX}pct_eval"
...
def build_signals_for_ticker(ticker: str) -> list[dict]:
    ...
    For a single ticker:
        ...
        • Apply the (slightly relaxed) multi-timeframe LONG strategy
    ...
    # IMPORTANT (weekly no same-date lookahead):
    df_w_sorted = df_w.sort_values("date").copy()
    df_w_sorted["date"] = df_w_sorted["date"] + pd.Timedelta(seconds=1)

    # Merge daily into hourly (Daily_Change_prev is already previous-day)
    df_hd = pd.merge_asof(
        df_h_sorted,
        df_d_sorted,
        on="date",
    ...
    # ----------------- STRATEGY CONDITIONS (LONG ONLY, RELAXED A LITTLE) -----------------

    # WEEKLY FILTER – slightly easier RSI, ADX same
    weekly_long_ok = (
    ...
    # DAILY FILTER — based on **previous-day** Daily_Change_prev_D
    daily_change_prev = df_hdw["Daily_Change_prev_D"]

    # LONG: previous day up ≥ +3.5%
    daily_long_ok = daily_change_prev >= 3.5

    # HOURLY TRIGGER – slightly relaxed ATR and price proximity
    ...
    hourly_long_ok = (
        ...
        (df_hdw["close_H"] >= df_hdw["Recent_High_5_H"] * 0.997)
    )
...
# ------------ SAME-DAY TARGET HIT STATS ----------------
def compute_same_day_target_stats(eval_df: pd.DataFrame) -> dict:
    """
    Compute how many signals hit the +TARGET_PCT% target on the SAME
    calendar date as entry.

    Conditions:
        - HIT_COL == True
        - TIME_CAL_COL == 0  (0 calendar days from entry to first hit)

    Adds column:
        SAME_DAY_HIT_COL (bool)
    """
    df = eval_df

    if HIT_COL not in df.columns or TIME_CAL_COL not in df.columns:
        print("! Cannot compute same-day stats: missing hit/time columns.")
        df[SAME_DAY_HIT_COL] = False
        return {}

    same_day_mask = df[HIT_COL] & (df[TIME_CAL_COL] == 0)
    df[SAME_DAY_HIT_COL] = same_day_mask

    total_signals = len(df)
    total_hits = int(df[HIT_COL].sum())
    same_day_hits = int(same_day_mask.sum())

    same_day_hit_pct_all = (same_day_hits / total_signals * 100.0) if total_signals > 0 else 0.0
    same_day_hit_pct_hits = (same_day_hits / total_hits * 100.0) if total_hits > 0 else 0.0

    print("\n--- SAME-DAY TARGET HIT STATS ---")
    print(f"Total signals               : {total_signals}")
    print(f"Total {TARGET_LABEL} hits         : {total_hits}")
    print(f"Same-day {TARGET_LABEL} hits      : {same_day_hits}")
    print(f"Same-day hits (% of all)   : {same_day_hit_pct_all:.1f}%")
    print(f"Same-day hits (% of hits)  : {same_day_hit_pct_hits:.1f}%")

    return {
        "total_signals": total_signals,
        "total_hits": total_hits,
        "same_day_hits": same_day_hits,
        "same_day_hit_pct_all": same_day_hit_pct_all,
        "same_day_hit_pct_hits": same_day_hit_pct_hits,
    }
...
def apply_capital_constraint(out_df: pd.DataFrame):
    """
    Apply a portfolio-level capital constraint and return
...
def main():
    ...
    # Compute same-day target stats + column
    compute_same_day_target_stats(out_df)
