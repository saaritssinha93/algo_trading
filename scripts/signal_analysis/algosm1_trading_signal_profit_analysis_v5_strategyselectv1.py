...
MAX_CAPITAL_RS: float = 30000000.0        # total capital pool
...
def build_signals_custom4(ticker: str) -> list[dict]:
    """
    CUSTOM STRATEGY 4 (SLIGHTLY STRICTER)

    Goal: keep the same structure but remove unnecessary signals by:
      - Slightly stricter weekly trend quality + optional location filter
      - Daily candle quality (bullish + strong close in range)
      - Hourly trigger needs clearer momentum + stronger breakout behavior
    """
    path_1h = os.path.join(DIR_1h, f"{ticker}_main_indicators_1h.csv")
    path_d  = os.path.join(DIR_D,  f"{ticker}_main_indicators_daily.csv")
    path_w  = os.path.join(DIR_W,  f"{ticker}_main_indicators_weekly.csv")
    ...
    df_w_sorted = df_w.sort_values("date").copy()
    df_w_sorted["date"] += pd.Timedelta(seconds=1)  # no same-date weekly lookahead
    ...
    # Require core columns used below
    required_dropna = [
        "close_W", "EMA_50_W", "EMA_200_W", "RSI_W", "ADX_W",
        "Daily_Change_prev_D",
        "close_D", "open_D", "high_D", "low_D",
    ...
    keep = [c for c in required_dropna if c in df_hdw.columns]
    df_hdw = df_hdw.dropna(subset=keep)
    ...
    weekly_mild = (
        (df_hdw["close_W"] > df_hdw["EMA_200_W"] * 1.025) &
        (df_hdw["RSI_W"].between(44.0, 68.0)) &
        (df_hdw["ADX_W"] > 13.0)
    )

    weekly_not_extreme = df_hdw["RSI_W"] < 80.0
    ...
    bullish_day = df_hdw["close_D"] > df_hdw["open_D"]
    strong_close = range_pos >= 0.62  # slightly stricter than 0.60

    daily_ok = daily_ok_band & bullish_day & strong_close

    # =========================
    # HOURLY (TIGHTEN TRIGGER)
    # =========================
    hourly_trend = (
        (df_hdw["close_H"] >= df_hdw["EMA_200_H"] * 0.998) &
        (df_hdw["close_H"] >= df_hdw["EMA_50_H"]  * 0.998)
    )

    rsi_ok = (
        (df_hdw["RSI_H"] > 53.0) |
        ((df_hdw["RSI_H"] > 49.0) & (df_hdw["RSI_H"] > df_hdw["RSI_H_prev"]))
    )

    macd_ok = (
        (df_hdw["MACD_Hist_H"] > -0.035) &
        (df_hdw["MACD_Hist_H"] >= df_hdw["MACD_Hist_H_prev"] * 0.92)
    )

    stoch_ok = (
        (df_hdw["Stoch_%K_H"] >= df_hdw["Stoch_%D_H"] * 0.99) &
        (df_hdw["Stoch_%K_H"] >= 22.0)
    )

    momentum_ok = rsi_ok & macd_ok & stoch_ok

    hourly_vol_ok = (
        (df_hdw["ATR_H"] >= df_hdw["ATR_H_5bar_mean"] * 0.96) &
        (
            (df_hdw["ADX_H"] >= 12.0) |
            (df_hdw["ADX_H"] >= df_hdw["ADX_H_5bar_mean"] * 1.04)
        )
    )
    ...
    breakout_strict = df_hdw["close_H"] >= df_hdw["Recent_High_5_H"]
    breakout_near   = df_hdw["close_H"] >= df_hdw["Recent_High_5_H"] * 0.997

    intra_chg = df_hdw.get("Intra_Change_H", pd.Series(0.0, index=df_hdw.index)).fillna(0.0)
    strong_up = intra_chg >= 0.14

    breakout_ok = price_above_vwap & (
        breakout_strict |
        (breakout_near & strong_up)
    )

    # optional tiny anti-chop: RSI not deteriorating
    rsi_improving = df_hdw["RSI_H"] >= df_hdw["RSI_H_prev"] - 0.2

    hourly_ok = hourly_trend & momentum_ok & hourly_vol_ok & breakout_ok & rsi_improving
