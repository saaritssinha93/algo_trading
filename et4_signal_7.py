...
For each date, it:
  ...
  4. Runs detection logic for 4 strategies with LangChain LLM confirmation:
       - Bullish Breakout
       - Bullish Pullback
       - Bearish Breakdown
       - Bearish Pullback
...
try:
    ...
    LLM_AVAILABLE = False
    print("LangChain or OpenAI not installed; skipping LLM calls. All raw signals will pass.")

def confirm_signal_with_llm(strategy_name, indicators_dict) -> bool:
    """
    Calls an LLM via LangChain to confirm the raw strategy signal.
    Returns True if the LLM response begins with 'yes'; else returns False.
    """
    if not LLM_AVAILABLE:
        return True  # Fallback: if no LLM, do not block signals

    system_prompt = (
        "You are a trading assistant with expertise in technical analysis. "
        "The user provides a set of indicator values that claim to detect a '{}' signal. "
        "Respond strictly with 'Yes' if the signal is plausible, or 'No' if it is not."
    ).format(strategy_name)

    user_prompt = (
        "Strategy: {}\nIndicators:\n{}\nAnswer strictly 'Yes' or 'No'.".format(
            strategy_name, json.dumps(indicators_dict, indent=2)
        )
    )
    ...
    chat = ChatOpenAI(
        ...
        openai_api_key="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"  # Replace with your actual API key
    )
    template = ChatPromptTemplate.from_messages([
        SystemMessagePromptTemplate.from_template(system_prompt),
        HumanMessagePromptTemplate.from_template(user_prompt)
    ])
    messages = template.format_prompt().to_messages()
...
###########################################################
# 6) Checks for Bullish / Bearish Patterns with LLM Enhancement
###########################################################
...
###########################################################
# 7) detect_signals_in_memory with LLM Enhancement
###########################################################
def detect_signals_in_memory(
    ...
    for idx_detect, row_detect in df_for_detection.iterrows():
        ...
        # ---- BULLISH BREAKOUT with LLM confirmation ----
        raw_bull_breakout = (
            check_bullish_breakout(daily_change, adx_val, price, high_10, vol, macd_val, roll_vol,
        ...
        if raw_bull_breakout:
            strategy_data = {
                ...
                "StochDiff": stoch_diff,
                "VWAP": vwap
            }
            if confirm_signal_with_llm("Bullish Breakout", strategy_data):
        ...
        # ---- BULLISH PULLBACK with LLM confirmation ----
        raw_bull_pullback = check_bullish_pullback(daily_change, adx_val, macd_above_signal_now,
                                                   stoch_val, stoch_diff, slope_bull, pullback_pct, vwap, price)
        if raw_bull_pullback:
            strategy_data = {
                "Daily_Change": daily_change,
                "ADX": adx_val,
                "Price": price,
            ...
            if confirm_signal_with_llm("Bullish Pullback", strategy_data):
        ...
        # ---- BEARISH BREAKDOWN with LLM confirmation ----
        raw_bear_breakdown = (
            check_bearish_breakdown(daily_change, adx_val, price, low_10, vol, roll_vol, macd_val,
        ...
        if raw_bear_breakdown:
            ...
            if confirm_signal_with_llm("Bearish Breakdown", strategy_data):
        ...
        # ---- BEARISH PULLBACK with LLM confirmation ----
        raw_bear_pullback = check_bearish_pullback(daily_change, adx_val, macd_above_signal_now,
                                                   stoch_val, stoch_diff, slope_bear, bounce_pct, vwap, price)
        if raw_bear_pullback:
            strategy_data = {
                "Daily_Change": daily_change,
                "ADX": adx_val,
                "Price": price,
            ...
            if confirm_signal_with_llm("Bearish Pullback", strategy_data):
