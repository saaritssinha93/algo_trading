...
def detect_signals_in_memory(ticker: str, df_today: pd.DataFrame) -> list:
    """
    Signal detection with moderately strict thresholds.
    ...
    Steps:
      ...
      2) Check ADX > 28 for strong momentum
    ...
    # -- 2) Check if ADX is "strong" (e.g., > 28 for moderately strict conditions)
    latest_row = df_today.iloc[-1]
    adx_val = latest_row.get("ADX", 0)
    if adx_val <= 28:  # Moderate ADX threshold
        return signals_detected

    # Decide if it's Bullish or Bearish scenario:
    if (
        daily_change > 1.0  # Moderate threshold for bullish
        and latest_row.get("RSI", 0) > 22  # Slightly stricter RSI threshold
        and latest_row.get("close", 0) > latest_row.get("VWAP", 0) * 1.01  # Moderate VWAP multiplier
    ):
        trend_type = "Bullish"
    elif (
        daily_change < -1.0  # Moderate bearish threshold
        and latest_row.get("RSI", 0) < 78  # Slightly stricter RSI for bearish
        and latest_row.get("close", 0) < latest_row.get("VWAP", 0) * 0.99  # Moderate VWAP condition
    ):
        trend_type = "Bearish"
    else:
    ...
    volume_multiplier = 1.4  # Slightly stricter volume multiplier
    rolling_window = 7  # Moderate rolling window
    MACD_BULLISH_OFFSET = -0.3  # Moderately forgiving MACD for bullish
    MACD_BEARISH_OFFSET = 0.3  # Moderately forgiving MACD for bearish

    if trend_type == "Bullish":
        conditions = {
            "Pullback": (
                (df_today["low"] >= df_today["VWAP"] * 0.978)  # Slightly stricter
                ...
                & (df_today["Stochastic"] < 48)  # Stricter threshold for stochastic
                & (df_today["Stochastic"].diff() > 0)
                & (df_today["ADX"] > 28)
            ),
            "Breakout": (
                (df_today["close"] > df_today["high"].rolling(window=rolling_window).max() * 1.01)
                & (df_today["volume"] > volume_multiplier * df_today["volume"].rolling(window=rolling_window).mean())
                & (df_today["MACD"] > df_today["Signal_Line"])
                & (df_today["MACD"] > MACD_BULLISH_OFFSET)
                & (df_today["close"] > df_today["20_SMA"])
                & (df_today["Stochastic"] > 52)
                & (df_today["Stochastic"].diff() > 0)
                & (df_today["ADX"] > 28)
            ),
        }
        ...
        conditions = {
            "Pullback": (
                (df_today["high"] <= df_today["VWAP"] * 1.022)  # Slightly stricter upper limit
                & (df_today["close"] < df_today["open"])
                & (df_today["MACD"] < df_today["Signal_Line"])
                & (df_today["MACD"] < MACD_BEARISH_OFFSET)
                & (df_today["close"] < df_today["20_SMA"])
                & (df_today["Stochastic"] > 52)  # Stricter threshold for bearish stochastic
                & (df_today["Stochastic"].diff() < 0)
                & (df_today["ADX"] > 28)
            ),
            "Breakdown": (
                (df_today["close"] < df_today["low"].rolling(window=rolling_window).min() * 0.99)
                & (df_today["volume"] > volume_multiplier * df_today["volume"].rolling(window=rolling_window).mean())
                & (df_today["MACD"] < df_today["Signal_Line"])
                & (df_today["MACD"] < MACD_BEARISH_OFFSET)
                & (df_today["close"] < df_today["20_SMA"])
                & (df_today["Stochastic"] < 48)  # Stricter lower stochastic
                & (df_today["Stochastic"].diff() < 0)
                & (df_today["ADX"] > 28)
            ),
        }
