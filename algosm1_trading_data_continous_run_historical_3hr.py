# -*- coding: utf-8 -*-
"""
Fetch 3-hour synthetic candles for selected_stocks for the LAST 1 YEAR up to now (IST).

Flow:
1) Fetch 60-minute candles (valid in Kite).
2) Resample to 3-hour OHLCV in IST (optionally offset to 09:15).
3) Compute indicators + Intra_Change + Daily_Change.
4) Save per-ticker CSV in main_indicators_3h/.
"""
...
START_DATE = (NOW_IST - timedelta(days=365)).replace(minute=0, second=0, microsecond=0)

# Optional: align resample buckets to 09:15 using a 15-minute offset
# (Requires pandas >= 1.1; comment this out if your pandas is older)
RESAMPLE_RULE = "3H"
RESAMPLE_LABEL = "right"
RESAMPLE_CLOSED = "right"
RESAMPLE_OFFSET = "15min"  # aligns bins like ...06:15, 09:15, 12:15, 15:15 IST

CACHE_DIR = "data_cache_3h"
INDICATORS_DIR = "main_indicators_3h"
...
def _ma(x, window, kind="sma"):
    return x.ewm(span=window, adjust=False).mean() if kind=="ema" else x.rolling(window, min_periods=window).mean()
...
def calculate_stochastic_fast(df, k_period=14, d_period=3):
    ...
    k = k.clip(0,100); d = k.rolling(d_period, min_periods=d_period).mean().clip(0,100)
    return k, d
...
def calculate_adx(df, period=14):
    ...
    tr = pd.Series(np.maximum.reduce([(high-low).to_numpy(), (high-prev_close).abs().to_numpy(), (low-prev_close).abs().to_numpy()]), index=df.index)
...
def calculate_mfi(df, period=14):
    tp = (df["high"] + df["low"] + df["close"]) / 3
    mf = tp * df["volume"]
    pos_mf = mf.where(tp.diff() > 0, 0); neg_mf = mf.where(tp.diff() < 0, 0)
...
# ========= FETCH 60M, THEN RESAMPLE TO 3H =========
def fetch_historical_60m_df(token, start_dt_ist):
    ...
    while s < end:
        ...
        for attempt in range(1, MAX_RETRIES+1):
            try:
                raw = kite.historical_data(token, s, e, "60minute")
                df = pd.DataFrame(raw)
                if df.empty: break
...
def resample_to_3h(df_60m):
    ...
    # Use lowercase '3h' per pandas deprecation of 'H'
    ...
    # Keep only canonical intraday endpoints (guards against any odd bins)
...
# ========= PER-TICKER PIPELINE =========
def process_ticker_last_year_3h_resampled(ticker, token):
    print(f"\nFetching {ticker} ...")
    df_60m = fetch_historical_60m_df(token, START_DATE)
    if df_60m.empty:
        print(f"  No 60m data for {ticker} in the given 1y window.")
        return
    ...
    # Change features (IST trading day buckets)
    ...
    out_path = os.path.join(INDICATORS_DIR, f"{ticker}_main_indicators_3h.csv")
...
if __name__ == "__main__":
    print(f"Fetching window (base 60m): {START_DATE.strftime('%Y-%m-%d %H:%M')} to {datetime.now(india_tz).strftime('%Y-%m-%d %H:%M')} (IST)")
    for ticker in selected_stocks:
        ...
        if not token:
            ...
            continue
        try:
            process_ticker_last_year_3h_resampled(ticker, token)
