...
def detect_signals_in_memory(ticker: str, df_today: pd.DataFrame) -> list:
    ...
    if adx_val <= 25:  # Maintain ADX threshold for general momentum
        return signals_detected
    ...
    if trend_type == "Bullish":
        conditions = {
            "Pullback": (
                ...
                & (df_today["Stochastic"] < 50)  # Slightly relaxed stochastic threshold
                & (df_today["Stochastic"].diff() > 0)
                & (df_today["ADX"] > 25)  # Reduced ADX threshold for pullbacks
            ),
            "Breakout": (
