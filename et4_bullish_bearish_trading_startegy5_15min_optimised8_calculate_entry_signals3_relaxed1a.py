...
def detect_signals_in_memory(ticker: str, df_today: pd.DataFrame) -> list:
    ...
    Steps:
      ...
      2) Check ADX > 30 for strong momentum
    ...
    # -- 2) Check if ADX is "strong" (e.g., > 30 for moderately strict conditions)
    latest_row = df_today.iloc[-1]
    adx_val = latest_row.get("ADX", 0)
    if adx_val <= 30:  # Moderate ADX threshold
        return signals_detected

    # Decide if it's Bullish or Bearish scenario:
    if (
        daily_change > 1.0  # Slightly stricter than relaxed
        and latest_row.get("RSI", 0) > 25  # Moderate RSI threshold
        and latest_row.get("close", 0) > latest_row.get("VWAP", 0) * 1.01  # Moderate VWAP multiplier
    ):
        trend_type = "Bullish"
    elif (
        daily_change < -1.0  # Stricter bearish threshold
        and latest_row.get("RSI", 0) < 75  # Moderate RSI for bearish
    ...
    volume_multiplier = 1.5  # Moderate volume multiplier
    rolling_window = 8  # Moderate rolling window
    MACD_BULLISH_OFFSET = -0.2  # Slightly more forgiving MACD for bullish
    MACD_BEARISH_OFFSET = 0.2  # Slightly more forgiving MACD for bearish

    if trend_type == "Bullish":
        conditions = {
            "Pullback": (
                (df_today["low"] >= df_today["VWAP"] * 0.980)  # Slightly relaxed
                ...
                & (df_today["Stochastic"] < 45)  # Moderate threshold for stochastic
        ...
        conditions = {
            "Pullback": (
                (df_today["high"] <= df_today["VWAP"] * 1.02)  # Moderate upper limit
                & (df_today["close"] < df_today["open"])
                & (df_today["MACD"] < df_today["Signal_Line"])
