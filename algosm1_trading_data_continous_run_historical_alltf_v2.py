# -*- coding: utf-8 -*-
"""
Unified Zerodha (KiteConnect) data fetch + indicator generator for multiple timeframes.

Key behavior:
- Market-session-aware cutoffs (IST) for intraday and daily.
- Optional NSE holiday file support.
- Incremental fetching with warmup, to reduce API calls.
- Per-ticker freshness skip:
    For each ticker in each timeframe, we check if the latest expected candle is already present.
    If yes -> skip that ticker (and we can even skip fetching tokens for skipped tickers).

Why this version is faster than v2_FIXED:
✅ Freshness check is done BEFORE creating worker jobs
✅ Tick data / tokens are fetched ONLY for tickers that actually need updates
✅ Intraday freshness in "previous" context uses the previous trading day (not current session)
✅ Weekly freshness uses the correct weekly bar anchor (week start), so it can actually skip

Run examples:
    python algosm1_trading_data_continous_run_historical_alltf_v2_FASTSKIP.py all --context live --max-workers 6
    python algosm1_trading_data_continous_run_historical_alltf_v2_FASTSKIP.py 5min --context live
    python algosm1_trading_data_continous_run_historical_alltf_v2_FASTSKIP.py daily --context previous

Spyder note:
- mode is optional (defaults to "all"), so runfile(...) works without CLI args.
"""
...
# Candle-end timestamps for intraday (recommended for your pipeline)
DEFAULT_INTRADAY_TIMESTAMP = "end"  # "end" or "start"

# Incremental fetch warmup bars (to re-stabilize indicators near the tail)
WARMUP_BARS = {
    "5min":  600,   # ~5 trading days of 5m
    "15min": 400,   # ~10 trading days of 15m
    "1h":    300,   # ~60 trading days of 1h
    "3h":    300,   # uses 60m base then resample
    "daily": 260,   # ~1y trading
    "weekly": 120,  # ~2y weeks
}

# Token cache
TOKENS_CACHE_FILE = "tokens_cache.json"
...
def setup_logger() -> logging.Logger:
    ...
    fh = logging.FileHandler("fetcher_run.log", mode="w", encoding="utf-8")
    fh.setFormatter(fmt)
    logger.addHandler(fh)
    return logger


# ========= STOCK UNIVERSE (similar to older alltf.py behavior) =========
...
def _normalize_ticker_list(obj) -> list[str]:
    """Accept list/tuple/set/dict/Series and normalize to unique uppercase tickers."""
    if obj is None:
        return []
...
def load_stock_universe(logger: logging.Logger) -> tuple[list[str], dict[str, int]]:
    """
    Load your stock universe + (optional) prebuilt token map.

    Priority:
      1) Import et4_filtered_stocks_MIS and read:
          - shares_tokens (dict[ticker -> token]) if present, else
          - selected_stocks (list/tuple/set/dict/Series/str)
             * if dict values are numeric -> treat as token map
      2) tickers.txt (one ticker per line) searched in:
          - current working dir
          - script dir
          - parent dir
    Returns:
      (tickers_list, token_map_partial_or_full)
    """
    cwd = Path.cwd().resolve()
    script_dir = Path(__file__).resolve().parent
    parent_dir = script_dir.parent

    # Ensure script_dir and parent_dir are importable (Spyder often changes CWD)
    ...
    if mod is not None:
        if hasattr(mod, "shares_tokens") and isinstance(getattr(mod, "shares_tokens"), dict):
            raw = getattr(mod, "shares_tokens")
            try:
                token_map = {str(k).strip().upper(): int(v) for k, v in raw.items() if str(k).strip()}
                tickers = sorted(token_map.keys())
                if tickers:
                    logger.info("Loaded %d tickers from et4_filtered_stocks_MIS.shares_tokens", len(tickers))
        ...
        if hasattr(mod, "selected_stocks"):
            ...
            # selected_stocks as dict may actually be a token map
            if isinstance(ss, dict):
                ...
                if tickers:
                    logger.info("Loaded %d tickers from et4_filtered_stocks_MIS.selected_stocks (dict/set-like)", len(tickers))
                    return tickers, token_map

            tickers = _normalize_ticker_list(ss)
            if tickers:
                logger.info("Loaded %d tickers from et4_filtered_stocks_MIS.selected_stocks", len(tickers))
                return tickers, token_map

    # tickers.txt fallbacks
    for base in (cwd, script_dir, parent_dir):
        ...
        if f.exists():
            ...
            if tickers:
                logger.info("Loaded %d tickers from %s", len(tickers), str(f))
                return tickers, token_map
    ...
    raise RuntimeError(
        ...
        "       - shares_tokens = {TICKER: TOKEN, ...}   OR\n"
        "       - selected_stocks = [TICKER, ...] / {TICKER, ...} / {TICKER: TOKEN, ...}\n"
        "  2) Or create tickers.txt (one ticker per line) in cwd / script dir / parent dir.\n\n"
...
def _read_holidays(path: str) -> set[date]:
    """
    Read holiday dates from a file if present.
    Supports:
      - one date per line (YYYY-MM-DD)
      - CSV with a column named 'date'
    """
...
def _round_down_session_anchored(ts: datetime, step_min: int) -> datetime:
    """
    Round down to step_min, anchored to 09:15.
    step_min=15 -> 09:15, 09:30, ... 15:30
    step_min=60 -> 09:15, 10:15, ... 15:15
    """
...
def last_completed_intraday_end(now_ist: datetime, step_min: int, holidays: set[date]) -> datetime:
    """
    Last completed candle END timestamp inside market session.
    After market close -> clamp to end-of-session (15:30), then session-anchor round-down.
    """
    ...
    # Non-trading day -> previous trading day close
    ...
    # Before open -> previous trading day close
    if now_ist.time() < MARKET_OPEN_TIME:
        d = _prev_trading_day(d, holidays)
        close_dt = IST_TZ.localize(datetime(d.year, d.month, d.day, 15, 30, 0))
        return _round_down_session_anchored(close_dt, step_min)

    # After close -> clamp to close time
...
def get_end_dt_for_mode(mode: str, now_ist: datetime, context: str, holidays: set[date], force_today_daily: bool) -> datetime:
    """
    End cutoff for fetching.

    NOTE:
    - For "previous" context intraday, we still return 23:59:59 of previous trading day.
      But freshness comparisons use the correct session close candle via expected_last_stamp().
    """
...
def _read_last_ts_fast_csv(path: str) -> Optional[pd.Timestamp]:
    """
    Fast-ish way to read only the last timestamp from a CSV without loading the whole file.
    Assumes a header row exists and a 'date' column exists.
    """
    try:
        with open(path, "rb") as f:
            ...
            data = b""
            offset = 0
            # Pull a few lines from the end (robust to long indicator rows)
...
def _read_last_ts_from_csv(path: str) -> Optional[pd.Timestamp]:
...
def _maybe_convert_existing_intraday_to_end(df: pd.DataFrame, step_min: int) -> pd.DataFrame:
    """
    If an existing file is start-timestamped, convert it to end timestamps.
    Heuristic: if earliest time is exactly 09:15, we assume START timestamps.
    """
...
def expected_last_stamp(mode: str, now_ist: datetime, context: str, holidays: set[date],
    ...
    """
    Returns a dict describing what's "fresh enough" for that mode:
      - daily: {"kind":"date", "value": cutoff_date}
      - weekly: {"kind":"date", "value": expected_week_start_date}
      - intraday: {"kind":"ts", "value": expected_last_ts, "step_min": step}
      - 3h: {"kind":"ts", "value": expected_last_ts}
    """
    mode = mode.lower().strip()
    context = context.lower().strip()
    ...
    # 3h: we keep end labels (09:15 / 12:15 / 15:15)
    return {"kind": "ts", "value": exp_end, "step_min": step}
...
def ticker_is_fresh(mode: str, out_path: str, now_ist: datetime, context: str,
    ...
    last_ts = _read_last_ts_from_csv(out_path)
    if last_ts is None:
        return False
    ...
    # exact match / close enough tolerance
    tol = timedelta(seconds=1)

    # If file is start-timestamped while we expect end (or vice versa), allow a 1-step correction.
    ...
    if last_ts >= (exp_ts - tol):
        return True
    if step_min > 0:
        # If the file is start-stamped but we're expecting end (or opposite), accept one-step shift.
...
def fetch_historical_generic(kite: KiteConnect, token: int, start_dt_ist: datetime, end_dt_ist: datetime, interval: str,
    ...
    """
    Fetch in chunks with retries.
    If intraday_ts == "end", shift returned timestamps to candle END.
    """
...
def resample_to_3h(df_60m: pd.DataFrame) -> pd.DataFrame:
    ...
    # shift for 09:15-anchored blocks
    df_shift = df.copy()
...
def load_or_fetch_tokens(kite: KiteConnect, tickers: list[str], logger: logging.Logger, refresh: bool = False) -> dict[str, int]:
    tickers_u = sorted({t.upper().strip() for t in tickers if t.strip()})
    ...
    if (not refresh) and os.path.exists(TOKENS_CACHE_FILE):
        try:
            ...
            if age_days <= TOKENS_CACHE_MAX_AGE_DAYS:
                ...
                if isinstance(cache, dict) and all(t in cache for t in tickers_u):
                    return {t: int(cache[t]) for t in tickers_u}
        except Exception:
            pass

    logger.info("Fetching NSE instruments for token map (this can take time)...")
    ins = pd.DataFrame(kite.instruments("NSE"))
    tokens = ins[ins["tradingsymbol"].isin(tickers_u)][["tradingsymbol", "instrument_token"]]
    ...
    return {t: int(mp[t]) for t in tickers_u if t in mp}
...
def _incremental_start_from_existing(mode: str, out_path: str, default_start: datetime) -> datetime:
    ...
    if mode == "5min":
        ...
        back = timedelta(hours=1 * warm)  # base 60m warmup
    elif mode == "daily":
...
def process_ticker(mode: str, ticker: str, token: int, kite: KiteConnect,
    ...
    if fetched is None or fetched.empty:
        logger.info("[%s] %s fetched empty", mode.upper(), ticker)
        return
    ...
    try:
        ...
        # Extra: daily cap by cutoff date (useful in live & previous)
        if mode == "daily":
        ...
        last_dt = pd.to_datetime(merged["date"], errors="coerce").dropna().max()
        logger.info("[%s] %s saved rows=%d last=%s", mode.upper(), ticker, len(merged), str(last_dt))
    except Exception as e:
...
def run_mode(mode: str, context: str, max_workers: int, force_today_daily: bool,
             skip_if_fresh: bool, intraday_ts: str, holidays: set[date], refresh_tokens: bool):
    logger = logging.getLogger("fetcher")
    mode = mode.lower().strip()
    ...
    # 1) Load tickers WITHOUT starting Kite (so skip can be super fast)
    tickers, pre_token_map = load_stock_universe(logger)

    # 2) Per-ticker freshness pre-scan (no Kite calls)
    tickers_need: list[str] = []
    if skip_if_fresh:
        for t in tickers:
            out_path = os.path.join(DIRS[mode]["out"], f"{t.upper()}_main_indicators_{mode}.csv")
            if ticker_is_fresh(mode, out_path, now_ist, context, holidays, force_today_daily, intraday_ts):
                continue
            tickers_need.append(t.upper())
    else:
        tickers_need = [t.upper() for t in tickers]

    if not tickers_need:
        logger.info("[%s] All tickers already fresh — nothing to do.", mode.upper())
        return

    logger.info("[%s] Need update for %d / %d tickers", mode.upper(), len(tickers_need), len(tickers))

    # 3) Now start Kite ONLY if we have work
    kite = setup_kite_session()

    token_map = {k.upper(): int(v) for k, v in dict(pre_token_map).items()}
    missing = [t for t in tickers_need if t.upper() not in token_map]

    if missing:
        fetched = load_or_fetch_tokens(kite, missing, logger, refresh=refresh_tokens)
        token_map.update({k.upper(): int(v) for k, v in fetched.items()})

    # 4) Build work items (only tickers that need update)
    work_items = []
    ...
    if not work_items:
        logger.info("No valid tickers with tokens.")
        return

    logger.info("Processing %d tickers with max_workers=%d ...", len(work_items), max_workers)

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(process_ticker, mode, tkr, tok, kite, start_dt, end_dt,
                            logger, context, holidays, force_today_daily, skip_if_fresh, intraday_ts): tkr
...
def main():
    ...
    if mode == "all":
        for m in VALID_MODES:
            run_mode(m, context=args.context, max_workers=args.max_workers,
        ...
        run_mode(mode, context=args.context, max_workers=args.max_workers,
                 force_today_daily=args.force_today_daily,
                 skip_if_fresh=skip_if_fresh,
                 intraday_ts=args.intraday_ts,
                 holidays=holidays,
